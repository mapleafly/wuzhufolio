...
> **有效需求基线**：**PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}**（`docs/dev/增量台账.md` 表头串；P6/P8/移动端对齐统一引用此串）
...
| 人工门（托盘/读屏/真实 Key/目标机/GUI 全流程/便携版） | §7 | 11 |
| **合计** | | **300** |
...
> **双环境（Windows / Ubuntu）的环境要求、构建运行步骤、安装包获取与取证方式见
> `docs/test/manual-test-guide.md`**（含用例 × 平台 × 路径矩阵、打包版安装/卸载命令、证据模板与故障排查）。
> 每条须记录：执行日期、平台与版本、结果、证据（截图/录屏/抓包文件/benchmark 输出）、结论（通过/不通过）。

### §7.0 人工门执行进展（截至 2026-09-21 · 第十一轮及其续轮）

| 用例 | 平台 | 结果 | 判定轮次/日期 | 证据指向 |
|------|------|------|---------------|----------|
| TC-MAN-01 真实桌面托盘走查 | Windows 11 ✅ · **Linux 未测** | ✅ **通过（Windows 侧）** | **第十一轮续 · 2026-09-21** | 首轮报出的菜单乱码（**DEF-15**）与语言不跟随（**DEF-18/19**）均已修复，本轮在真实 Windows 11 桌面完成正式判定：**菜单三项动作 + 关窗行为 + 托盘驻留 + 后台通知**逐项通过（人工原话「TC-MAN-01托盘走查，在windows11下已经完成通过」）；**Linux / macOS 托盘未测试** → 按人工拍板转 P7 携带（见卡片注与 `manual-test-guide.md §19.1`） |
| TC-MAN-02 开机自启 | Windows 11 · 打包安装版 | ✅ **通过** | **第十一轮 · 2026-09-21** | 第十轮步骤 3 曾弹「Failed to launch JVM」（DEF-42/DEF-43）→ 装 **DEF-43 修复版**（run 35115439554，msi `5d2ec322…`）后复验：安装版可正常启动，**自启开关 → 注册项 → 注销重登驻留 → 关开关复查** 全链路走通；判定口径（登出后自启仍有效）见 `manual-test-guide.md §6`（DEF-45） |
...
| TC-MAN-11 便携版解压即用 | Windows 11 | ✅ **通过** | **第十一轮 · 2026-09-21** | D34 产物（`portable\WuZhuFolio-portable-windows-x64.zip`）解压到纯 ASCII 路径 → 双击 exe 运行正常；判据（数据仍落 `~/.wuzhufolio`、不写注册表/不建系统入口、删除目录即卸载）见 `manual-test-guide.md §17` |

> **同轮补充（第十一轮续 · 2026-09-21）**：**TC-MAN-01 托盘走查 Windows 11 侧判定通过 ✅**（人工原话「TC-MAN-01托盘走查，
> 在windows11下已经完成通过，linux下还未测试」）——该用例**仅剩 Linux / macOS 托盘未测**，继续按 P7 携带（DoD = `M11 §5-3` §4 步骤 4 之 ⑤）。
> **仍未执行 1 项**：**TC-MAN-09**（出站抓包 + 权限实证，Ubuntu 侧；`scripts/outbound-capture-proxy.py` +
> `security-checklist.md §8-4`）——**按人工拍板显式延期 → P7 携带，到期检查点 = P7 发布前，未到点前不得视为已完成**。
> **已由人工判定通过 10 项**（记录日期与轮次见上表；每轮反馈与修复见 `defects.md` DEF-01…DEF-46）：
> TC-MAN-01（Windows）/ 02 / 03 / 04 / 05 / 06 / 07 / 08 / 10 / 11。**2026-09-21 第十一轮及其续轮新增通过：TC-MAN-01 托盘走查（Windows）、TC-MAN-02 开机自启、TC-MAN-11 便携版**；
> 至此 **P6 人工门关闭**（DoD 三项全部达成）。

### TC-MAN-01 真实桌面托盘走查（Windows / macOS / Linux）

> **2026-09-21 Windows 11 判定：通过 ✅**（第十一轮续）——真实桌面完成正式走查：关闭主窗口 → 进程存活并驻留托盘、
> 托盘右键菜单**三项动作**（打开主界面 / 立即同步 / 退出）逐项生效、设置项「关闭窗口最小化到托盘」关闭后关窗直接退出、
> 驻留期间后台同步按通知开关弹出或静默；首轮报出的菜单乱码（**DEF-15**）与语言跟随（**DEF-18/19**）在本轮未复现（对应缺陷均已修复）。
> **Linux / macOS 托盘未测试** → 按人工拍板转 P7 携带（到期检查点 = P7 发布前；GNOME 需 AppIndicator 扩展，无宿主时验证降级分支）。

> **2026-09-14 Windows 首轮走查**：菜单文字乱码（**DEF-15**，已修复：自建 AWT 托盘 + 内嵌字体 + i18n）→ **需复验**（见 `manual-test-guide.md §10`）。
...
> **2026-09-21 Windows 11 复验：通过 ✅**（第十一轮）——打包安装版（DEF-43 修复版 run 35115439554）下，
> 开关 → `reg query HKCU\...\Run` 有项 → 注销重登驻留 → 关开关后注册项消失 全链路走通；
> 该用例此前在第十轮步骤 3 被 **DEF-42/DEF-43**（打包版启动失败）阻断，两缺陷均已闭环。
...
- **前置**：目标低配机（4 GB 内存/双核）或等效受限容器；**打包版自带私有 JRE、目标机无需预装 Java**
  （用开发态跑则需 JDK 17）。
- **无真机时的等效替代（2026-09-14 已执行，人工可裁决放过）**：`taskset -c 0,1 ./gradlew :domain:kdfBenchmark`
  与 `taskset -c 0,1 ./gradlew :domain:backupBenchmark -PbenchXmx=512m` —— KDF 172.6 ms（≪2 s 预算，约 11× 余量）、
  备份轻/典型/重度档峰值 88/139/434 MiB；**首屏计时仍需人工或明确放过**。
...
- **预期口径（DEF-16，2026-09-14 人工门核实）**：**拔网本身不会立即改状态**——指示由「最近一次行情刷新结果」驱动（PRD 故事 3.2-3 / interaction §1.1 N1），最长需等下一轮自动刷新（默认 5 分钟）；手动刷新即刻反映。**这是设计行为，不是缺陷**。
...
> **2026-09-15 Windows 复验：通过 ✅**（第九轮）——六～八轮累计修复 DEF-27…DEF-41（切页入口焦点、窄窗表格与组件统一、
> 卡片指标层级、表格网格线、弹窗尺寸策略）后，本用例在 1024×768 / 1280×800 / 2560×1600（含 150%/200% 缩放）三档下复验通过。
...
### TC-MAN-11 便携版解压即用走查（D34 新增产物）

> **2026-09-21 Windows 11 判定：通过 ✅**（第十一轮）——D34 便携包解压即用验证通过
> （本机以纯 ASCII 路径运行；数据仍落 `~/.wuzhufolio`、不写注册表、删除目录即卸载）。
> Linux/macOS 便携包未人工执行，随 P7 三平台实测携带（见 `test-report.md §7`）。

- **前置**：CI artifact `wuzhufolio-<os>-native`（含 `portable/`）；无需管理员权限。
- **步骤**：① 解压便携包到**纯 ASCII 路径**；② 双击 `WuZhuFolio.exe`（Linux：`bin/WuZhuFolio`）；③ 登录后做一次增资/交易；
  ④ 核对 `HKCU\...\Run` 无 `WuZhuFolio` 项、解压目录内无用户数据、数据仍在 `~/.wuzhufolio`（时间戳新）；
  ⑤ 删除解压目录 → 系统无残留入口。
- **通过判据**：应用可启动并进入登录页；数据只落 `~/.wuzhufolio`；不写注册表/不建系统入口；删除目录即完成卸载。
- **常见假失败**：解压到非 ASCII 路径 → Windows 启动器限制（DEF-42，见 `manual-test-guide.md §16`）。
- **对应验收点**：D34 A5/A6；`manual-test-guide.md §17`。
