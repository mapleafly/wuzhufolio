...
> **状态**：✅ **P6 已关闭（2026-09-21 人工拍板通过）**——DoD 三项全部达成：**P0/P1 清零** + **P2 明确处理结论** +
> **`security-checklist.md` 全部通过**；人工裁决 5 项已按建议全部落地（2026-09-14）；**人工门用例判定通过 10/11**——
> 截至 **2026-09-21 第十一轮及其续轮**：**托盘走查（Windows 11）** / 读屏 / 目标机性能 / 断网态 / 纯键盘 /
> 真实只读 Key 冒烟 / 真实桌面 GUI 全流程 / 外链与关于页 / **开机自启** / **便携版解压即用**；
> **仍未执行 1 项**：TC-MAN-09（出站抓包 + 权限实证）——**按人工拍板显式延期 → P7 携带**（**到期检查点 = P7 发布前**）；
> **TC-MAN-01 仅剩 Linux / macOS 托盘未测**（Windows 侧本轮已判定通过）→ 同样 P7 携带。
> 进展表见 `test-cases.md §7.0`，本轮留痕见 `manual-test-guide.md §19 / §19.1`。
> **结论：达到发布标准，解锁 P7 发布。**
...
| DoD | 结论 | 依据 |
|-----|------|------|
| **P0/P1 缺陷清零** | ✅ **达成（2026-09-21 第十一轮人工复验后归零）**：历史上 P0 = 0、P1 曾出现 6 项均已闭环；第十轮暴露的 **DEF-42**（安装版弹 `Failed to launch JVM`）与 **DEF-43**（打包运行时缺 `jdk.accessibility`）经 **D34 缓解 + 模块集补齐**后，人工在打包安装版复验 **TC-MAN-02 开机自启 ✅ / TC-MAN-11 便携版 ✅** → **P1 = 0** | `defects.md` §0（P1 全部闭环）与 DEF-42/DEF-43 条目；`manual-test-guide.md §19`（复验留痕）；P2 六项全部处置完毕——4 项修复（DEF-01/02/03/06）、1 项登记 P8（DEF-04）、1 项 C0 文档澄清（DEF-05） |
...
| 全量回归绿 | ✅ **达成** | `./gradlew clean build detekt --no-build-cache` → **718 用例（710 执行 0 失败 0 错误 + 8 跳过）** + detekt 0 + 编译警告 0（33 任务全部真正执行；报告时为 678，人工门八轮修复回归增至 718） |

**一句话结论**：P6 全量验证完成，**无 P0/P1 缺陷（2026-09-21 复验后 P1 归零）**，安全与隐私五条硬约束全部通过；人工裁决 5 项已按建议全部落地；
**达到发布标准——P6 门已于 2026-09-21 关闭**：人工门不可替代用例已完成 **10/11**（**托盘走查（Windows）**、读屏、真实 Key、
目标机、GUI 全流程、**开机自启与便携版运行**等）；**仍未执行 1 项 + 未覆盖平台**（TC-MAN-09 出站抓包+权限实证、
TC-MAN-01 的 Linux/macOS 托盘）**按拍板转 P7 携带**，到期检查点 = P7 发布前。
...
→ 718 用例 = 710 执行 0 失败 0 错误 + 8 跳过 · detekt 0 issue · 编译警告 0
```

> **口径说明**：下表为 P6 报告时（`64dcc1a`）的 678；人工门四轮修复期间随缺陷回归递增
> （678 → 683 → 684 → 692 → 700 → 708 → 710 → 713 → **718**），**当前最新为 718 用例（710 执行 0 失败 + 8 跳过）**，
> 对应 `defects.md` DEF-13…DEF-41 的修复回归（**第十一轮复验未新增代码，用例数保持 718**）；各轮数字见 `docs/dev/STATUS.md` 交接记录。
...
| **合计** | **678** | **8** | **0** | 基线（P5 关闭时）为 664 用例；**P6 净增 14 项**；**人工门修复轮再 +40 → 当前 718**（`ui`/`data` 增加：焦点流 5 + 焦点交接 3 + 弹层宿主 3 + 层级统一 1 + 保存语义 2 + 同步契约 2 + 其余键盘/托盘/偏好回归） |
...
**仍需人工执行的用例**（`test-cases.md §7`）：**1 项按人工拍板（2026-09-21）显式延期 → P7 携带，到期检查点 = P7 发布前**——
**TC-MAN-09 出站抓包 + 权限实证（Ubuntu）**；**TC-MAN-01 托盘走查**已在本轮完成 **Windows 11 侧判定（✅）**，
仅剩 **Linux / macOS 托盘**未测（同 P7 携带）。
其余 10 项已人工判定通过（**托盘（Windows）** / 读屏 / 目标机 / 断网态 / 纯键盘 / 真实只读 Key / GUI 全流程 /
外链与关于页 / **开机自启** / **便携版**）；**发布标准已拍板：P6 关闭、解锁 P7 发布**。

---

## 6.1 人工门执行进展（截至 2026-09-21 · 第十一轮及其续轮）

| 用例 | 平台 | 结果 | 判定轮次 | 说明 |
|------|------|------|----------|------|
| **TC-MAN-01 真实桌面托盘走查** | **Windows 11 ✅ · Linux 未测** | ✅ **通过（Windows 侧）** | **第十一轮续 · 2026-09-21** | 关窗驻留托盘 / 托盘菜单三项动作 / 关窗开关行为 / 后台同步通知逐项通过；DEF-15（乱码）与 DEF-18/19（语言跟随）本轮未复现；**Linux/macOS → P7 携带** |
| TC-MAN-03 读屏 NVDA/JAWS | Windows 11 | ✅ 通过 | 第七轮 | 修复 DEF-13（隐形焦点目标）后实测 |
| TC-MAN-04 目标机性能 | Windows 11 | ✅ 通过 | 第七轮 | 含受限环境等效模拟（KDF 172.6 ms ≪ 2 s 预算） |
| TC-MAN-05 断网/代理异常 | Windows 11 | ✅ 通过 | 第八轮 | 口径确认见 DEF-16（拔网不即时改状态属设计） |
| TC-MAN-06 纯键盘全流程 | Windows 11 | ✅ 通过 | 第六轮 | 修复 DEF-13/14/20/21/27 后实测；逐键脚本见 `keyboard-walkthrough.md` |
| TC-MAN-07 真实只读 Key 冒烟 | Windows 11 · Binance | ✅ 通过 | 第六轮 | 修复 DEF-20/22/25/26（焦点交接/弹层/保存语义/不覆盖手写行）后复验 |
| TC-MAN-08 真实桌面 GUI 全流程 | Windows 11 · 三档分辨率 | ✅ 通过 | 第九轮 | 修复 DEF-27…DEF-41（入口焦点/窄窗表格与组件统一/卡片层级/网格线/弹窗尺寸）后复验通过 |
| TC-MAN-10 外链与关于页 | Windows 11 | ✅ 通过 | 第六轮 | 4 个外链入口经系统浏览器打开 |
| **TC-MAN-02 开机自启** | Windows 11 · 打包安装版 | ✅ **通过** | **第十一轮 · 2026-09-21** | 装 DEF-43 修复版（run 35115439554）后：开关 → `reg query` 有项 → 注销重登驻留 → 关开关消失；原阻断项 DEF-42/DEF-43 均闭环 |
| **TC-MAN-11 便携版解压即用** | Windows 11 | ✅ **通过** | **第十一轮 · 2026-09-21** | D34 便携包解压到纯 ASCII 路径即用；数据仍落 `~/.wuzhufolio`、不写注册表、删除目录即卸载 |
| TC-MAN-09 出站抓包 + 权限实证 | Ubuntu（建议） | ⬜ 未执行 | — | 工具与判据见 `security-checklist.md §8-4`；**按拍板转 P7 携带（到期检查点 = P7 发布前）** |

> 完整记录（含每条证据指向）见 `test-cases.md §7.0` 与 `manual-test-guide.md §1.1-1`；
> 2026-09-21 三项判定的留痕见 `manual-test-guide.md **§19 / §19.1**`。**判定通过 10/11**。
...
| **DEF-42 jpackage 启动器非 ASCII 路径限制（上游行为）** | 低 | **已闭环（2026-09-21 人工复验）** + **登记 P8**：D34 已以「per-machine ASCII 默认目录 + 关闭目录选择页 + 便携版」缓解，且人工在打包安装版复验通过（TC-MAN-02 ✅）；默认安装路径不受影响；P7 用户指南增「路径请用英文」提示 |
| **DEF-43 辅助技术（Java Access Bridge）启动失败** | 已闭环 | 模块集补 `jdk.accessibility` + `AssistiveTech` 兜底；CI `INSTALLED_LAUNCH_SMOKE_AT=True` 实证（C0，人工拍板 2026-09-16）+ **2026-09-21 人工实机复验通过** |
| 真实只读 Key 的线上同步未覆盖 | 中 | 人工门（提供 Key 即可随时验）；回环 HTTP 已覆盖同步/校准全链路。**TC-MAN-07 已判定通过**（第六轮） |
| 托盘 / 三平台自启 / 读屏 环境相关项 | 低 | **Windows 侧全部闭环**：读屏（TC-MAN-03 ✅）、开机自启（TC-MAN-02 ✅）、便携版（TC-MAN-11 ✅）、**托盘走查（TC-MAN-01 ✅ 2026-09-21）**；**剩余按拍板转 P7 携带（到期检查点 = P7 发布前）**：Linux/macOS 托盘与自启（GNOME 需 AppIndicator 扩展；无宿主时验证降级分支） |
| **TC-MAN-09 出站抓包 + 权限实证（Ubuntu）未人工执行** | 低 | **P7 携带（到期检查点 = P7 发布前）**：Agent 侧已有运行期抓包证据（仅三白名单主机，§5.2）与打包版权限实证（700/600）；人工按 `security-checklist.md §8-4` 复跑即闭环 |
| 测试文档重复段（`manual-test-guide.md` §16.4 起整段复粘） | 低 | **DEF-46 已修正**（2026-09-21，C0 文档勘误）：删除复粘副本 115 行，保留 §18.1 留痕与「随包运行时无 `java.exe`」口径 |
...
# 全量（当前应 718 用例 = 710 执行 0 失败 + 8 跳过、detekt 0、警告 0；
#       报告时为 678，人工门修复轮增至 718）
./gradlew clean build detekt --no-build-cache
