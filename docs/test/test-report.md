...
> **状态**：**待人工复核**（人工裁决 5 项已按建议全部落地，2026-09-14；人工门用例**已判定通过 7/10**，
> 截至 2026-09-15 第九轮：读屏 / 目标机性能 / 断网态 / 纯键盘 / 真实只读 Key 冒烟 / 真实桌面 GUI 全流程 / 外链与关于页；
> 剩余 TC-MAN-01（待正式判定）、TC-MAN-02（未执行）、TC-MAN-09（未执行）——进展表见 `test-cases.md §7.0`）
...
| DoD | 结论 | 依据 |
|-----|------|------|
| **P0/P1 缺陷清零** | ⏳ **第十轮 2 项 P1 均已修复并通过 CI 验收，待人工实机复验**（DEF-42 打包版启动／DEF-43 辅助技术）：历史上 P0 = 0、P1 曾出现 6 项均已闭环；第十轮人工门暴露 DEF-42（安装版弹 `Failed to launch JVM`）→ 根因 CI 实证 + **D34 修复已落地**（per-machine 安装 + 便携版 + CI 启动冒烟），**待人工 Windows 复验（TC-MAN-02 / TC-MAN-11）通过后归零** | `defects.md` §0：P2 六项全部处置完毕——4 项修复（DEF-01/02/03/06）、1 项登记 P8（DEF-04）、1 项 C0 文档澄清（DEF-05） |
...
建议**具备进入 P7 发布准备的条件**——人工门不可替代用例已完成 7/10（读屏、真实 Key、目标机、GUI 全流程等），
**剩余 3 项**（托盘正式判定、开机自启、出站抓包+权限实证）完成后即可拍板发布标准。
...
> 对应 `defects.md` DEF-13…DEF-41 的修复回归；各轮数字见 `docs/dev/STATUS.md` 交接记录。
...
| TC-MAN-01 真实桌面托盘走查 | Windows 11 | ⏳ 待正式判定 | — | 乱码（DEF-15）与语言跟随（DEF-18/19）均已修复且后续五轮未复现 |
| TC-MAN-02 开机自启 | Windows 11 | ⬜ 未执行 | — | 打包版操作步骤见 `manual-test-guide.md §6` |
| TC-MAN-09 出站抓包 + 权限实证 | Ubuntu（建议） | ⬜ 未执行 | — | 工具与判据见 `security-checklist.md §8-4` |
...
| **DEF-42 jpackage 启动器非 ASCII 路径限制（上游行为）** | 低 | **登记 P8**（人工裁决 2026-09-16）：D34 已以「per-machine ASCII 默认目录 + 关闭目录选择页 + 便携版」缓解，默认安装路径不受影响；P7 用户指南增「路径请用英文」提示 |
| **DEF-43 辅助技术（Java Access Bridge）启动失败** | 已闭环 | 模块集补 `jdk.accessibility` + `AssistiveTech` 兜底；CI `INSTALLED_LAUNCH_SMOKE_AT=True` 实证（C0，人工拍板 2026-09-16） |
