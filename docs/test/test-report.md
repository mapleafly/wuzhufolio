...
| 全量回归绿 | ✅ **达成** | `./gradlew clean build detekt --no-build-cache` → **700 用例（692 执行 0 失败 0 错误 + 8 跳过）** + detekt 0 + 编译警告 0（33 任务全部真正执行；报告时为 678，人工门四轮修复回归增至 700） |
...
> （678 → 683 → 684 → 692 → **700**），**当前最新为 700 用例（692 执行 0 失败 + 8 跳过）**，
> 对应 `defects.md` DEF-13…DEF-21 的修复回归；各轮数字见 `docs/dev/STATUS.md` 交接记录。
...
| **合计** | **678** | **8** | **0** | 基线（P5 关闭时）为 664 用例；**P6 净增 14 项**；**人工门修复轮再 +22 → 当前 700**（`ui` 一列相应增加：`ShellFocusFlowUiTest` 5 + DEF-20 回归 3 + 其余键盘/托盘/偏好回归） |
...
# 全量（当前应 700 用例 = 692 执行 0 失败 + 8 跳过、detekt 0、警告 0；
#       报告时为 678，人工门修复轮增至 700）
./gradlew clean build detekt --no-build-cache
