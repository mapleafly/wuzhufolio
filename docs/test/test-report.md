...
| DoD | 结论 | 依据 |
|-----|------|------|
| **P0/P1 缺陷清零** | ⏳ **第十轮新增 1 项 P1（DEF-42）已修复待复验**：历史上 P0 = 0、P1 曾出现 6 项均已闭环；第十轮人工门暴露 DEF-42（安装版弹 `Failed to launch JVM`）→ 根因 CI 实证 + **D34 修复已落地**（per-machine 安装 + 便携版 + CI 启动冒烟），**待人工 Windows 复验（TC-MAN-02 / TC-MAN-11）通过后归零** | `defects.md` §0：P2 六项全部处置完毕——4 项修复（DEF-01/02/03/06）、1 项登记 P8（DEF-04）、1 项 C0 文档澄清（DEF-05） |
