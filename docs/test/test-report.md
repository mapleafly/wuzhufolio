...
| 全量回归绿 | ✅ **达成** | `./gradlew clean build detekt --no-build-cache` → **678 用例（670 执行 0 失败 0 错误 + 8 跳过）** + detekt 0 + 编译警告 0（33 任务全部真正执行） |
...
→ 678 用例 = 670 执行 0 失败 0 错误 + 8 跳过 · detekt 0 issue · 编译警告 0
```
...
| **合计** | **678** | **8** | **0** | 基线（P5 关闭时）为 664 用例；**P6 净增 14 项** |
...
# 全量（应 678 用例 = 670 执行 0 失败 + 8 跳过、detekt 0、警告 0）
./gradlew clean build detekt --no-build-cache
