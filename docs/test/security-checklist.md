...
**自动化证据**：`./gradlew clean build detekt --no-build-cache` → **636 用例（632 执行 0 失败 + 4 钥匙串真实后端跳过）** + detekt 0 + 编译警告 0（M13 新增 22 项：结构守护 7 + API 隔离 2 + 权限 4 + 脱敏漏斗 5 + KDF 下限 1 + 备份边界 3 + 密钥文件自愈 1 + 路径误伤回归 1）。
