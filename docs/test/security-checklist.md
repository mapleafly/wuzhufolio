...
> **日期**：M13 版 2026-09-12 → **P6 复跑版 2026-09-14** · **有效需求基线**：**PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32}**
...
| 2 | 行情请求外发用户币种 id 集合 | 登记转 P6 | ⏳ **评估完成，待人工定级**（§1.3）：不违反硬约束；最小化边际额度成本 ≈0 | P6 门裁决（接受 / C1 开关 / C2 改默认） |
...
| 7 | 读屏（NVDA/JAWS）实测、完整 a11y 走查 | 登记转 P6 | 🔵 **转人工门**（需真实桌面 + 读屏软件；本机 WSLg 不可执行） | `test-cases.md` §7 用例 3 |
| 8 | 三平台托盘实测 / 打包版开机自启端到端 | 转 P7 | 🔵 **转人工门/P7**（WSLg 无系统托盘；自启仅打包版） | `test-cases.md` §7 用例 1/2·`M11.md §5-2/§5-3` |
| 9 | 签名/公证合规 | 转 P7 | ➖ **P7**（流水线与 Secrets 清单已交付） | ADR-006 §2.1 |
| 10 | 目标机（4GB 双核）KDF ≤2s 复核 | 转 P7 前 | 🔵 **转人工门**（本机实测 151–169 ms，非目标机） | `test-cases.md` §7 用例 5；超标预案 = `KdfParams.OWASP_MINIMUM` |
| 11 | P5-4 备份导出失败模式 | 转 P6 | ✅ **已修复**（DEF-01/06） | — |
| 12 | CMC 兜底计入 CG 额度账本（P6 新发现） | — | ⏳ **待人工定级**（DEF-04，建议 C1） | P6 门裁决 |
...
# 1) 全量构建 + 静检（期望：677 用例 = 669 执行 0 失败 + 8 跳过，detekt 0，编译警告 0）
