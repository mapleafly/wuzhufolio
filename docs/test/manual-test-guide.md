...
| 观察 | 结论 | 处置 |
|------|------|------|
| ① 为 `True`，且 ④ 能跑通 | **安装路径含非 ASCII**（中文用户名/中文安装目录）——**CI 已实证该机制的失败形态与本缺陷一致**（非 ASCII 且系统 ANSI 代码页无法表示时，启动器 `exitCode=2`、无任何应用日志） | 重装时在向导里把目录改成纯 ASCII 路径（`dirChooser` 已开启）；或直接用 ④ 的拷贝目录运行。**根治方案见 §16.5（安装目录显式设 ASCII / per-machine 安装 / 便携版 zip）** |
...
## 20. 0.1.1 修复轮人工复验结果（2026-09-25）✅

> **人工原话**：「**托盘功能通过，windows走查通过**」。

| # | 验收面 | 环境 | 覆盖内容 | 结论 |
|---|--------|------|----------|------|
| 1 | **托盘功能** | Ubuntu 24.04 真机（GNOME 46 / X11 / 缩放 2 的 HiDPI 屏） | 托盘图标完整显示（DEF-48 二轮修复：像素级取证 → 实体位图 + 逻辑尺寸 + `autoSize=false`）；托盘菜单四项（打开主界面 / 立即同步交易 / 立即刷新行情 / 退出）；点选后**可见反馈**（DEF-49：Linux 走应用内提示窗）；「打开主界面」置前（DEF-50） | ✅ 通过 |
| 2 | **Windows 走查** | Windows 真机 | 侧边栏**无**「组件走查」项（DEF-47）；托盘菜单与气泡反馈（DEF-49，Windows 走原生气泡）；候选检索查 `usdt` 首屏为 Tether 且可滚动（DEF-51）；窗口 / 任务栏图标为应用图标（DEF-52①，此前为 Java 默认图标，且托盘在高 DPI 下保持 64px + 系统缩放口径） | ✅ 通过 |

**背景（复验前的两轮修复）**：
- 一轮（2026-09-24 白天）：给托盘图标加透明安全边距 —— 人工复测**仍显示不全**；
- 二轮（2026-09-24 晚）：**像素级取证**（`xwininfo` 定位 XEmbed 窗口 → `xwd` 抓 32×32 内容 → 逐像素分析 + 放大目视）
  定位真因 = AWT 把图标**再乘一次屏幕缩放**后 1:1 画进物理像素窗口 → 改为「实体 `BufferedImage` + `报告值 ÷ 屏幕缩放` + `autoSize=false`」，
