...
| # | 人工观察 | 核实结论 | 复验指引 |
|---|----------|----------|----------|
| 1 | 托盘三行菜单文字**乱码** | **P1 缺陷 DEF-15**：Compose `Tray` 的菜单由 AWT `PopupMenu` 渲染，文字走目标机 AWT 逻辑字体（无法注入字体）；且菜单项硬编码中文 | **已修复**：改为自建 AWT 托盘 + 显式挂内嵌 Noto Sans SC + i18n 文案。复验：重跑打包版 → 托盘右键应见「打开主界面 / 立即同步 / 退出」可读中文；设置切 English 后应变为英文；三项动作分别生效 |
