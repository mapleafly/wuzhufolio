...
### DEF-31 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C1 随方案）· 高 DPI/宽窗下资产列表币种列第三枚徽标被裁
...
| **分级建议** | 随 DEF-37（列宽分配）一并处置：属实现偏差 → **C0**；若人工按「统一组件方案」整体判 C1，则并入 C1 |
...
| **分级建议** | **C0**（实现偏差：窄窗可读性属 PRD §6 一致性要求） |

### DEF-33 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C0）· 表格滚动条压住操作列、手续费长数字换行
...
### DEF-34 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C0）· 环形图图例币种名换行
...
### DEF-35 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C1 随方案）· 截断的数据无悬停全值
...
| **分级建议** | **C1**（新增交互能力——「截断可悬停查看」属可用性增强；未触 §8.1 红线）→ 若人工判 C0 则并入勘误 |

### DEF-36 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C0）· 表单弹窗多出一条「不到一行」的滚动条
...
### DEF-37 ✅ 已修复（**P2** · 人工门第七轮 · 建议 C0）· 列宽分配不保证「每列 ≥ 最小宽」
...
### DEF-38 ✅ 已实现（**P3/体验** · 人工门第七轮 · 建议 C1 随方案）· 列表缺少单元线
...
| **分级建议** | **C1**（新增视觉规范条目：列表单元线口径，写入 `design-tokens.md §4.4`） |
