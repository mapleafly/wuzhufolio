...
唯一约束：表达式唯一索引 UNIQUE(COALESCE(account_id, 0), key)——SQLite 中 NULL 参与 UNIQUE 视为互不相等，普通 UNIQUE(account_id, key) 对全局设置（account_id NULL）不生效（评审 N1）。账户级备份按 key 覆盖（备份优先）。**行情平台 Key（CG/CMC）**：存本表**全局行**（account_id NULL，key=market.coingecko_key / market.cmc_key），按**设备密钥**加密（非账户 DEK，ADR-002 §2.1 方案甲）；**不随 .cpro 备份导出**，恢复后在设置中重新配置（ADR-005 §3）。**行情自选（watch.coins，决策 D21）**：亦存本表**全局行**（account_id NULL，key=watch.coins，value=JSON 数组 [cg_id]，**无数量上限**（D35，2026-09-24：原 50 条上限取消）；未写入=默认种子=仅 USDT）。**成交币自动加入（D35）**：手动交易/编辑、CSV 导入、交易所同步三路触达的币（base ∪ quote）自动并入；幂等（重复不写、顺序不变）、用户移除后再次触达会自动加回。属**展示偏好**（与 theme/fiat 同层，多账户共享），非财务数据，**不随 .cpro 备份导出**（D21 §2 归属口径）。 **M10 通用设置键（T10.1，2026-09-10）**：`display.precision`（精度档 "8/2/2"｜"2/2/2"，M12 数字格式化消费）、
...
**不进 .cpro 备份**（与 theme/fiat/watch.coins 同层）。**M11 无表结构变更，schema 维持 12**。均为全局行、展示/行为偏好层，不进 .cpro。
...
> **成本/已实现的负区间口径（D26，2026-09-11 人工拍板方案甲）**：派生成本遵循不变式
> **「持仓数量 ≤ 0 ⟹ 成本 = 0」**；负持仓期间的流入只对「抬到 0 以上」的部分计入成本
> （按数量比例拆分，`v × (q − d) / q`），清偿部分不建成本；数量归零时成本同步归零。
> 影响字段：`CoinHolding.costFiat`/`avgCostFiat`/`realizedPnlFiat` 及其派生的
> 浮动盈亏/已实现盈亏/总收益/ROI（`PortfolioCalculator`）。决策档
> `docs/dev/decisions/D26-负持仓区间成本口径.md`；界面以「成本不可靠」徽标标注曾转负的币种。

> **折算价口径（D27 + D28，2026-09-13 人工拍板）**：**1:1 锚定集合固定为 USDT（tether）**
> （D28 修订 D27 的「白名单 = 锚定集合」写法），在基础法币为 USD 时**折算价恒取 1**（不查市价快照）——
> 同时作用于事件折算（成本基数 / 卖出所得 / 资金 base_amount / 手续费）与当前估值
> （市值 / 可用现金 / 未实现 / 24h 恒 0）。**现金类币种白名单**（可用现金口径）默认 = {USDT}，
> 其余稳定币由用户在设置中增删，**其金额按市价快照折算（不锚定）**；
> 非白名单币种与非 USD 基础法币同样照市价。决策档 `D27-稳定币1比1折算口径.md`（§2 已被修订）、
> `D28-白名单收敛USDT锚定.md`；任务 T4.6。
>
> **负持仓口径（D29，2026-09-13 人工拍板方案 B）**：持仓数量 < 0 的币种（只可能来自导入路径的数据缺口——
> PRD「导入路径例外」）其市值**不计入净值与可用现金**，被排除金额单列 `PortfolioMetrics.anomalousExcludedFiat`
> 并在仪表盘显式提示；币种行自身仍展示负市值与「持仓异常」标记。决策档 `D29-负持仓不计入净值.md`；任务 T4.7。

| 派生值 | 公式/来源 | 需求依据 |
|--------|-----------|----------|
| 持仓数量/平均成本 | 三类事件按时间升序重放（**数量 < 0 的币不进净值/可用现金**，D29） | 共享规范 §2 |
...
| 可用现金余额 | 现金白名单持仓折算基础法币之和（**USDT 按 1:1；扩展项按市价**，D27+D28） | PRD 名词解释、共享规范 §2 名词 |
| 24h 盈亏 | Σ(持仓×现价) − Σ(持仓×24h 前价)，同源/覆盖 N/M 口径（**USDT 恒 0**，D27） | PRD 名词解释 |
