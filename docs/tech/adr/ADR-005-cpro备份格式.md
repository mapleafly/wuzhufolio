...
- 备份密码独立设置；须满足与账户密码相同最低强度（PRD 故事 5.2-3）。
  **修订（2026-09-10，amends D24——人工裁决，见 `docs/dev/decisions/D24-备份密码独立设置不回填.md`）**：
  「导出默认填当前账户密码」不可实现（密码永不落盘/不留存，PRD 5.1-4、ADR-002 §2）——
  改为**由用户设置独立密码（不回填、禁止空密码）**；恢复语义不变（只依赖备份文件密码）。
