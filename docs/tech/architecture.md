...
> **负持仓区间的成本口径（D26 · 2026-09-11 人工拍板方案甲，C2 返工）**：全量重放引擎对「数量 ≤ 0」的
> 持仓执行**不变式「数量 ≤ 0 ⟹ 成本 = 0」**；流入（买入到账/卖出入账/增资/正差额校准）在负持仓上
> **只对「抬到 0 以上」的部分**按数量比例计入成本（穿越点拆分），清偿部分不建成本。
> 语义 = 「你为当前持有的这些币实际付了多少钱」；对从未转负的账本与旧口径逐字等价。
> 详见决策档 `docs/dev/decisions/D26-负持仓区间成本口径.md`、task-breakdown T4.5。
