...
| 侧边栏 | 仪表盘 / 资产列表 / 交易管理 / 资金管理 / 行情 / 设置 六个一级入口（行情页为 D21 增补，2026-09-04） | — |
...
**键盘焦点流（D31，2026-09-15 人工门第四轮）**：主壳三区的焦点编排固定为——

1. **进入页面**：在侧边栏项上按 **Enter**（或对当前页项再次 Enter）→ 焦点**直接进入页面内容**，
   落到**页面声明的入口控件**（DEF-27：`Modifier.pageEntryFocus()`；设置=基础法币、交易/资金/行情=搜索框、
   资产列表=首列排序、币种详情=返回；未声明页面回落「子树第一个可聚焦控件」）——不必再 Tab 穿过侧边栏其余项与顶栏；
2. **回到外壳**：焦点在页面内容内时，**未被页面控件消费**的 **Esc / ↑ / ↓** → 焦点回到侧边栏**当前页项**，
   恢复「侧边栏 7 项 → 顶栏 3 项 → 页面内容」的完整循环；
3. **侧边栏内**：**↑ / ↓** 在导航项之间移动焦点（顺序同渲染顺序：仪表盘→资产列表→交易管理→资金管理→行情→设置→组件走查）；
4. **弹层优先**：任一弹层（`WzModal` / `InPlaceModal`）打开时，Esc/方向键归弹层，外壳不接管；带 Ctrl/Alt/Meta 的组合键
   外壳一律不接管（留给 P8 全局快捷键）；
5. **只读页面**（仪表盘卡片/环形图等不参与 Tab 序）没有可聚焦控件 → 焦点留在侧边栏属预期，Tab 仍可到达页面内的操作按钮。

> 逐键脚本与实测顺序见 `docs/test/keyboard-walkthrough.md`；决策依据见 `docs/dev/decisions/D31-键盘焦点流.md`。
...
- **入口**：资产列表点击单行。
- **核心元素**：汇总（持有数量、当前持有价值、占总资产占比、平均成本、总浮动盈亏、累计已实现盈亏）；该币种交易记录列表（卖出行显示该笔已实现盈亏，按**交易所/类型/时间**三维筛选 + 搜索（时间档位口径与资金页一致：全部时间/近 30 天/30–90 天/90 天以上，D30）；滚动加载）；「以交易所余额校准持仓」按钮（仅单一数据来源显示）；校准历史列表。
...
- **入口**：侧边栏「设置」。
- **核心元素**（分组）：通用（基础法币、主题明/暗、**界面语言（中文 / English，D25 增补 2026-09-11）**、盈亏颜色方案、默认精度、登录页用户名枚举开关、稳定币白名单、小额币种阈值）；网络（系统代理，PRD 6.2）；托盘与后台（最小化到托盘、开机自启、同步通知、备份提醒）；行情与同步（行情刷新频率、API 同步间隔、CoinGecko API Key、CoinMarketCap API Key）；**日志与诊断（查看日志、导出日志（导出前提示检查敏感信息）、生成诊断报告（复用日志脱敏规则、内容受限清单：版本/schema/脱敏日志片段/调用计数），PRD 6「日志管理与可追溯性」**）；手续费设置；API 管理；数据管理；关于。
...
### 2.19 行情页（Market Watch，D21 增补 2026-09-04）

- **入口**：侧边栏「行情」（六个一级入口之一；V1.9 范围增量见 `docs/dev/decisions/D21-行情浏览页-范围增量.md`）。
- **核心元素**：只读现价列表——每行币种 symbol/名称、现价（基础法币，settings.fiat）、数据源（COINGECKO /
  COINMARKETCAP / 无行情 "--"）、更新时间；顶部「立即刷新行情」与数据源指示；「我的自选」搜索添加（coins 目录检索，
