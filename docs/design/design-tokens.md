...
- **层级**：
  ...
  - 页面标题：20px/600；
  - **卡片指标数字两级（DEF-39，2026-09-15）**：一级 `metricPrimary` = **27px/600 衬线 + tnum**、次级 `metricSecondary` = **21px/600**（对齐原型 `.card .big` / `.big.sm`，比值 ≈1.29；**禁止**再用 32sp/14sp 这类跨度过大的组合）；
  - **分组/区段一级标题：15px/600**（P6 人工门第五轮 DEF-24 确立；Compose 令牌 `WzTypography.sectionTitle`，色 `ink`）；
  - **卡内二级标题：14px/600**（Compose 令牌 `bodyStrong`，色 `ink`）；
  - 表格头：12px 大写/加宽字距；
  - 正文：13–14px（原型现状：导航 13.5px、正文 12.5–13.5px；P4 实现以 14px 为正文默认）；行标签/设置项标题 = 14px/400（`body`，色 `ink`）；
  - 表格/密集数据：≥ 12.5px（原型 12.5px）；
  - 标签/注释：11–12px（11px 仅限 hint/表头/时间戳等非关键辅助信息，且须满足对比度要求）。
- **设置页层级统一（DEF-24，2026-09-15 人工门第五轮确立）**：设置页（含各分组组件）**只允许四种标题取值**——
  ① 页面标题「设置」= 20/600；② 分组一级标题（通用 / 网络 / 托盘与后台 / 行情与同步 / 日志与诊断 / 手续费 / API 管理 / 数据管理 / 关于）
  = **15/600（`sectionTitle`）**；③ 卡内二级标题（数据管理的备份/恢复/明文导出、手续费的全局默认费率/交易所费率、
  API 管理的已保存密钥/最近同步记录、行情与同步的子段标题）= **14/600（`bodyStrong`）**；④ 行标签/设置项标题 =
  14/400（`body`）；说明文字 = 11/400（`caption`，`ink3`）。**禁止**在分组组件里各自另取字号（此前 `caption`/`pageTitle`/
  `bodyStrong` 三种并存，导致「一级标题比二级还小」「同一层级大小不一」）。守护测试：`SettingsPageUiTest::all settings first level titles share one typography level`。
- 中文标点使用「」引号。
