...
- **勘误（C0）**：`CoinResolutionException` 增类型化 `kind`、`FeeRulePolicy.validate` 改类型化 `Error`
  ...
  detekt 配置补回测试目录排除
- 测试：**602 用例（598 执行 0 失败 + 4 跳过待 CI win/mac）** = domain 204 / data 253（+16） /
  ui 129（+31） / app 16；detekt 0；**编译警告 0**；无缓存 clean build 32 任务全执行
...
② 复跑 `./gradlew clean build detekt --no-build-cache`（应 602 用例 = 598 执行 0 失败 + 4 跳过 + detekt 0 + 警告 0）；
