...
| 2026-09-08 | Agent | **修复 M2 遗留 CI 缺陷** | 推送后核查：上轮 CI macos/windows Test 失败——remember-me parse 形状/账户/长度分支 require/error 抛 IAE，与 KDoc 契约（损坏一律 ISE）不符（Linux 跳过未暴露，win/mac 真实后端首跑暴露）；统一为类型化 ISE + 全平台 parse 契约回归测试（C0 勘误）；M7.md §7.9；CI run 34323064010 复跑观察 |
