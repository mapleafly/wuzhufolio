...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M6 交易所同步 ✅ 已通过（2026-09-07 人工「M6通过」）**；M1–M6 均已通过（含 D21 行情页增补 + M6 三轮修复/诊断）。P0–P3 均已关闭。
...
- **下一人工门**：**M7 交易管理（T7.1–T7.4）启动**——M6 已通过解锁；transactions 表（M009）已建、120 笔真实同步成交已入账待窗口呈现；依赖 T3/T4（过）+ M6 产出的 transactions 仓库；待人工下达启动指令（建议 M7，见 M6.md §9）。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ✅、M4 ✅、M5 ✅、**M6 ✅（2026-09-07 人工通过，含三轮修复/诊断）**；下一模块 = M7/M10（待启动指令） |
...
- **P4-M7 待启动**：M6 已通过（2026-09-07 人工「M6通过」）——空校验/B2/保存即首次同步/真实 Key 入账 120 笔（BNB 歧义根因闭环）/增量幂等/编辑/移除全部实测通过；M6.md §8 三轮闭环。下一模块 = **M7 交易管理**（T7.1–T7.4：交易列表页让同步成交在窗口可见 + 手动增删改 + CSV 历史导入 + 手续费自动计算；依赖 T3/T4 过 + M6 transactions 表/仓库就绪）或并行面 M10 设置日志，待人工下达启动指令（建议 M7）。转后续模块注意清单：M6 遗留（同步定时宿主归 M11、SettingsSectionsHost 汇合归 M10、真实 Binance 冒烟留 P5、api_keys 备份归 M9、CI 三平台复跑待推送、歧义资产人工固化入口未建——如再现「歧义」样本需人工确认映射，M7/M10 评估）+ M5 遗留（折算解析接线归 M7、调度循环宿主归 M11、状态栏提示归 M12、真实 API 冒烟留 P5）+ M4 遗留（事件构造层接线归 M7/M8、费率 CRUD 接线归 M7/M10、校准执行流归 M8、校验宽松口径复核归 M8）+ M3 遗留（搜索扫描优化、CI 三平台复跑待推送）+ M2 遗留（手动/CSV/恢复向导占位归 M7/M9、枚举开关 UI M10、顶栏/下拉/眼睛 M12）+ 4GB 目标机 KDF 复核改为 M13 前开放待办。
