...
- **当前状态**：**P4 分模块开发 ✅ 已通过（2026-09-12 人工「M13 通过」关闭）——M1–M13 全部通过，P4 收尾完成**；
  P0–P4 全部关闭。**P5 集成与联调 解锁为「进行中」，待人工下达启动指令。**
- **推进顺序**：先桌面端，后移动端。**P1–P8 只针对桌面端或两端共同部分；移动端相关工作放到下一个版本。**（移动端相关技能/技术方案/开发待桌面端主线稳定后再启用。）
- **下一人工门**：**P5 集成与联调启动指令**——目标：把 P4 十三模块拼起来打通核心用户旅程
  ...
  **P5 启动时须带上 P4 交接的开放项**：P6/P7 待办清单见 `docs/dev/modules/M13.md §6`
  （读屏实测、`.cpro` 大载荷内存、签名/公证、托盘与自启实测等）。
...
| P5 | 集成与联调 | 进行中（待启动指令） | `docs/test/integration-report.md` | P4 关闭后解锁（2026-09-12）；输入 = P4 全部模块 + api-contracts |
...
- **阶段推进点（2026-09-12）**：**P4 分模块开发 ✅ 已通过（M1–M13 全部通过）** —— 无待审核门；
  **P5 集成与联调 已解锁为「进行中」，待人工下达启动指令**（目标：登录 → 增资 → 交易 → 看板/ROI →
  备份恢复主流程打通 + `docs/test/integration-report.md`；输入 = P4 全部模块 + `api-contracts.md`）。
- **P5 启动须携带的开放项**（各自到期检查点见条目，不得视为已完成）：
  ① M11 §5-3 托盘 GUI 走查 → 真实桌面（检查点 = P6 系统测试）；
  ② M11 §5-2 打包版开机自启端到端 → P7 打包版；
  ③ D25 原型补「界面语言」行（检查点 = P5 联调或 P7 前视觉终审）；
  ④ 签名/公证合规实证 + Linux 包 GPG 签名 + 证书采购 → P7（ADR-006 §2.1）；
  ⑤ `.cpro` 非流式读写（大载荷内存曲线）→ P6；⑥ 行情请求币种集合隐私最小化 → P6；
  ⑦ settings 键命名空间守护建议 + 登出后调度 tick 噪声 → P6。
