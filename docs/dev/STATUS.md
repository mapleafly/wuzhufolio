...
| 24 | 上述四项的**变更分级** | 待人工拍板：**DEF-27/28/29/30 均建议 C0**（实现偏差纠正：焦点顺序、窄窗可读性、候选浮层语义均属既有要求未正确落地；DEF-28 若判「1024 允许横向滚动属交互口径变化」则可改判 C1） |
...
30. **⏳ 待人工拍板：第六轮四项变更分级（DEF-27/28/29/30，2026-09-15）**：四项**均建议 C0**（实现偏差纠正）——
    **DEF-27**（切页入口焦点契约 `PageEntryFocusState` + `Modifier.pageEntryFocus()`）属 DEF-21 落地方式未达
    「焦点顺序合理」；**DEF-28/29**（窄窗表格与按钮不再换行/竖排：新增 `AdaptiveTable`、过滤行 `FlowRow`、按钮单行文案）
    属 PRD §6 一致性/核心操作可达的实现要求；**DEF-30**（行情候选浮层随清空/Esc 收起）属 `interaction.md §2.7` 既有语义
    未正确落地。**若人工认为「1024×768 下允许横向滚动」属交互口径变化，可将 DEF-28 改判 C1**（则需补决策档 + 台账 +
    下游回写）。四项均已修复并加回归，**不阻断** P6。
