...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M3 币种主数据 ⏳ 待审核（2026-09-03 产出，停人工门）**；M1 ✅、M2 ✅（2026-09-03 人工通过）；M3 通过后解锁 M4 计算引擎。P0–P3 均已关闭。
...
- **下一人工门**：**M3 币种主数据验收**——验收标准见 docs/tech/task-breakdown.md §3 M3 + 模块记录 docs/dev/modules/M3.md §3/§4；依赖 T1（已过）。人工验收通过后解锁 M4 计算引擎（T4.1–T4.4，黄金用例硬前置）。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ⏳ 待审核（2026-09-03 产出，停人工门）；M3 通过后解锁 M4 |
...
## P4 · M3 币种主数据（⏳ 待审核--2026-09-03 产出，停人工门）
...
- **P4-M3 停人工门**：M3 币种主数据已于 2026-09-03 产出并置「待审核」（代码全绿：129 测试 0 失败、detekt 0、GUI schema=5 迁移实证）；**待人工按 docs/dev/modules/M3.md §4 验收**。转 M4 注意清单：M3 遗留（搜索扫描优化、status 维护/无行情归 M6、cmc_id 对齐归 M5、排名提供者归 M5、CI 三平台复跑待推送）+ M2 遗留（向导占位 M6/M7/M9、枚举开关 UI M10、顶栏/下拉/眼睛 M12）+ 4GB 目标机 KDF 复核改为 M13 前开放待办。
