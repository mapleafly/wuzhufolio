...
- **上一阶段状态**：**P6 系统测试与质量 ✅ 已通过（2026-09-21 人工拍板关闭）**——按 PRD V2.0 验收标准完成全量验证，
  ...
  **✅ P0/P1 缺陷 = 0**（P1 六项 DEF-13/15/20/22/25/27 由人工门暴露并修复闭环；第十轮的 **DEF-42 / DEF-43**
  经 **D34 缓解 + `jdk.accessibility` 补齐**后，**2026-09-21 人工在打包版复验通过**）；P2 全部有明确结论；
  **人工裁决 5 项（2026-09-14「5项都按建议来处理」）全部落地**（新增决策档 **D30** + 台账 + T12.5）；
  **Windows 11 人工走查已执行十一轮（2026-09-14 ~ 09-21）**：累计报出 32 条 ——
  **5 项 P1 + 6 项 P2 缺陷已修复闭环**（DEF-13 焦点链重复目标 / DEF-15 托盘乱码 / DEF-20 候选选中后焦点掉出弹窗 /
  **DEF-22·DEF-23 设置页弹层撑开页面与错位** / **DEF-25 添加密钥保存后弹窗不关** / DEF-14 登录回车不提交 /
  DEF-18·DEF-19 托盘语言不跟随 / DEF-21 焦点流 / **DEF-24 设置页层级不统一**），
  **2 项口径确认（非缺陷）**：DEF-16 断网指示在下次刷新才变（符合 PRD 与 interaction N1）、
  **DEF-26 同步不覆盖手写交易（已加回归）**，**1 项等效替代**（目标机性能：2 核受限模拟 KDF 172.6 ms ≪2 s）。
  **变更分级已拍板（2026-09-15「按建议变更分级」）**：**DEF-20 = C0**（勘误已登记 `M7/M8` 模块记录）、
  **DEF-21 = C1 → 决策档 D31 已建档**（台账 + 索引 + **T12.6** + `ia.md §1.1` + `interaction.md §3-9` 全部回写完毕）；
  **第十一轮（2026-09-21）人工复验通过 2 项**：**TC-MAN-02 开机自启 ✅**（打包安装版：开关 → `reg query` 有项 →
  注销重登驻留 → 关开关消失）与 **TC-MAN-11 便携版解压即用 ✅**（D34 便携包解压到纯 ASCII 路径即用），
  被测产物 = DEF-43 修复版（run 35115439554，`build=0.1.0+6f2c77b`）→ **D34 §6 A2/A5/A6 闭环、DEF-42/DEF-43 闭环、P1 归零**；
  **人工门用例累计通过 10/11**（**TC-MAN-01 托盘走查（Windows 11）** / 02 / 03 / 04 / 05 / 06 / 07 / 08 / 10 / 11 ✅；
  记录表见 `test-cases.md §7.0`、`manual-test-guide.md §1.1-1`，2026-09-21 留痕见 `manual-test-guide.md **§19 / §19.1**`）。
  **第十一轮续（2026-09-21）新增通过：TC-MAN-01 托盘走查（Windows 11 侧）**——人工原话「TC-MAN-01托盘走查，在windows11下已经完成通过，
  linux下还未测试」；菜单三项动作 / 关窗行为 / 驻留 / 后台通知逐项通过，DEF-15 与 DEF-18/19 未复现。
...
- **P7 携带项（含 P6 显式延期两项，到期检查点 = P7 发布前，未到点前不得视为已完成）**：
  ...
  {api.coingecko.com, pro-api.coinmarketcap.com, api.binance.com} + 数据目录 700 / 密钥与库 600，判据 `security-checklist.md §8-4`）；
  ③ macOS / Linux 便携包与打包版开机自启实测（Windows 侧 2026-09-21 已 ✅）；④ 签名/公证合规实证 + Linux 包 GPG 签名 + 证书采购（ADR-006 §2.1）；
  ⑤ 托盘正式图标与打包图标、jlink 裁剪与字体子集化；⑥ 发布产物 SHA256 / CHANGELOG / 用户指南
  （含「安装/解压路径请用纯英文」与「备份文件密码强度 = 凭证保护强度」提示）；⑦ **产品宣传动画（P7 必做项，AGENTS.md §7.2-3）**。
...
- **三项延期验证在册**（到期检查点与 DoD 见各自条目，未到期前不得视为已完成）：
  ① ~~M11 §5-3 托盘 GUI 走查 → 真实桌面~~ → **Windows 侧 ✅ 已闭环（2026-09-21：TC-MAN-01 判定通过）**；**仅剩 Linux / macOS 托盘 → P7 携带**（到期检查点 = P7 发布前）；
  ② ~~M11 §5-2 开机自启端到端 → P7 打包版~~ → ✅ **已闭环（2026-09-21，提前于 P7：TC-MAN-02 通过）**；
...
9. **【第三轮：人工裁决 2 项已实施（D28 / D29，均 C2，第二轮追问的落地）】**
   ① **D28（amends D27）白名单收敛**：**1:1 锚定集合固定 = 仅 USDT**（`PortfolioCalculator.ANCHORED_COIN_IDS`，
   不可由设置扩展）；**现金白名单默认 = {USDT}**（`CASH_COIN_DEFAULTS` 由 4 项收敛为 1 项），
   其余稳定币用户可增删且**按市价折算**；事件构造层参数改为 `anchoredCoinIds`（锚定与白名单解耦）；
   行情页自选开箱种子与白名单解耦（`MarketConfig.DEFAULT_WATCH_SEED`，展示偏好不变）。
   决策档 `docs/dev/decisions/D28-白名单收敛USDT锚定.md` + 台账修订行（不覆盖 D27）+ 索引 + T4.6 修订 +
   data-model/api-contracts 补注 + M8/M10 模块注。
   ② **D29 负持仓不计入净值（方案 B）**：数量 < 0 的币市值**不计入净值与可用现金**，
   被排除金额单列 `PortfolioMetrics.anomalousExcludedFiat` 并在仪表盘显式提示
   （`dashboard-anomaly-notice`，中英双档）；币种行仍展示负市值 + 「持仓异常」徽标。
   决策档 `docs/dev/decisions/D29-负持仓不计入净值.md` + 台账 + 索引 + **T4.7** + data-model/api-contracts 补注 +
   interaction.md **§2.8** + M12 模块注。回归：域层 +2（负持仓两用例改写/新增）、UI +1（提示行）、
   设置/组合测试断言更新。
10. **【负持仓来源结论（回答人工第 2 问）】**：负持仓**只来自导入路径**——手动新增/编辑/删除被 PRD
   「重放校验规则」的相对校验挡住（V5/V7/V9），校准的负差额 ≤ 本地持仓；CSV/API 导入按 PRD
   「导入路径例外」不做余额校验，缺入金/缺早期成交/超出 API 500 条窗口/当时未解析跳过都会留下负持仓。
   即：**它不是真实头寸，而是账本数据缺口信号**，PRD 要求标记 + 引导补录（附录 A 用例 6 闭环）。

11. **【第四轮：PRD 基线回写 + 原型/自选种子收敛（2026-09-13 人工拍板）】**
    ① **回写《跨端共享规范》/PRD 正文**（人工明确指令；§8.3「改则升版本号并走评审」）：
    `跨端共享规范.md` **V1.0 → V1.1**、`桌面端prd.md` **V1.9 → V2.0**；修订 8 处表述（名词表：资产净值/可用现金余额/
    稳定币白名单 + 新增「1:1 锚定折算」「持仓异常」；全局说明：折算价格解析/导入路径例外；故事 6.1/7.2；
    附录 A 用例 6）；评审材料 = **`docs/prd/桌面端prd-V2修订说明.md`**（差异清单 + 影响面核对 + 评审要点，
    📌 **待人工评审确认**）。有效需求串 = **PRD V2.0** + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}；
    移动端 PRD/SRD 随下一版本按共享规范对齐（本轮不启用移动端工作）。
    ② **行情页自选开箱种子收敛为仅 USDT**（人工拍板）：`MarketConfig.DEFAULT_WATCH_SEED` 由 4 币 → 1 币；
    原型 `wuzhufolio-light.html` 的 `WATCH_SEED` 与 `prototype-verify.js` 断言同步（复跑 `errors=[]`、
    默认行数 1 / 添加后 2 / 移除后 1）；数据层测试 3 处期望更新。

12. **【第五轮：缺价假亏损修复 + 锚定口径确认（2026-09-13 人工验收反馈）】**
    ① 人工实测「增资 100,000 → 买入 1 BTC@50,000 → 仪表盘 ROI 显示 **−50%**」经复核＝**缺价币不计入净值**
    + **记录成交不触发行情刷新**的叠加（BTC 无快照 → 净值只剩现金 50,000 → 总收益 −50,000）；
    新增回归用例锁定该数值（缺价 −50.00% / 补价后 0.00%）。
    ② 修复（C0 实现/表达补全，未改产品语义）：**聚合页缺价自动补价**（`PortfolioViewModel.maybeAutoPrice`，
    静默、60s 冷却、持仓 ∪ 自选）+ **仪表盘缺价提示**（`dashboard-unpriced-notice`，中英双档）。
    ③ 同轮人工裁定：**不再允许 USDT 之外再增锚定币**；明文确立「**锚定币 ≠ 稳定币**」——稳定币可增删、
    非 1:1、有实时价格（写入 D28 §5-3、台账 D28 行、《跨端共享规范》V1.1 与 PRD V2.0 术语）。
...
## P4 · M8 资金管理（✅ 已通过--2026-09-09 人工「gui走查完成。M8通过」关闭）

> 启动记录：人工原话「执行p4-M8」（2026-09-09）。范围 = task-breakdown M8（T8.1 增资/撤资 · T8.2 校准
> 记录入列 + 校准执行流 · T8.3 资金页+表单 UI）。**模块记录：docs/dev/modules/M8.md**（实现摘要 /
> 文件清单 / 验收清单 / 规格落档 6 条 / 遗留）。
> 里程碑门槛（M7–M8）：interaction V 系全过 + 黄金用例 2/3/4/5/6/8——本模块覆盖 2/3/4/7/8 的接线形状
> （引擎数值 M4 已守护）。
> **关闭记录（2026-09-09）**：人工原话「gui走查完成。M8通过」——验收覆盖：资金页（总览卡/命令区/
> 混合列表/筛选搜索）、增资撤资（默认币种预选/候选点选直达保存/法币提示/撤资 V7/编辑删除 V9）、
> 校准三态（多来源提示/无密钥引导/预览+执行+sync_logs 留痕）；两轮走查反馈（§8-1 候选点选直达
> 保存、§8-2 默认币种置顶）全部闭环；402 测试 0 失败 + detekt 0 + 无缓存 clean build 全绿 + GUI
> 冒烟 schema=12 无异常。**M8 关闭 ✅，M9/M10/M11 解锁待启动指令**。

**产物清单（代码 + 测试，全量绿）**：

- **T8.1**：domain/ledger/FundModels（FundService 契约 + FundConflictClassifier：撤资 V7/其余 V9）+
  **M011 capital_flows / M012 reconciliation_records（schema 10→12，金额 TEXT 勘误链）** +
  FundFlowRepository/ReconciliationRepository + **事件构造层资金半边**（LedgerEventAssembler：三类记录
  → FundEvent/AnchorEvent，确定性决胜序 (at,seq,类型序,uuid)）+ DefaultFundService（相对校验 +
  **V7 撤资同点绝对校验收紧**[M4 §5-5 复核结论] + 折算[记录时价链；完全缺失 = 名义零折算 + estimated，
  黄金 9 回填自动纠正] + 法币不入账提示[孪生法币点名稳定币] + 默认币种 USD→USDT/其余→USDC）
- **T8.2**：CalibrationUseCase + DefaultCalibrationService——单一来源门（多来源提示 PRD 原文）→ 只读
  密钥（最近同步优先）→ fetchBalances 实时余额（**CoinCatalog.exchangeAssetFor 反向映射** + symbol 兜底）
  → 校准时市价（不可得 → NO_MARKET_PRICE，PRD「行情完全不可用时暂不允许执行」）→ plan 差额规划 →
  锚点入库前相对校验（锚点后冲突 V9）→ reconciliation_records 入库 → **sync_logs 留痕**（PRD 故事
  4.1-5）；校准行以「校准」类型入资金列表（不可编辑、可删除[删除同样相对校验]）
- **T8.3**：ui/ledger 六文件（FundsCopy/VM/FundFormModal/CalibrationModal/FundsPage/Parts）——总览卡
  （可用现金 + 投入本金（净）+ 累计增撤副行，PortfolioCalculator 单一真源）+ 命令区（增资/撤资/编辑/
  删除/**校准持仓**[入口偏差登记 M8.md §5-3，M12 迁移币种详情] + 类型/日期筛选 + 搜索）+ 混合列表
  + 删除确认框（「将删除 N 条资金记录并重算投入本金与 ROI」）+ 表单（币种补全/法币实时提示/默认
  稳定币/折算预览）+ 共性约束 7.3（首输入框聚焦 + performTextInput/assertIsFocused）
...
**本次改了什么**：见模块记录 §1（三任务摘要 + 交易半边接线 + §5 规格落档 6 条，重点：V7 撤资同点
绝对校验、校准锚点入库前相对校验、校准入口暂挂资金页、名义零折算、三类事件决胜序、M011/M012
TEXT 勘误）。
...
**GUI 走查修复轮（2026-09-09，人工 3 项反馈，见 M8.md §8）**：①② 增资/撤资表单输入 USDT（同名
...
**建议的下一步**：人工审核 M8 → 通过后按依赖图解锁 **M9 备份恢复**（.cpro 全量就绪）或并行面
**M10 设置与日志诊断** / **M11 桌面集成**，由人工拍板。

## P4 · M9 备份恢复（✅ 已通过--2026-09-10 人工「m9通过」关闭）

> 启动记录：人工原话「执行P4-M9」（2026-09-10）。范围 = task-breakdown M9（T9.1 CproCodec ·
> T9.2 增量合并/全量覆盖 · T9.3 恢复流程与 CSV 明文导出 · T9.4 备份恢复 UI）。**模块记录：
> docs/dev/modules/M9.md**（实现摘要 / 文件清单 / 验收清单 / 规格落档 6 条 / 遗留）。
> 里程碑门槛（M9）：**备份往返无损、幂等（黄金用例 10）——planner 级 + 真实库服务级双重覆盖全绿**。
> 本模块无 schema 变更（7 张业务表 M001–M012 已全部就绪；schema 维持 12）。
> **走查反馈修复轮（2026-09-10，人工门验收反馈 2 项 + 1 项裁决，见 M9.md §7）**：
> ① 保存文件自动补全 `.cpro`/`.csv` 后缀（恢复侧维持不校验扩展名——格式由明文头部识别）；
> ② 恢复摘要异常拆分「本次导入新产生 / 账户既存」（同账户回导既存异常不再误读为导入造成）；
> ③ **偏差裁决 D24：备份文件密码由用户独立设置（不回填当前账户密码、禁止空密码）**——决策档
> `docs/dev/decisions/D24-备份密码独立设置不回填.md` + 台账登记 + 五处下游回写（flows §6 /
> ADR-005 修订 / 原型两处文案 / task-breakdown T9.4 / api-contracts §3）；
> **有效需求版本串更新 = PRD V1.9 + Δ{D21, D24}**（P6 备份用例与 P8 复盘以此为备份密码口径）。
> **关闭记录（2026-09-10）**：人工原话「m9通过」——GUI 走查（备份生成/增量回导/跨账户恢复/CSV/错误
> 路径）+ 修复轮两项复验通过（后缀自动补全；异常「新产生/既存」拆分）+ D24 裁决落档；全量 430 执行
> 0 失败（434 报告，4 跳过待 CI）+ detekt 0 + clean build 全绿 + GUI 冒烟 schema=12 无异常。
> **M9 关闭 ✅，M10/M11/M12 解锁待人工启动指令**。

**产物清单（代码 + 测试，全量绿）**：

- **T9.1 CproCodec**（domain/backup，纯计算）：文件布局 = 头部 JSON 单行 + '\n' + nonce + 密文
  （**AAD 绑头部完整字节**，防头部篡改）；Argon2id(备份密码, 头部 salt/m/t/p) + AES-256-GCM；
  头部字段与 ADR-005 §1 逐字对齐（format_version/app_version/exported_at/counts7 表/range/kdf/cipher）；
  载荷 meta + records（7 表 DTO）；**币种引用统一 cg_id**（跨设备键，导入经目录解析回行主键）；
  金额十进制原串/时间 UTC 保真；api_keys 凭证先解密入载荷（PRD 5.2-9）；内容范围严格按 ADR-005 §3
  （不含 accounts/全局设置/coins/exchange_coin_map；**行情 Key 不进备份——方案甲**）；原子写文件；
  `CproDecodeException` 三态（损坏/版本不支持/密码错或损坏——PRD 有意不区分）。
- **T9.2 增量合并/全量覆盖**：BackupMergePlanner 纯规则（uuid → 交易所+订单号去重[v1 模糊级无输入面
  §5-2]；api_keys (exchange+别名) 跳过；fee_rules/settings 备份优先覆盖；快照 币种+法币+小时桶 幂等
  **本地同桶保留**）+ BackupRestoreStore 应用器（**单写事务**：全量覆盖先清账户业务表[FK 序]再导入；
  api_keys 先插后包目标账户 DEK 重加密；全局公共表不动）；缺失币种行跳过并计数（目录刷新后可重导）。
- **T9.3 恢复流程 + CSV 导出**：previewBackup（明文头部摘要，无需密码）→ prepareRestore（验证+解密+
  合并规划预览，不落库）→ restoreBackup（应用 → **全量重放 LENIENT** 出「持仓异常」币种清单）；
  全量覆盖自动临时备份 `~/.wuzhufolio/backups/pre-restore-*.cpro`（同备份密码加密，路径随摘要返回，
  不记 backup.last_at）；CSV 明文导出三类（交易[与导入模板同列可回导]/资金流水[含校准行]/持仓汇总，
  **不含任何 API 密钥**，RFC 4180 转义）；backup.last_at / restore.last_at 账户级元数据。
- **T9.4 备份恢复 UI**：DataManagementSection（设置 → **数据管理**第四分组：备份区域[按钮+上次备份
  时间]/恢复区域[按钮+上次恢复时间]/明文导出[三个 CSV 按钮]）+ BackupExportModal（敏感提示内嵌 +
  密码双栏 + 强度/一致性本地校验）+ RestoreWizardModal（**单弹窗四步**：选文件→摘要+密码→合并预览+
  方式[全量覆盖二次确认勾选]→结果；多步聚焦按步骤显式请求——共性约束 7.3 全项）；向导「从备份恢复」
  占位改真实路径（落设置页数据管理分组，M6 API 卡先例）；接线 = AppBootstrap BackupServicesBundle +
  Runtime.backupService + Main 挂载（复用 M7 FilePicker）+ SettingsSectionsHost backupContent 参数。
- 测试：**427 执行 0 失败（431 报告，4 钥匙串真实后端跳过待 CI win/mac）**（domain 175 / data 177 /
  ui 79；M9 新增 29 项：domain 15[data+9 planner+6 codec] / data 8 / ui 6）+ detekt 0 + 编译警告 0 +
  无缓存 clean build 全绿
- GUI 冒烟实证：真实 v12 开发库 → **schema=12 无迁移**、bootstrap ok、窗口驻留（rc=124）、0 异常

**本次改了什么**：见模块记录 §1（四任务摘要 + §5 规格落档 6 条，重点：「导出默认填账户密码」不可
实现偏差、模糊级去重无输入面、快照幂等本地优先、多步向导聚焦、临时备份同密码、app_version 常量）。

**怎么验收（人工）**：
1. docs/dev/modules/M9.md §3 逐项打勾（重点走读 §5 规格落档 1–6 条）；
2. 复跑 clean build detekt（命令见 M9.md §4，应 427 执行 0 失败 + detekt 0 + 警告 0）；
3. 聚焦：`:domain:test --tests "…domain.backup.*"`（15）/ `:data:test --tests "…data.backup.*"`（8）/
   `:ui:test --tests "…ui.backup.*"`（6）；
4. GUI 走查（详见 M9.md §4 步骤 3：备份生成 → 增量合并回导幂等[黄金 10] → 跨账户恢复[凭证重加密
   生效] → 全量覆盖演练[临时备份+全局表保留] → CSV 三类导出无密钥 → 错误路径[错密码/非 .cpro]）。

**遗留问题（转后续模块，详见模块记录 §6）**：真实数据端到端演练归本人工门第 3 步（不便演练项
转 P5）；状态栏备份提醒（M12，元数据已就绪）；大载荷流式分块（P6 观察项）；CI 三平台复跑待推送；
目标机 KDF 复核（M13 前）。

**建议的下一步**（已执行：M9 已通过关闭）：解锁 **M10 设置与日志诊断** / **M11 桌面集成**（并行面
可先行）/ **M12 UI 整合收尾**（主壳聚合页 + 异常态全量走查 + a11y + i18n），待人工下达启动指令。

## P4 · M10 设置与日志诊断（✅ 已通过--2026-09-10 人工「M10通过」关闭）

> 启动记录：人工原话「执行P4-M10」（2026-09-10）。范围 = task-breakdown M10（T10.1
> settings/fee_rules · T10.2 日志脱敏与轮转 · T10.3 诊断报告 · T10.4 设置页 UI）。**模块记录：
> docs/dev/modules/M10.md**（实现摘要 / 文件清单 / 验收清单 / 规格落档 6 条 / §7 走查反馈修复轮 / 遗留）。
> **关闭记录（2026-09-10）**：人工原话「M10通过」——验收覆盖：完整设置页单页 8 分组 GUI 走查
> （通用组即时生效/主题重启保持/枚举开关即时生效登录页；频率下拉 + 间隔归位；日志查看导出脱敏 +
> 诊断报告；手续费完整 CRUD；API·数据管理回归；关于组）+ 修复轮三项闭环（阈值自由数值 0 = 不启用 /
> 交易所可搜索选择 / 代理含义答复）；**470 报告（466 执行 0 失败 + 4 跳过待 CI）** + detekt 0 +
> 无缓存 clean build 全绿 + GUI 冒烟 schema=12、轮转日志行落盘。**M10 关闭 ✅，M11/M12 解锁待启动指令**。

> 启动记录：人工原话「执行P4-M10」（2026-09-10）。范围 = task-breakdown M10（T10.1
> settings/fee_rules · T10.2 日志脱敏与轮转 · T10.3 诊断报告 · T10.4 设置页 UI）。**模块记录：
> docs/dev/modules/M10.md**（实现摘要 / 文件清单 / 验收清单 / 规格落档 6 条 / 遗留）。
> 本模块**无 schema 变更**（settings 全局行新增键值项，schema 维持 12）。M5/M6/M7/M9 遗留的
> 「设置页汇合」四项登记全部闭环（SettingsSectionsHost 移除）；M2 遗留「枚举开关设置 UI」闭环；
> M8 遗留「法币孪生设置化」以稳定币白名单接线落地。
> **产物要点**：完整设置页（单页 8 分组：通用/网络/行情与同步/日志与诊断/手续费/API 管理/数据管理/
> 关于）+ 主题/盈亏设置与顶栏双向同步并持久化 + 通用设置用例（法币/精度/枚举开关/稳定币白名单扩展
> 生效/小额阈值/代理开关）+ 费率完整 CRUD（编辑/改名键迁移）+ 日志轮转（1 万条或 90 天先到为准）
> + 查看与导出日志（逐行脱敏）+ 诊断报告（PRD §6 内容受限清单）。
> **托盘与后台分组随 M11 落地**（行内行为 = T11.1/T11.2 验收载体，避免无行为开关——模块记录 §5-1）。

**产物清单（代码 + 测试，全量绿）**：

- **T10.1**：domain/settings/GeneralSettings（契约 + 视图 + 白名单/精度/阈值常量）+
  DefaultGeneralSettingsService（settings 全局行：fiat〔既有键〕/display.precision/
  login.username.enum〔既有键〕/cash.coins/small.threshold/network.proxy.enabled）；**稳定币白名单
  生效接线**（TransactionEventBuilder + DefaultFundService 注入 provider——USD 锚定集合 + 可用现金，
  默认白名单固定、扩展项可移除；端到端测试 700→1100）；**费率完整 CRUD**（saveExchangeEdit：同键
  覆盖/改名 = 备份去重键 account+exchange 联动迁移/全局行与缺失行类型化拒绝）。
- **T10.2**：LogRotationPolicy 纯规则（1 万条或 90 天先到为准）+ LogRotator（启动裁剪/删档，摘要
  入日志）+ logback maxHistory 30→90 + SyncLogRepository.rotate（删过期→裁 newest N）+
  LogAccess/FileLogAccess（查看与**导出均逐行再脱敏**，导出 = 确认弹窗〔PRD 逐字提示〕→保存）。
- **T10.3**：DiagnosticsService + DefaultDiagnosticsService（应用/OS 版本、schema 版本、行情调用
  计数〔额度账本〕、同步调用计数〔sync_logs 累计 + 最近同步〕、最近日志片段〔再脱敏〕——结构上
  不含密钥与响应体）+ DiagnosticsReportText 预览/导出同源渲染 + UI 生成→预览→保存。
- **T10.4**：ui/settings 五文件（SettingsPage 单页滚动 8 分组 + SettingsCopy + 三个 VM）+
  WzSelect/WzSwitch/WzSegmented 组件（M5「分段→下拉」登记项闭环）+ MarketSettingsSection/
  ApiManagementSection/DataManagementSection 嵌入化 + SettingsSectionsHost 删除 + ShellViewModel
  setTheme/setPnlScheme 持久化钩子（PRD 6.1 双向同步闭环）+ AuthGate 枚举开关读取器（登录页进入
  重读，M2 遗留闭环）+ Main/AppBootstrap 组合根接线。
- 测试：**470 报告（466 执行 0 失败 + 4 钥匙串真实后端跳过待 CI win/mac；含修复轮 +4）**+ detekt 0 +
  编译警告 0 + 无缓存 clean build 全绿
- GUI 冒烟实证：真实 v12 开发库 → **schema=12 无迁移**、bootstrap ok（settings(count=9)）、
  **M10 轮转日志行落盘**、窗口驻留（rc=124）、0 渲染异常

> **走查反馈修复轮（2026-09-10，3 项反馈，见 M10.md §7，全部闭环）**：① 小额阈值重设计——预设档
> （默认 4000）废弃 → **自由数值输入（基础法币，≥0，0 = 不启用）**，按用户规模自定；② 费率交易所 →
> 可搜索选择（候选建议点击填入，自由输入保留）；③ 系统代理含义答复（开 = 自动使用系统代理、
> 关 = 直连；应用内代理地址/端口不在 PRD 范围，行为随 M11）。修复轮新增 4 测试，**470 报告
> （466 执行 0 失败 + 4 跳过待 CI）** + detekt 0 + build 全绿；data-model/api-contracts 已回写。

**本次改了什么**：见模块记录 §1（四任务摘要 + §5 规格落档 6 条，重点：托盘与后台分组随 M11、
代理开关先行/行为 M11、白名单默认固定+扩展可移除、同步调用计数口径、运行中实时轮转未做、
主题不走 GeneralSettingsService 走 ShellViewModel 单一通路）。

**怎么验收（人工）**：

1. docs/dev/modules/M10.md §3 逐项打勾（重点走读 §5 规格落档 1–6 条）；
2. 复跑 clean build detekt（命令见 M10.md §4，应 462 执行 0 失败 + 4 跳过 + detekt 0 + 警告 0）；
3. 聚焦：`:ui:test --tests "com.wuzhufolio.ui.settings.*"`（8）/
   `:data:test --tests "com.wuzhufolio.data.settings.*"`（22）/
   `:data:test --tests "com.wuzhufolio.data.logging.*"`（5）/
   `:domain:test --tests "com.wuzhufolio.domain.settings.*"` +
   `"com.wuzhufolio.domain.redaction.LogRotationPolicyTest"`（6）；
4. GUI 走查（M10.md §4 步骤 4 七项：8 分组滚动 / 通用组设置即时生效 + **主题切换重启保持** +
   枚举开关关后登录页即改纯输入 / 间隔行归位 + 频率下拉 / 日志查看导出脱敏 + 诊断报告 /
   费率编辑预填 / API·数据管理回归 / 关于组）。

**遗留问题（转后续模块，详见模块记录 §6）**：托盘与后台分组 + 代理行为接入（M11）；调度宿主挂
日志轮转运行中执行点（M11）；精度/阈值消费 + 设置页视觉精修 + 状态栏提示（M12）；同步调用计数
口径与备份范围确认（P6）；CI 三平台复跑待推送；目标机 KDF 复核（M13 前）。

**建议的下一步**（已执行：M10 已通过关闭）：解锁 **M11 桌面集成**（T11.1 托盘与通知 / T11.2 开机
自启 / T11.3 系统代理检测与指示 + 行情/同步调度循环宿主 + 托盘与后台设置分组）或 **M12 UI 整合收尾**
（主壳聚合页 + 异常态全量走查 + a11y + i18n + 精度/阈值消费），待人工下达启动指令。

## P4 · M11 桌面集成（✅ 已通过--2026-09-10 人工「m11 通过」关闭）

> **关闭记录（2026-09-10）**：人工原话「m11 通过」——M11 ✅ 已通过关闭。走查基准 = 模块记录 §4
> 步骤 4 六项；本机可验部分（代理检测与状态栏指示端到端实证、调度宿主启动与降频口径、托盘与后台
> 分组四行开关与写入、关窗决策与通知开关单测钉死）已全部通过；两项环境相关验证按人工拍板**延期留册**
> （§5-2 开机自启 → P7 打包版；§5-3 托盘 GUI → 真实桌面），到期检查点与 DoD 见模块记录。
> **M11 关闭 ✅，M12 解锁待人工启动指令。**
>
> 启动记录：人工原话「执行P4-M11」（2026-09-10）。范围 = task-breakdown M11（T11.1 托盘与通知 ·
> T11.2 开机自启 · T11.3 系统代理检测与指示）**＋ M5/M10 登记的三项遗留**：调度循环宿主
> （窗口可见性/托盘降频，M5 §5-5）、托盘与后台设置分组（M10 §5-1）、日志轮转运行中执行点（M10 §5-5）。
> **模块记录：docs/dev/modules/M11.md**（实现摘要 / 文件清单 / 验收清单 / §5 规格落档 6 条 /
> §6 遗留 / §7 勘误 2 条）。
> 本模块**无 schema 变更**（新增 4 个 settings 全局键值项，schema 维持 12）。

**产物清单（代码 + 测试，全量绿）**：

- **T11.3 系统代理**：domain/proxy/SystemProxy（ProxyEndpoint 仅 host:port 不含凭据 / ProxyMode /
  ProxyStatus 指示文案与悬停提示 / ProxyEnvironment 环境变量解析）+ data/proxy/SystemProxyDetector
  （**JdkSystemProxy.enable() 置 java.net.useSystemProxies=true** —— JDK 系统代理探测的真正前提，
  Main 首行 + 打包版 jvmArgs 双保险；检测 = JDK 系统代理优先、环境变量兜底；ToggleableProxySelector
  每请求读开关；ProxyRuntime 状态流）+ 两个 HTTP 客户端共用同一 selector（Ktor OkHttpConfig.config）
  + 状态栏文字指示/悬停提示 + 设置开关落盘后即时切换请求走向（无需重启）。
- **调度循环宿主**：domain/schedule/BackgroundSchedule（ScheduleJitter ±20% / BackupReminder 30 天 /
  LogRotationCadence 6 小时）+ data/schedule/BackgroundScheduler（三条循环：行情按 RefreshCadence
  频率并在托盘驻留时按同步间隔降频、额度 80% 降档；交易同步按 15/30/60；运行期日志轮转 6 小时
  —— M10 §5-5 遗留闭环）+ 唤醒通道（**窗口恢复可见立即刷新一次**，429 退避窗口内忽略）+
  窄接口 SchedulerSources（宿主不依赖两个用例服务的 22 个方法面）+ 事件流 SharedFlow +
  每 tick 异常隔离（长驻进程不因单次网络异常而死）。
- **T11.1 托盘与通知**：app/AppHost（托盘 + 窗口可见性 + 调度宿主 + 通知派发**共享同一可见性真源**
  `windowVisible && !minimized`）+ TrayIcon（程序化占位图标，见 §5-1）/ TraySupport /
  DesktopNoticeText（同步完成/失败/备份提醒文案与截断）/ DesktopBehavior（**关窗决策与通知开关抽为
  纯函数**——理由见 §5-3）+ 降级口径（托盘不可用 → 关窗即退出，绝不藏窗口）。
- **T11.2 开机自启**：domain/autostart（平台规则 + plist/.desktop/Run 键生成 + AutostartCommand）+
  data/autostart/PlatformAutostartService（三平台注册/注销/读回，全参数可注入；enable/disable
  永不抛异常）+ **接线只在打包版注册**（`jpackage.app-path` 存在时；开发态 supported=false 并如实置灰，
  见 §5-2）+ 启动自愈 reconcileAutostart。
- **托盘与后台设置分组（M10 §5-1 闭环）**：domain/settings/DesktopSettings +
  data/settings/DefaultDesktopSettingsService（四项键：tray.minimize_on_close 默认 on /
  autostart.enabled 默认 off / tray.sync_notification 默认 on / backup.reminder 默认 on；
  **自启先平台注册后落键**，失败不落键）+ DesktopPreferences 运行期镜像（EDT 判定不查库）+
  ui/settings/DesktopSettingsViewModel（**Mutex 排队串行**，不用 M10 的「busy 中 return」——那会静默
  吞掉连点的第二个开关，见 §5-4）+ SettingsPage 第 3 分组（网络之后，四行开关 + 不可用置灰与原因）。
- 测试：新增 10 个测试文件 **85 用例**（domain 23 / data 39 / app 16 / 既有 UI 类 +7）；
  **555 报告（551 执行 0 失败 0 错误 + 4 钥匙串真实后端跳过待 CI）** + detekt 0 + 编译警告 0 +
  **无缓存 clean build 全绿（32 任务全部真正执行）**。
- GUI 冒烟实证：真实 v12 开发库 → **schema=12 无迁移** + bootstrap ok + `proxy init enabled=true
  indicator=直连` + `scheduler started syncInterval=60min` + `tray support supported=false` +
  窗口驻留（rc=124）+ 0 渲染异常；**T11.3 端到端实证**：`https_proxy=… ./gradlew :app:run`
  → `indicator=代理：系统代理`。

**本次改了什么**：见模块记录 §1（三任务摘要 + 遗留闭环 + §5 规格落档 6 条，重点：①托盘图标为程序化
占位资产；②自启仅打包版可用（开发态如实置灰）；③本机 WSLg 无系统托盘，托盘 GUI 走查不可执行→决策
抽纯函数单测钉死；④托盘组 VM 改 Mutex 串行；⑤备份提醒基线退化口径；⑥通知范围只含同步与备份提醒）。

**怎么验收（人工）**：

1. docs/dev/modules/M11.md §3 逐项打勾（重点走读 §5 规格落档 1–6 条，尤其 §5-2/§5-3 两项**延期登记**）；
2. 复跑 `./gradlew clean build detekt --no-build-cache`（应 555 用例 = 551 执行 0 失败 + 4 跳过 +
   detekt 0 + 警告 0）；
3. GUI 走查（M11.md §4 步骤 4 六项）：托盘与后台分组四行 + 置灰原因 / 代理开关**即时**改变状态栏
   指示 + 悬停提示 / 关窗行为（托盘不可用应直接退出）/ 托盘可用环境的菜单与驻留 / 打包版自启端到端；
4. **本机限制**：WSLg 无系统托盘 → 步骤 3 的托盘项与打包版自启项**须在真实桌面/打包版验证**
   （§5-2/§5-3 已登记到期检查点与 DoD）。

**遗留问题（转后续模块，详见模块记录 §6）**：M12 —— 托盘正式图标资产与打包图标、状态栏数据源/
额度/429 频发文案、**toast 遮挡打磨**（连点被吞，M11 UI 测试实测复现）、通用组/网络组 VM 防重入
写法统一；P6 —— 诊断报告同步计数口径复核、备份范围不含 M11 新全局键确认；P7 —— 打包版自启端到端 +
三平台托盘实测 + nativeDistributions 图标。**CI 三平台复跑已闭环**（run 34501319152 全绿）。

**建议的下一步**（待人工走查）：M11 通过后解锁 **M12 UI 整合收尾**（主壳聚合页 + 异常态全量走查 +
a11y + i18n + 精度/阈值消费 + 状态栏数据源），或先安排**真实桌面环境的托盘/自启补充验证**。

## P4 · M12 UI 整合收尾（✅ 已通过--2026-09-12 人工「M12通过」关闭）

> **关闭记录（2026-09-12）**：人工原话「**M12通过**」——M12 ✅ 关闭，**M1–M12 全部通过**。
> 验收轨迹：T12.1–T12.4 逐条走查 + 人工 GUI 走查 **10 项反馈全闭环**（修复轮一：语言即时切换/精度挂钩与
> 适用范围/候选浮层/成本可靠性标注 + 4 项问数真实库复算；修复轮二：状态栏文案渲染期派生/候选浮层紧贴搜索框）
> + **D26 引擎返工**（人工拍板方案甲；真实库前后对比：USDT 均价 1.4874→1.0000、BTC 125,176.63→79,449.55、
> 累计已实现 −28,819.65→+32,196.75）+ 全量 **614 用例 0 失败** + detekt 0 + 警告 0 + 无缓存 clean build 全绿 +
> GUI 冒烟 schema=12 驻留（rc=124）。
> **CI 三平台全绿（run [34693442450](https://github.com/mapleafly/wuzhufolio/actions/runs/34693442450)，
> 2026-09-12 推送触发）：windows 4m02s / macos 2m30s / ubuntu 2m58s 全部 ✓**——M12 新增的 i18n/聚合页/
> 状态栏用例与 Compose UI 离屏测试在三平台均通过；M1–M12 全部代码已推送至 `origin/main`（`354a47a`）。
> **下一模块 = M13 发布准备，待人工启动指令。**

> 启动记录：人工原话「执行P4-M12」（2026-09-11）。范围 = task-breakdown M12（T12.1 主壳整合 +
> 聚合页〔仪表盘/资产列表/币种详情〕+ 行情页 UI 收尾 · T12.2 异常态全量走查 · T12.3 a11y 基线 ·
> T12.4 i18n）**＋ M11 §6 交接的四项打磨**。**模块记录：docs/dev/modules/M12.md**。
> 本模块**无 schema 变更**（schema 维持 12）。
> **走查修复轮（2026-09-11，人工 8 项反馈）**：4 项问数（累计已实现盈亏/ROI/USDT 均价/BTC 卖出已实现）
> 已用真实开发库逐事件复算作答（模块记录 **§7.2**）；4 项缺陷已修复 + 回归：
> ① **语言切换不即时生效**（主壳误用启动期常量驱动主题）② **精度档只持久化未驱动展示**（+ 适用范围表 §7.3）
> ③ 行情页搜索候选内联挤占列表（改就地浮层 + 可滚动 + 上限 20）④ 成本口径不可见（新增「成本不可靠」标注）。
> **走查修复轮二（同日，人工再 2 项）**：⑤ 状态栏/顶栏文案被预存进 StateFlow → 语言切换滞后（改渲染期派生）
> ⑥ 候选浮层跨滚动容器求锚点导致偏移（改为固定页头 + 列表区叠加，回归断言「间隙 ≤24dp 且不推动列表」）。
> **引擎级缺陷已获人工拍板（§7.4）**：**「拍板选甲」→ C2 返工已实施**（决策档 **D26**，
> task-breakdown **T4.5**）——负持仓只吃本金不建成本、成本随数量归零、穿越归零点按数量比例拆分；
> `ReplayEngine` + `SellRealizedTracer` 同步，黄金用例 1–9/12 与全部既有断言**零回归**，
> 新增 6 项验收（不变式/穿越点精确拆分/增资穿越/锚点归零/走查场景复现/轨迹交叉校验）。
>
> **人工门前置拍板（本次唯一分叉）**：T12.4 i18n 范围 —— 人工选 **「B 本次一并做全量 zh/en」**
> （50 个 UI 文件 / ~620 条文案全量抽取 + 回归全部 UI 测试）；该拍板派生的「界面语言」设置项
> 按 **C1** 落盘为 **D25**（决策档 + 台账 + ia.md / data-model / task-breakdown 回写；
> 原型补行显式延期，检查点 = P5 联调或 P7 前视觉终审）。

**产物清单（代码 + 测试，全量绿）**：

- **聚合页后端（新增只读服务）**：`domain/portfolio/PortfolioService` +
  `data/portfolio/DefaultPortfolioService` —— 事件装配 → 全量重放 → `PortfolioCalculator` → 逐币补目录/现价；
  **现价取数链与资金页同源**（快照 → USD 锚定 1:1 → 最近可得），修正「首刷前仪表盘净值 0 而资金页有现金」
  的两页口径分裂（2 项交叉断言守护）
- **T12.1 聚合页**：仪表盘（7 张总览卡 + 环形图〔小额阈值归并「其他」/点击高亮 + 浮窗〕+ 安全与隐私面板）、
  资产列表（7 列排序 + 无行情/持仓异常/估算中徽标 + 行点击进详情）、币种详情（汇总 4 卡 + 交易记录三重筛选
  + 单一来源校准入口 + 校准历史）、行情页收尾（命令区置顶 + 数据源徽章 + 自动轮询说明 + 搜索打开即聚焦）、
  主壳（顶栏数据源徽章与手动刷新行情 + 状态栏数据源/额度/断链）
- **T12.2 异常态**：新增 N1 网络断开 / N3 估算中 / B3 额度 80% / B4 共享限流 / B5 兜底标注 + 各页加载态空态；
  A1–A4、V1–V9、B1、B2 逐条复核一致
- **T12.3 a11y**：交互件统一「键盘可达 + 焦点可见 + 按钮语义 + 无障碍动作标签」；环形图 contentDescription；
  徽标文字载体 + 盈亏强制 +/-；读屏实测留 P6（ADR-001 既有风险）
- **T12.4 i18n（D25）**：`ui/i18n/` 10 个模块目录 + `I18n` + `WzFormat`（~3700 行）；
  7 个 `*Copy` 门面改动态取值（调用点零改动）；设置 → 通用 →「界面语言」（中文/English，键复用既有 `locale`）
  切换即时生效并持久化；精度档（8/2/2、2/2/2）与微小价格自适应、小额阈值、日期格式随语言/时区本地、
  金额前缀随基础法币；**守护测试三条**（双档非空 / en 档零 CJK / `ui/i18n` 外无内联中文文案）
- **M11 §6 四项闭环**：状态栏数据源·额度·429 文案（`ShellStatusViewModel`）/ 精度与阈值消费 /
  **toast 点击穿透**（弃 M3 `Surface`，其 `pointerInput` 会吞掉右下角点击）/ 通用组 VM 改 Mutex 串行
- **勘误（C0）**：`CoinResolutionException` 增类型化 `kind`、`FeeRulePolicy.validate` 改类型化 `Error`
  （不再让 UI 靠中文正文判分支 / en 档漏中文）；`MarketCopy.WATCH_EMPTY` 订正为文档/原型口径；
  detekt 配置补回测试目录排除
- 测试：**614 用例（610 执行 0 失败 + 4 跳过待 CI win/mac）** = domain 209（+5） / data 255（+18） /
  ui 134（+36） / app 16；detekt 0；**编译警告 0**；无缓存 clean build 全执行
- 走查修复轮（§7）：语言即时切换 / 精度挂钩与适用范围 / 行情候选浮层 / 成本不可靠标注（含 4 项回归测试）
- GUI 冒烟实证：真实 v12 开发库 → **schema=12 无迁移**、`settings(count=13)`、bootstrap ok、
  scheduler started、**窗口驻留 GRADLE_RC=124**、0 渲染异常

**怎么验收（人工）**：① `docs/dev/modules/M12.md` §3 逐项打勾（重点走读 §5 规格落档 6 条 + §1.7 勘误 3 条）；
② 复跑 `./gradlew clean build detekt --no-build-cache`（应 614 用例 = 610 执行 0 失败 + 4 跳过 + detekt 0 + 警告 0）；
   并复核 **§7 走查修复轮**四项（语言切换即时生效 / 精度在资产·交易·行情三处生效 / 候选浮层不挤占列表 /
   成本不可靠徽标与警示），以及 **§7.4 引擎缺陷提报的定级**（建议 C2，需人工拍板后才动引擎）；
③ GUI 走查（M12.md §4 步骤 3 九项：仪表盘数值与环形图 / 资产列表排序与徽标与行跳转 / 币种详情三重筛选与校准入口 /
行情页聚焦与上限 / **语言切换（全站文案 + 日期格式即时变英文，重启保持）** / 精度档 / 断网与额度异常态 /
toast 穿透 / 双主题）；④ 走查 `docs/dev/decisions/D25-界面语言设置与全量zh-en.md` 的分级与影响面扫描是否认可。

**遗留问题（转后续模块，详见模块记录 §6）**：P6 —— 读屏（NVDA/JAWS）实测、`TxFilter.coinSymbol` 子串匹配口径、
诊断同步计数与备份范围确认；P7 —— 托盘图标/打包图标/打包版自启/三平台托盘实测/字体子集化；
**D25 原型补「界面语言」行**（检查点 = P5 联调或 P7 前视觉终审）；英文文案润色抽查；CI 三平台复跑待推送；
目标机 KDF 复核（M13 前）。

**建议的下一步**（已执行：M12 已通过关闭）：按 task-breakdown 依赖图，P4 仅剩 **M13 发布准备**
（T13.1 安全自查：§1.1 硬约束逐条核验 → `security-checklist`；T13.2 签名公证打包：jpackage 三平台产物 +
...
## P4 · M13 发布准备（✅ 已通过--2026-09-12 人工「M13 通过」关闭）

> **关闭记录（2026-09-12）**：人工原话「**M13 通过**」——M13 ✅ 关闭，**M1–M13 全部通过，P4 分模块开发收尾完成**。
> 验收覆盖：`docs/test/security-checklist.md` 五条硬约束逐条核验（13 项发现 = 7 修复 + 6 登记）+
> `docs/dev/modules/M13.md §4` 六步与 **§5 规格落档 10 条**（含安装器元数据 ASCII、macOS bundle 版本解耦、
> WiX 下载口径、版本号口径、CI 触发口径）+ T13.2 三平台产物与签名公证流水线 +
> 四轮 CI 闭环 5 个环境级缺陷（`M13.md §8`）。
> **CI 全绿留痕**：build ×3 + package ×3 六 job（run [34698502287](https://github.com/mapleafly/wuzhufolio/actions/runs/34698502287)，
> 末次 [34699241892](https://github.com/mapleafly/wuzhufolio/actions/runs/34699241892)）——三平台原生产物
...
## P5 集成与联调（✅ 已通过--2026-09-13 人工「P5通过」关闭）

> **关闭记录（2026-09-13）**：人工原话「**P5通过，完成git本地提交，远程推送**」——P5 ✅ 关闭，
> **P0–P5 全部关闭**。验收轨迹：核心用户旅程实机走查（步骤 1/6/7/8 直接通过；2–5 报出问题）→
> **五轮修复闭环**（1 个 P0 数据完整性 + 2 个 P1 + 缺价假亏损 + 5 项人工裁决）→ 复验通过；
> **PRD V2.0 / 跨端共享规范 V1.1 回写随本门一并确认生效**（评审材料 `docs/prd/桌面端prd-V2修订说明.md`）。
> 终态：**664 用例（657 执行 0 失败 + 7 跳过）+ detekt 0 + 编译警告 0**；集成测试 10 项（app）+ 真实网络冒烟 2 项（data，env 门控）；
> 契约一致性核对 22 接口（发现并补 1 处文档回写缺口）；D25 原型补行与自选种子收敛同步完成。
> **P6 系统测试与质量 解锁为「进行中」，待人工启动指令。**

> 启动记录：人工原话「执行P5」（2026-09-13）。范围 = `AGENTS.md §4 P5`：把 P4 十三模块拼起来打通核心用户旅程
> （登录 → 增资 → 交易 → 看板/ROI → 备份恢复），产出 `docs/test/integration-report.md` + 集成测试；
> 输入 = P4 全部模块 + `docs/tech/api-contracts.md`；DoD = 主流程无阻断性缺陷 + 跨模块接口与契约一致。
>
> **人工验收轮（2026-09-13）**：人工按报告 §7 实机走查——步骤 1/6/7/8 通过；步骤 2/3/4/5 报出 4 类问题
> （含 **1 个 P0 级数据完整性缺陷**：跨账户恢复丢交易行）。根因与修复见报告 **§11**，回归见下「修复轮」。

**修复轮（2026-09-13，人工验收 4 类反馈 → 3 项已修复 + 1 项待裁决；均为 C0 实现偏差/实现补全）**：

1. **【P0】跨账户恢复丢交易行**（报告 §11-1）：`BackupMergePlanner.txOrderKey` 把订单号 null 的手动/CSV 行
   统一规约为 `"EXCHANGE|"` → 同交易所全部手动交易互相判重，增量合并与**全量覆盖**都只导入第一笔
   （人工实测 3 笔 → 1 笔）。**修复** = 订单号非空才成键（无订单号只按 uuid 判重）+ 既有键装配同步收紧；
   **备份格式与加密链未变**（旧 `.cpro` 用修复版重导即可完整恢复）。回归：域层 +4 / 集成用例扩为 3 笔
   （买入 + 卖出 + CSV，修复前必红 3→1）。
2. **【P1】全新安装首启目录为空 → 录入被阻断**（§11-2）：行情循环先等满一个频率间隔（默认 5 分钟）才首刷
   → 期间 `coins` 目录为空，增资/交易报「币种未收录」（误导文案）。**修复** = 行情循环**启动即刷新一次**
   （手动/额度/429 退避/托盘降频口径不变）。回归：调度 +1；真实网络 `LiveMarketRefreshWiringTest` 复现并验证。
3. **【P1】持仓币不被自动刷新**（§11-3）：定时刷新与设置页「立即刷新」不传币集 → 回退开箱 4 个现金币，
   用户买入的新币永远「无行情」（看板只显示现金，手动刷新才出现）。**修复** = 落地 M5 §6 登记的
   「持仓币集注入点」：`DefaultMarketRefreshService.defaultCoins` 注入 + 组合根晚绑定「持仓 ∪ 自选」
   （全空仍回退 4 币，行为不变）。回归：数据层 +2；真实网络实测 `coins=4 → coins=5`、`bitcoin:0.01@76764` 已定价。
4. **【口径·待裁决】稳定币按真实市价折算**（§11-4）：折算链的「USD 锚定 1:1」只是**缺价兜底**，有快照时按
   市价（USDT≈0.99965）→ 增资 100,000 显示可用现金 99,964.80、卖出已实现 999.65 而非 1,000。
   **本轮未改代码**，交人工裁决（报告 §8-1：A 维持市价 / **B 建议：白名单币种优先 1:1 锚定**，若选 B 属 C2）。
5. **【真机首启实证】** 隔离数据目录 `:app:run`：启动后 1.5s 触发 `market refresh started manual=false`、5s 后
   `market refresh finished source=COINGECKO coins=4 untracked=0 error=null` —— 目录在窗口出现后数秒内就绪（修复前需等 5 分钟）。
6. **【文档】§7 验收脚本写死数值**（§11-5）：101,000 / +1.00% 只在「集成测试固定价」下成立，真机按当前
   行情计算 → 已改为自洽校验口径（我的脚本缺陷，非产品缺陷）。
7. **【待人工补信息】CSV 导入 ETH 的 toast 原文**（§11-6）：人工判读「猜测是和库中没有 eth 记录，所以导入
   的时候的提示」——与首启目录为空（第 2 项）同源，已随该修复闭环；如需彻底区分文案分支，请补 toast 原文。
8. **【第二轮：人工裁决 3 项已实施】**
   ① **D27（C2）白名单稳定币 1:1 锚定**：决策档 `docs/dev/decisions/D27-稳定币1比1折算口径.md` + 台账 +
   决策索引 + 任务 **T4.6**；落点 = `TransactionEventBuilder.priceOf`（锚定优先，公开 `isAnchoredUsdStable`）
   + `DefaultPortfolioService`（现价/24h 同源锚定，快照仅用于 `priceAsOf` 展示）；**白名单 = 默认四项固定
   不可移除 + 设置可扩展项**。真实网络实测：`tether@0.999664` → **`tether@1`**（BTC 仍按市价 77767）；
   端到端复现人工三个数值（可用现金 100,000 / 已实现 1,000 / 净值 101,000 / ROI +1.00%）。
   ② **P5-2 行情异常 cause 保留（C1）**：`MarketApiException`/`ExchangeApiException` 增 `cause`；
   行情编排失败分支记 `cause=` 摘要（只进日志、经脱敏漏斗、不进用户文案）。
   ③ **启动时自动同步（C1）**：`AuthGate.onSessionActive`（登录/建户/免密恢复/切换账户各触发一次）→
   组合根执行一次「交易同步 + 行情刷新」，补齐 PRD 故事 4.2「应用启动时」口径。

**产物清单**：

- `docs/test/integration-report.md` —— **P5 主产物**（结论摘要 / 取证分层 / 旅程数值轨迹 / 契约一致性核对表 /
  问题清单与处置 / 实测证据 / P4 交接项衔接 / 人工验收步骤 / 待裁决项 / 复跑命令 / 需求回溯）
- `app/src/test/kotlin/com/wuzhufolio/app/integration/IntegrationHarness.kt` —— 集成基座：**真实组合根**
  （`AppBootstrap.run`：密钥链→SQLCipher 库→迁移→Koin→服务束）+ 临时数据目录隔离（每用例独立加密库）
- `app/src/test/.../integration/CoreJourneyIntegrationTest.kt`（3）—— 主流程（登录/记住我 → 增资 → 手动交易 +
  CSV 导入 → 看板 ROI → 备份 → 增量合并回导 → 备份元数据/CSV 导出）、多账户隔离与跨账户恢复、登出后重新登录
- `app/src/test/.../integration/ExchangeLoopbackIntegrationTest.kt`（1）—— 交易所回环链路：**真实 HTTP**
  （JDK HttpServer 桩 + 真 OkHttp/Ktor + 真 HMAC 签名，`BinanceAdapter(baseUrl=回环)`，不改生产代码）
  → 首次同步入账 → 增量幂等 → 看板 → 校准执行流（差额锚点 + 留痕）→ 跨账户恢复后凭证仍可驱动同步
- `app/src/test/.../integration/ErrorPathIntegrationTest.kt`（4）—— 跨模块异常态：A1（两种输入同型异常）、
  V5/V7/V9（真实库上类型化上浮 + 失败操作不留半成品）、BACKUP_INVALID 两态、校准三类阻止原因
  （MULTI_SOURCE / NO_RECORDS / NO_EXCHANGE_KEY）
- `data/src/test/kotlin/com/wuzhufolio/data/smoke/LiveNetworkSmokeTest.kt`（2，**env 门控默认跳过**）——
  真实网络冒烟：CoinGecko 匿名档报价 + 全量目录 → 快照落真实 SQLCipher 库；Binance `/api/v3/exchangeInfo` 公开端点
- `docs/tech/api-contracts.md` §3 —— **回写 M2/M3/M11/M12/M13 五条契约补录 + P5 一致性核对说明**
  （F-1 文档回写缺口处置）+ `MergePlan` 口径澄清
- `docs/design/prototype/wuzhufolio-light.html` + `docs/design/prototype-verify.js` —— **D25 到期项闭环**：
  通用组新增「界面语言」行（与设置页同位同序）、小额阈值行对齐 M10 自由数值输入；新增 3 条 Playwright 断言
- `docs/dev/decisions/D25-界面语言设置与全量zh-en.md` §4/§5 + `docs/dev/增量台账.md` D25 行 —— 延期项转「已完成」
- 代码接缝（C0）：`AppBootstrap.SessionRuntime.sessions` 由 private 改公开（组合根之外装配同类服务需同一会话
  持有器；写入路径仍只经 `authService`，`close()` 统一擦除）

**本次改了什么**：

1. **建集成基座**：以真实组合根驱动测试——不再 new 服务、不再换替身；隔离靠 `-Dwuzhufolio.dataDir`（既有开发/测试
   入口）+ 临时目录，每用例独立加密库、结束释放会话/密钥/数据库并停 Koin。
2. **核心旅程端到端打通**（数值轨迹见报告 §2.1）：净值 101,000 / 总收益 +1,000 / ROI +1.00% / 可用现金 54,500；
   备份往返 **0 重复写入 + 恢复后增量保留 + 重放口径无漂移**（黄金用例 10 在真实 `.cpro` 上复现）。
3. **交易所链路真实 HTTP 化**（报告 §2.2）：本地回环桩 + 真实适配器/同步/校准服务 → 同步入账、增量幂等、
   校准锚点、**跨账户恢复后凭证按目标账户 DEK 重加密仍可调用交易所**（凭证重加密的可解密性首次被端到端证明）。
4. **异常态服务层核验**（报告 §2.3）：A1/V5/V7/V9/BACKUP_INVALID/校准三类阻止原因全部类型化上浮。
5. **契约一致性核对**（报告 §3，DoD ②）：22 个 domain 接口逐签名比对，发现 **F-1**——M2/M3/M11/M12 的契约
   补录只在模块记录声明、`api-contracts.md` 正文未回写（**文档回写缺口，非契约行为不一致**）→ 已回写五条补录。
6. **D25 到期检查点执行**（P5 联调窗口）：原型补「界面语言」行 + verify 断言守护；台账/决策档同步闭环。
7. **真实网络冒烟**（`WZF_LIVE_SMOKE=1` 实测）：CG 报价 `bitcoin=76569 USD`/`tether=0.999753 USD`、目录 21,162 条、
   快照落库；Binance 公开端点 3,698 个交易对（全程**不使用任何 Key**）。该用例 env 门控默认跳过（外部 API
   不进默认回归——首跑即遇到一次瞬时失败，见报告 P5-2）。

**怎么验收（人工）**：

1. 读 `docs/test/integration-report.md`：§0 结论 → §3 契约核对表 → §4 问题清单（3 项已处置 + 2 项登记）→ §8 待裁决。
2. **核心用户旅程实测**（报告 §7 八步；建议隔离数据目录）：
   `WUZHUFOLIO_DATA_DIR=/tmp/wzf-p5-manual JAVA_TOOL_OPTIONS="-Dskiko.renderApi=SOFTWARE_FAST" ./gradlew :app:run`
   —— 建账户/记住我 → 增资 100,000 USDT → 买 1 BTC@50,000、卖 0.1 BTC@60,000、CSV 导入 0.5 ETH@3,000
   → 仪表盘净值 101,000 / ROI +1.00% → 备份 → 恢复（增量幂等）→ 跨账户恢复 → 异常态抽样 → 语言/双主题。
3. 复跑：`./gradlew clean build detekt --no-build-cache`（应 **664 用例 = 657 执行 0 失败 + 7 跳过** + detekt 0 + 警告 0）。
4. 集成测试聚焦：`./gradlew :app:test --tests "com.wuzhufolio.app.integration.*"`（10 项；其中真实网络首启链路 1 项需 `WZF_LIVE_SMOKE=1`，默认跳过）。
5. （可选，联网）真实端点冒烟：`WZF_LIVE_SMOKE=1 ./gradlew :data:test --tests "com.wuzhufolio.data.smoke.LiveNetworkSmokeTest" --rerun-tasks -i`。
6. （可选）原型视觉基准：`node docs/design/prototype-verify.js`（errors=[]；D25 语言行断言）。
7. §8 两项裁决 + 是否有真实只读 Key 供线上同步冒烟（报告 §8-3）。

**已验证（Agent 实证，供人工复核）**：

- 全量无缓存构建 `./gradlew clean build detekt --no-build-cache`：**664 用例（657 执行 0 失败 0 错误 + 7 跳过）**
  = domain 221 / data 276 / ui 136 / app 31；detekt 0；**编译警告 0**；跳过 7 = 4 钥匙串真实后端（CI win/mac 实证）
  + 3 live smoke（env 门控：data 2 + app 首启链路 1）。
- 集成测试 9 项全绿（真实组合根 / 回环 HTTP / 异常态）。
- 真实网络冒烟实测输出见报告 §5.2；GUI 冒烟（隔离数据目录）见 §5.3：`bootstrap ok | schema=12`、
  `scheduler started`、窗口驻留 GRADLE_RC=124、权限 700/600、日志 `market_api_key=****`、0 渲染异常。
- 原型 verify：83 项取值、`errors = []`（含 D25 新增 3 条断言）。

**遗留问题（转 P6/P7，详见报告 §6）**：

- **待人工裁决 2 项**（报告 §8-1/§8-2）：负持仓有价币的净值口径（C2 待定级，Agent 未动代码）；
  行情网络异常 cause 丢失（建议 C1：保留 cause 链供日志/诊断定位）。
- **转 P6**：P5-4（备份导出遇到不可解密 `api_keys` 密文时的失败模式）、P5-2（同上）、
  M13 既有登记项（`.cpro` 大载荷内存曲线、出站抓包实证、读屏实测、settings 键命名空间、登出后调度 tick 噪声）。
...
`security-checklist.md`（复跑版）/ `defects.md` / `test-report.md`，**待人工下达启动指令**。

**历史建议（P5 期间）**：人工按报告 §7 验收核心用户旅程 → 通过后 **P6 系统测试与质量**解锁
...
## P6 系统测试与质量（✅ 已通过--2026-09-21 人工拍板关闭）

> **关闭记录（2026-09-21）**：人工拍板「**先关闭 P6 门并把两项登记为 P7 携带**」——关闭依据 = **DoD 三项全部达成**
> （**P0/P1 = 0**（DEF-42/DEF-43 于第十一轮人工复验闭环）+ **P2 明确处理结论** + **`security-checklist.md` 五条硬约束全部通过**）
> + **人工门用例判定通过 9/11〔关闭时口径；同日第十一轮续后已为 10/11，见下行〕**（TC-MAN-02 / 03 / 04 / 05 / 06 / 07 / 08 / 10 / 11）；
> **剩余 TC-MAN-01（托盘走查正式判定）与 TC-MAN-09（出站抓包 + 权限实证）显式延期至 P7**（到期检查点 = P7 发布前）。
...
> **输入**：P5 集成版 + PRD V2.0（有效需求 = PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}）+
...
- `docs/test/test-cases.md` —— **300 条测试用例**（PRD §5 故事 109 + §7.2 功能 60 + interaction 异常态 21 +
...
- `docs/test/keyboard-walkthrough.md` —— **纯键盘操作指南与走查脚本**：键位语义对照表（Tab/Enter/空格/Esc/方向键适用范围）、
  实测焦点顺序（侧边栏 7 → 顶栏 3 → 页面内容）、7 步逐键脚本（登录/焦点环自检/资金页增资/交易页买入/仪表盘刷新/设置备份/Esc 回焦点）、
  判定表（用法问题 vs 缺陷 vs 设计如此）、失败取证方式、改进提案 A/B、最新构建获取方式
...
1. **全量验证**：按 PRD V2.0 逐条建立 300 条用例并执行；全量无缓存构建 **677 用例（669 执行 0 失败 + 8 跳过）**
...
**人工门进展 · Windows 11 首轮走查（2026-09-14 人工执行，Agent 已按反馈修复）**：
...
| 2 | 开机自启**未测** | ✅ **已闭环（2026-09-21 第十一轮）**：打包安装版实测通过 → **TC-MAN-02 ✅**（其间受打包版启动失败 DEF-42/DEF-43 阻断，两缺陷均已修复）；步骤见 `manual-test-guide.md §6 TC-MAN-02`，留痕见 **§19** |
...
| 7 | **三轮复验**：切 English 后托盘不即时变、**重启才变**；切回中文同样要重启 | **P2 · DEF-19**（根因：`Runtime.uiState` 是**启动快照**，主壳之外无观察来源）→ **已修复**：新增 `UiPreferenceState`（StateFlow）+ `Main.onShellPreferenceChange` 同步更新 + `AppHost` 订阅；菜单即时跟随语言/主题；回归 3 例 |
...
| 11 | 该两项的**变更分级** | **✅ 已拍板（2026-09-15 原话「按建议变更分级」）：DEF-20 = C0、DEF-21 = C1**。C0 侧 = `M7.md`/`M8.md` §勘误 + `interaction.md §3-9` 回写；C1 侧 = 决策档 **`D31-键盘焦点流.md`** + 台账 D31 行 + 决策索引 + `task-breakdown **T12.6**` + `ia.md §1.1` + `M12.md §1.7`（验收标准 A1–A6 见 D31 §6） |

**人工门进展 · Windows 11 第五轮走查（2026-09-15 · 真实币安只读 Key 冒烟，Agent 已按反馈修复）**：

| # | 人工观察 | 核实与处置 |
|---|----------|-----------|
| 12 | 设置 → API 管理「添加 API」弹窗像**插进页面**（挤占后面空间） | **P1 · DEF-22**（根因：弹层落在设置页 `verticalScroll` 容器内部 → 高度约束无限 → `WzModal.fillMaxSize()` 退化为内容高度 = 流内块）→ **已修复**：新增**页面级叠加槽** `ui/components/PageOverlay.kt`（`PageOverlayHost` + `PageOverlay`），设置页根承载弹层；回归 `PageOverlayUiTest` 2 例 + `SettingsPageUiTest` 弹层宿主 1 例 |
| 13 | 设置 → 行情与同步「已配置/未配置」弹窗**撑开页面** | **P1 · DEF-22**（同根因）→ **已修复**（`MarketSettingsSection` 改走 `PageOverlay`） |
| 14 | 设置 → 数据管理「恢复数据」弹窗**浮在备份区域、错位** | **P1 · DEF-23**（同根因：卡片在自己内容块内居中，位置随分组漂移）→ **已修复**（`DataManagementSection` 备份/恢复两弹层改走 `PageOverlay`） |
| 15 | 「添加 API」保存后**弹窗不关但内容已保存**；编辑保存会关 | **P1 · DEF-25**（根因：`addAndSync` 先落库后同步，同步失败以异常上抛 → UI 呈现「保存失败且弹窗不关」；重填再保存撞「别名已存在」）→ **已修复**：数据层落库后不再抛（返回 `status=FAILED` 结果）、VM 保存成功后一律关弹窗 + 刷新、文案改「密钥已保存；首次同步未成功（原因）…可重试」+ 忙碌态「保存中…」与提示行；回归 data 1 例 + UI 2 例 |
| 16 | 设置分组标题（通用/网络/托盘与后台…）**比二级标签还小**；手续费卡片加粗、数据管理卡片标题**明显更大** | **P2 · DEF-24**（根因：三处组件各自取值 `caption` 11sp / `pageTitle` 20sp / `bodyStrong` 14sp）→ **已修复**：新增排版令牌 **`sectionTitle`（15/600）** 作分组一级标题唯一取值；卡内二级标题统一 14/600；标准写入 `design-tokens.md §3`；回归 `SettingsPageUiTest::all settings first level titles share one typography level`（9 个一级标题高度一致 + 5 个二级一致 + 一级 > 二级 > 行标签） |
| 17 | 「同步交易数据是否**覆盖**手动填写的交易？」 | **DEF-26 核实为非缺陷**：同步只**追加**交易所成交（`insertIfAbsent` 仅 INSERT；去重键含 `exchange_order_id`，手写行为 NULL 不参与）→ 新增回归 `sync appends exchange trades without touching manually entered rows`（同 pair/同时间/同价量的手写行逐字段不变 + 交易所行照常新增 + 幂等）；**口径提示**（先手写后同步 = 各存一行 → 重复计入）已写入 `manual-test-guide.md §12` |
| 18 | 上述四项的**变更分级** | **✅ 已拍板（2026-09-15 原话「拍板，按建议变更分级」）：DEF-22/23/25 = C0、DEF-24 = C1**。C0 侧 = `M6/M9/M10` §勘误 + `api-contracts.md §3` + `design-tokens.md §4.2` + `AGENTS.md §7.3-5/6`；C1 侧 = 决策档 **`D32-设置页层级标准.md`** + 台账 D32 行 + 决策索引 + `task-breakdown **T10.5**` + `design-tokens.md §3`（验收 B1–B5 见 D32 §6） |
| — | **规范沉淀** | `AGENTS.md §7.3` 新增第 **5** 条（弹层必须挂页面根、覆盖整页）与第 **6** 条（保存类操作的状态与视觉必须一致）；`design-tokens.md §3` 增「设置页层级统一」、§4.2 增 Modal 承载位置约束；`interaction.md §3-12` 增保存失败语义 |

**人工门进展 · Windows 11 第六轮走查（2026-09-15 · 真实桌面 GUI 全流程 × 三档分辨率）**：

| # | 人工观察 | 核实与处置 |
|---|----------|-----------|
| 19 | **TC-MAN-06 纯键盘全流程 / TC-MAN-07 真实只读 Key 冒烟 / TC-MAN-10 外链与关于页** | ✅ **人工判定通过**（记录见 `manual-test-guide.md §13`） |
| 20 | 点设置进入设置页时**焦点定位在页面中部的「手续费 → 买入费率」**，页面滚到中部（希望初始在页面开始处） | **P1 · DEF-27**（根因：DEF-21 的「进页面」用容器 `FocusRequester` + Compose 子树遍历实现，该遍历**不按 Tab 序**——设置页 Tab 首项是 `fiat-select`，遍历却落到第 22 项 `fee-global-buy`）→ **已修复**：新增页面入口焦点契约 `PageEntryFocusState` + `Modifier.pageEntryFocus()`，页面在首个可聚焦控件上声明入口焦点，主壳切页优先请求它（6 个页面已声明；未声明页面回落旧路径）；回归走真实主壳路径 |
| 21 | 1280×800 下资产列表币种列**标签竖排**、8 位小数互挤；交易管理**手续费换行**、交易对+估算中占两行 → 行高参差 | **P2 · DEF-28**（根因：两表列宽用 `weight` 分配，窄窗压到内容宽度以下 → 文本换行/竖排）→ **已修复**：新增 `AdaptiveTable`（列最小宽 + 窄窗整表横向滚动、不换行不截断）；`Badge` 单行；「估算中」与交易对内联 |
| 22 | 1024×768 下**删除按钮显示不全、文字竖排**；资金管理「近90天」按钮**竖排** | **P2 · DEF-29**（根因：操作列/过滤按钮行被压窄）→ **已修复**：操作列为自适应表一等列（最小 120dp）；过滤行改 `FlowRow` 自动换行；`WzButton` 文案统一单行不换行 |
| 23 | 行情页搜索框输入后候选浮层**只有点「添加」才消失**；删光字母也不收起 | **P2 · DEF-30**（根因：`onQueryChange` 未取消在途搜索 → 旧结果回来又把候选写回，浮层被顶回；且无 Esc/失焦收起）→ **已修复**：取消在途搜索 + 结果落地复核关键词 + Esc 取消搜索 + 失败提示不再静默 |
| 24 | 上述四项的**变更分级** | **✅ 已拍板（2026-09-15 原话「按建议」）：DEF-27/28/29/30 全部 C0**。回写 = `M5/M7/M12` §勘误 + `design-tokens.md §4.3-1` + `interaction.md §2.7/§3-9/§3-13` + `ia.md §1.1`；均不建档、不进台账（`defects.md §0.1 ⑧`） |

**人工门进展 · Windows 11 第七轮走查（2026-09-15 · 真实桌面 GUI 全流程 × 三档分辨率，含 2560×1600 高 DPI）**：

| # | 人工观察 | 核实与处置 |
|---|----------|-----------|
| 25 | **TC-MAN-03 读屏（NVDA）/ TC-MAN-04 目标机性能** | ✅ **人工判定通过**（目标机性能以受限环境模拟 + 真机复验结论为准，见 `manual-test-guide.md §14`） |
| 26 | 2560×1600 下资产列表币种列**第三枚标签不能完全显示** | **DEF-31/37**（根因：宽窗模式按固定 `weight` 分配，**不保证每列 ≥ 最小宽**；2560×1600 在 200% 缩放下有效宽度 ≈1280dp）→ **已修复**：列宽改**按最小宽比例**分配 + 币种列最小宽 260dp |
| 27 | 2560×1600 / 1024×768 下交易表仍需横向滚动、手续费换行、滚到最右**删除按钮显示不全** | **DEF-33**（根因：① 纵向滚动条压住最后一列；② 手续费等单元格漏加单行约束）→ **已修复**：列表右侧预留 12dp 滚动条槽 + 全部单元格改 `SingleLineText` |
| 28 | 添加交易 / 记录增资弹窗**总有「不到一行」的滚动条** | **DEF-36**（根因：内容高度上限写死 470/420dp，与窗口无关）→ **已修复**：`modalContentMaxHeight()` = 窗口高 × 0.66（下限 320dp）+ 表单内边距紧凑化 |
| 29 | 1024×768 下仪表盘净值/本金卡片**数字换行变形**；环形图图例**币种名换行** | **DEF-32/34** → **已修复**：统一卡片组件 `WzCard` + 指标数字 `WzMetric`（单行 + 自动缩字号）；图例改 `SingleLineText` |
| 30 | 1024×768 下资产表长字段**截断后悬停看不到完整数据** | **DEF-35** → **已修复**：`SingleLineText` 只在确实截断时挂 `TooltipArea`，悬停显示全值 |
| 31 | 「各页列表能否加单元线？」 | **DEF-38** → **已实现**：`AdaptiveTable(divider = true)` + `rowDivider()`（1px `line` 色） |
| 32 | **人工追问：能否有总体方案（合用表格/卡片组件、统一应对分辨率）而非小修小补？** | **已产出总体方案 `docs/design/responsive-components.md`**（断点 → 组件 → 页面三层）：`WzWindowClass` 断点（COMPACT/MEDIUM/WIDE，按**内容区有效宽度**判定，覆盖高 DPI 缩放）+ 统一表格 `AdaptiveTable`（列最小宽比例分配 / 单行 / 徽标内联 / 单元线 / 操作列一等公民 / 滚动条槽）+ 统一卡片 `WzCard`/`WzMetric` + 弹窗尺寸策略 + 列表单元线；三张表与全部卡片已迁移 |
| 33 | 上述八条的**变更分级** | **✅ 已拍板（2026-09-15 原话「按建议」）：DEF-31/32/33/34/36/37 = C0、DEF-35/38 与「统一组件 + 响应式规范」= C1**。C0 侧 = `M5/M7/M12` §勘误 + `design-tokens.md §4.3-1` + `interaction.md §3-13`；C1 侧 = 决策档 **`D33-响应式与组件统一.md`** + 总体方案 `responsive-components.md` + 台账 D33 行 + 决策索引 + `task-breakdown **T12.7**`（验收 C1-1…C1-8 见 D33 §6） |

**人工门进展 · Windows 11 第八轮走查（2026-09-15 · GUI 全流程复验）**：

| # | 人工观察 | 核实与处置 |
|---|----------|-----------|
| 34 | **TC-MAN-05 断网态** | ✅ **人工判定通过** |
| 35 | 仪表盘第一行四个卡片的数字**比下面卡片明显偏大**（问「为何不一致」） | **P2 · DEF-39**（根因：`StatCard` 两级误映射为 `display` 32sp vs `bodyStrong` 14sp（相差 2.3×）；原型基准是 `.card .big` 27px / `.big.sm` 21px ≈1.29×）→ **已修复**：新增排版令牌 `metricPrimary`(27sp)/`metricSecondary`(21sp)，逐项对齐原型；回归断言两级比值 ≤1.6 |
| 36 | 表格只有横线，**要加竖线** | **P2 · DEF-40** → **已实现**：`AdaptiveTable` 新增 `verticalDivider`（默认开）在**列边界**画 1px `line` 色纵线（贯穿表头与数据行；最后一列右边界不画），列边界由纯函数 `columnBoundariesOf` 计算并单测守护 |
| 37 | 币种资产详情页的成交表**没有单元格线** | **P2 · DEF-41**（根因：该表是另一套手写 `Row + weight` 实现，未接入统一表格组件）→ **已修复**：迁移到 `AdaptiveTable`，自动获得横线 + 纵线 + 单行 + 截断悬停全值 + 窄窗横向滚动 |
| 38 | 上述三项的**变更分级** | **✅ 已拍板（2026-09-15 原话「按建议」）：DEF-39/40/41 全部 C0**。回写 = `M12.md` §1.7 + `M7.md` §5.z + `design-tokens.md §3/§4.3-1` + `responsive-components.md §2/§3`（`defects.md §0.1 ⑩`） |

**本轮修复验证**：`./gradlew clean build detekt --no-build-cache` → **718 用例（710 执行 0 失败 + 8 跳过）** + detekt 0 + 警告 0
（较上轮 +8：`PageOverlayUiTest` 2 + 设置页层级/弹层 2 + API 保存语义 2 + 同步契约/手写行保护 2）；外壳焦点顺序实测 = `侧边栏 7 → 顶栏 3 → 页面内容 → 循环`（**无空焦点步进**），
...
3. 复跑全量：`./gradlew clean build detekt --no-build-cache`（期望 **718 用例 = 710 执行 0 失败 + 8 跳过** + detekt 0 + 警告 0）；
...
7. **Windows 首轮走查修复复验（2026-09-14，见 `manual-test-guide.md §10`）**：
   ① 托盘菜单三行为可读中文、切 English 后为英文、三项动作生效（DEF-15）；
...
- **P8 登记**：**保存与首次同步解耦**（DEF-25 残留体验项：落库即关窗 + 后台同步 + 完成 toast，需新增「仅落库」服务入口）、
   **手写/同步疑似重复提示**（DEF-26 口径提示的自动化增强）、UI 展示层 🟡 项的视觉走查深化、DB 损坏全屏框演练、ROI=0 提示展示、行情行内 loading、
...
10. **DEF-43 定位与修复（2026-09-16）**：控制台调试包（`console-debug-windows`）给出真实错误 —— 人工机开启了
    **Java Access Bridge**（`%USERPROFILE%\.accessibility.properties`），而**打包运行时缺 `jdk.accessibility` 模块**
    → AWT 初始化抛 `AWTError: Assistive Technology not found: com.sun.accessibility.AccessBridge`，应用在业务日志前终止，
...
> ✅ **本节后续已执行完毕（2026-09-16 拍板 C0 + 2026-09-21 第十一轮复验）**：①② 已于 2026-09-16 拍板（DEF-43 = C0、
> DEF-42 上游限制登记 P8）；③ 已于 **2026-09-21 复验通过**（TC-MAN-02 ✅ / TC-MAN-11 ✅）→ **P1 归零、P6 DoD 达成**；
> 现仅剩 TC-MAN-01（托盘正式判定）与 TC-MAN-09（出站抓包+权限实证）及发布标准拍板。详见本文件「当前阶段/当前阻塞点」。
...
26. **负持仓区间成本口径（D26，2026-09-11，人工「拍板选甲」·C2 引擎返工）**：M12 走查实测发现引擎在负持仓区间仍累积成本，持仓回正后成本被摊到更小数量上 → 平均成本与已实现盈亏失真（实测 USDT 均价 1.4874、BTC 均价 125,176.63、单笔卖出已实现 −57,843.47，而该笔真实为获利）；命中 AGENTS.md §8.1 红线 3 → C2 mini 闭环。**方案甲**：负持仓只吃本金、不建成本；不变式「数量 ≤ 0 ⟹ 成本 = 0」；流入穿越归零点时仅「抬到 0 以上」的部分按数量比例承担成本（`v×(q−d)/q`）；数量归零时成本同步归零。决策档 `docs/dev/decisions/D26-负持仓区间成本口径.md`；技术回写 architecture §3.2 / data-model §3 / api-contracts §3 + task-breakdown **T4.5**；`ReplayEngine` + `SellRealizedTracer` 同步返工，黄金用例与既有断言零回归；**有效需求版本串 = PRD V1.9 + Δ{D21, D24, D25, D26}**。
...
28. **键盘焦点流（D31，2026-09-15 人工门第四轮裁决「按建议变更分级」· C1）**：人工第四轮走查提出两项
    （① 候选选中后焦点留在表单；② 回车进页面内容 + 方向键/Esc 回外壳循环）。分级 = **DEF-20 按 C0**
    （实现偏差纠正：`AGENTS.md §7.3` 键盘可用性强制项 → 模块记录 §勘误 `M7.md`/`M8.md` + 交互文档回写，
    不建档不进台账）+ **DEF-21 按 C1**。D31 落地内容：① 回车选中侧边栏项（或对当前项再次回车）→ 焦点**进页面内容**
    （页面槽 `focusRequester` 挂非可聚焦容器 = 焦点落到子树内第一个可聚焦控件；容器不加 `focusable()`，避免 DEF-13 式
    隐形 Tab 停靠点）；② 页面内**未被控件消费**的 Esc/↑/↓ → 焦点回侧边栏当前项（冒泡阶段；弹层打开时不接管
    〔`WzOverlayRegistry` 计数〕；Ctrl/Alt/Meta 组合键不接管，留给 P8 全局快捷键）；③ 侧边栏内 ↑/↓ 在导航项间移动。
    **C1 最小落盘清单已齐全**：决策档 `docs/dev/decisions/D31-键盘焦点流.md`（含影响面扫描 + 验收标准 A1–A6）+
    `增量台账.md` D31 行（有效需求串 = **PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}**）+
    `决策索引.md` + `task-breakdown **T12.6**` + `ia.md §1.1` + `interaction.md §3-9` + `M12.md §1.7`。
    **不触 §8.1 五条红线**（无数据/schema/加密/备份格式/接口变更；原型不适用已在台账勾选）。

29. **设置页层级标准（D32，2026-09-15 人工门第五轮裁决「按建议变更分级」· C1）**：人工第五轮走查指出设置页层级不统一
    （分组标题 11sp 比二级标签 14sp 还小；数据管理卡片标题 20sp 比手续费卡片 14sp 大 6sp，同级却不同字号）。
    分级 = **DEF-24 按 C1**（新增设计规范条目 + 跨模块样式统一）+ **DEF-22/23、DEF-25 按 C0**（实现偏差纠正/失败模式补全）。
    D32 落地：设置页（含内嵌分组组件）**只允许四种标题取值**——页面标题 20/600 → **分组一级 15/600（新增排版令牌
    `sectionTitle`）** → **卡内二级 14/600（`bodyStrong`）** → 行标签 14/400（`body`）；说明 11/400（`caption`）。
    **C1 最小落盘清单已齐全**：决策档 `docs/dev/decisions/D32-设置页层级标准.md`（含影响面扫描 + 验收 B1–B5）+
    `增量台账.md` D32 行（有效需求串 = **PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}**）+
    `决策索引.md` + `task-breakdown **T10.5**` + `design-tokens.md §3`；C0 侧 = `M6/M9/M10` 模块勘误 +
    `api-contracts.md §3`（`addAndSync` 失败语义）+ `design-tokens.md §4.2`（弹层承载）+ `AGENTS.md §7.3-5/6`。
    **不触 §8.1 五条红线**（无数据/schema/加密/备份格式/接口签名变更；原型不适用已勾选）。

30. **第六轮四项按 C0 勘误收口（2026-09-15 人工门第六轮裁决「按建议」）**：**DEF-27 / DEF-28 / DEF-29 / DEF-30 全部 C0**
    （实现偏差纠正，均不建档、不进台账）。**DEF-27**（切页入口焦点契约 `PageEntryFocusState` + `Modifier.pageEntryFocus()`）——
    DEF-21 的落地方式未达「焦点顺序合理」；**DEF-28/29**（窄窗表格/按钮不再换行竖排：`AdaptiveTable` + 过滤行 `FlowRow` +
    按钮单行文案）—— PRD §6 一致性/核心操作可达的实现要求，人工确认「1024×768 下表格横向滚动」为修复手段而非口径变化；
    **DEF-30**（行情候选浮层随清空/Esc 收起）—— `interaction.md §2.7` 既有语义未正确落地。
    回写：模块勘误 `M5.md`/`M7.md`/`M12.md`；设计/技术 `design-tokens.md §4.3-1`（窄窗适配口径）+ `interaction.md §2.7/§3-9/§3-13` +
    `ia.md §1.1`；裁决记录见 `defects.md §0.1 ⑧`。

31. **响应式与组件体系统一（D33，2026-09-15 人工门第七轮裁决「按建议」· C1）**：人工第七轮明确要求
    「总体方案而非小修小补」→ 确立**断点 → 组件 → 页面**三层并落地。分级 = **DEF-31/32/33/34/36/37 按 C0**
    （实现偏差纠正）+ **DEF-35/38 与「统一组件 + 响应式规范」按 C1**。
    D33 落地：① 断点唯一来源 `ui/theme/Breakpoints.kt`（`WzWindowClass` COMPACT <1200dp ≤ MEDIUM <1760dp ≤ WIDE，
    按**内容区有效宽度**判定，覆盖 Windows 125/150/200% 缩放；主壳 `ProvideWzWindowClass` 下发，页面不得自行判断）；
    ② 统一表格 `AdaptiveTable`（列宽**按最小宽比例**分配 → 每列 ≥ 自身最小宽；单元格单行 + 截断悬停全值；徽标单行内联；
    单元线；右侧 12dp 滚动条槽；操作列一等公民）；③ 统一卡片 `WzCard`/`WzCardLabel`/`WzMetric`（指标数字单行 +
    自动缩字号下限 0.68×）/`SingleLineText`/`rowDivider`/`HoverTooltip`；④ 弹窗尺寸策略 `modalContentMaxHeight()`
    = 窗口高 × 0.66（下限 320dp）+ 表单内边距紧凑化。**C1 最小落盘清单已齐全**：决策档
    `docs/dev/decisions/D33-响应式与组件统一.md`（含影响面扫描 + 验收 C1-1…C1-8）+ 总体方案
    `docs/design/responsive-components.md` + `增量台账.md` D33 行（有效需求串 = **PRD V2.0 + Δ{…, D32, D33}**）+
    `决策索引.md` + `task-breakdown **T12.7**` + `M12.md §1.7`。**C0 侧**：`M5/M7/M12` §勘误 +
    `design-tokens.md §4.3-1` + `interaction.md §3-13`。**不触 §8.1 五条红线**（原型不适用已勾选）。

32. **第八轮三项按 C0 勘误收口（2026-09-15 人工门第八轮裁决「按建议」）**：**DEF-39 / DEF-40 / DEF-41 全部 C0**（实现偏差纠正，均不建档、不进台账）。**DEF-39** 卡片指标两级误映射（`display` 32sp / `bodyStrong` 14sp）→ 新增 `metricPrimary`(27sp)/`metricSecondary`(21sp) 对齐原型 `.card .big`/`.big.sm`；**DEF-40** 统一表格组件补**纵向列线**（`verticalDivider`，列边界与列宽同源 + 纯函数单测）；**DEF-41** 币种详情成交表由手写实现迁移到 `AdaptiveTable`。回写：模块勘误 `M12.md` §1.7 + `M7.md` §5.z；设计/技术 `design-tokens.md §3/§4.3-1` + `responsive-components.md §2/§3`；裁决记录见 `defects.md §0.1 ⑩`。
    **DEF-39**（卡片指标两级 `metricPrimary` 27sp / `metricSecondary` 21sp）属与 P1 原型视觉基准不一致；
    **DEF-40**（表格纵向列线 `verticalDivider`）属 D33「统一表格组件」网格线口径未补齐；
    **DEF-41**（币种详情成交表接入统一组件）属 D33「既有数据表一律用统一组件」未落实。三项均已修复并加回归，**不阻断** P6。

33. **D34：Windows 安装形态调整 + 便携版产物（2026-09-15 人工门第十轮拍板「A 维持 C0（追认）+ B 采纳 C1（D34）」）**：
    解决 **DEF-42（P1）**——安装版双击弹 `Failed to launch JVM`（TC-MAN-02 步骤 3 阻断）。CI 对照实验实证根因：
    ...
    `%LOCALAPPDATA%\<用户名>\…` → 改为 **per-machine**（`C:\Program Files\WuZhuFolio`）+ 关闭目录选择页，
    并新增三平台**便携版**产物（解压即用，覆盖无管理员权限/自定义位置/非 ASCII 用户名场景）。
    落盘：决策档 `D34-安装形态与便携版.md` + 台账 D34 行（有效需求串 → `Δ{…, D33, D34}`）+ 决策索引 +
    `task-breakdown **T12.8**` + `ADR-006 §1/§1.1/§6` + `M13.md §9` + `test-cases` **TC-MAN-11**；
    随行 **A 项（CI 打包版/安装版启动冒烟）按 C0 追认**（观察期非阻断）。✅ **人工复验闭环（2026-09-21 第十一轮）**：
...
- **P6 → P7 显式延期项（2026-09-21 拍板；**到期检查点 = P7 发布前，未到点前不得视为已完成**）——范围已随同日第十一轮续收敛**：
  ① ~~TC-MAN-01 托盘走查正式判定（Windows）~~ → ✅ **已于 2026-09-21 在 Windows 11 真实桌面判定通过**
  （菜单三项动作 + 关窗行为 + 驻留 + 后台通知；DEF-15 与 DEF-18/19 未复现；留痕 `manual-test-guide.md §19.1`）；
  **仍余**：**Linux / macOS 托盘**（GNOME 需 AppIndicator 扩展；无宿主时验证「关窗即退出」降级分支；DoD = `M11 §5-3`）；
...
- **P6 门须人工执行的用例**（`test-cases.md §7`，**已判通过 10/11**）：~~真实桌面托盘走查~~（✅ **Windows 2026-09-21**；Linux/macOS → P7 携带）/
  ~~读屏 NVDA·JAWS~~（✅ 第七轮）/ ~~真实 Binance 只读 Key 线上同步冒烟~~（✅ 第六轮，Agent 无凭据、不索取）/
  ~~4GB 目标机 KDF ≤2s~~（✅ 第七轮，等效模拟）/ ~~全流程 GUI 走查~~（✅ 第九轮）/ ~~开机自启~~（✅ **第十一轮**）/
  ~~便携版解压即用~~（✅ **第十一轮**）/ **出站抓包 + 权限实证（TC-MAN-09，Ubuntu 侧）——按拍板转 P7 携带**。
...
| 2026-09-11 | 人 | 下达 M12 启动指令 + i18n 范围拍板 | 「执行P4-M12」；T12.4 i18n 范围选 **B 全量 zh/en**（50 文件 / ~620 条文案 + 回归全部 UI 测试）→ 派生 C1 决策档 **D25** |
| 2026-09-12 | 人 | **通过 M12** | 原话「M12通过」——M12 ✅ 关闭，**M1–M12 全部通过**；走查 10 项反馈与 D26 引擎返工全闭环；下一模块 = M13 发布准备（待启动指令） |
| 2026-09-11 | Agent | 完成 M12 UI 整合收尾 | 聚合页三页 + 行情页收尾 + 主壳/状态栏数据源 + 异常态 + a11y + 全量 zh/en（D25）+ M11 §6 四项闭环；602 用例 0 失败 + detekt 0 + 警告 0 + GUI 冒烟 schema=12 驻留；**停人工门待审核** |
...
| 2026-09-09 | Agent | **执行 P4-M8 资金管理** | 人指令「执行p4-M8」；T8.1–T8.3 产物见 docs/dev/modules/M8.md（FundService/CalibrationUseCase 契约 + M011/M012 schema 10→12 + 事件构造层资金半边 LedgerEventAssembler + 资金/校准服务 + 资金页六文件 + M7 交易半边接线[增资解锁手动买入 V5]）；31 新增测试全绿，398 执行 0 失败 + detekt 0 + 编译警告 0 + 无缓存 clean build 全绿 + GUI schema=12 冒烟无异常；§5 规格落档 6 条（V7 同点绝对收紧/锚点入库相对校验/校准入口暂挂资金页/名义零折算/决胜序/TEXT 勘误）待人工走读；**P4 维持进行中，M8 置待审核，停人工门** |
| 2026-09-09 | Agent | **M8 两轮走查反馈修复** | ①候选点选未直达保存（USDT 同名 49 个纯符号解析必歧义）→ pickedCoinId 贯穿（表单/编辑/校准弹窗 + defaultCoin 按 cg_id 直取）+ 候选行展示 cg_id + 文案中文化；②候选列表找不到 tether（同名按名称字母序并列挤出头部）→ M8 层默认币种置顶 + 候选上限 10→20（M3 检索口径不动，排名排序登记为后续提案）；402 测试 0 失败 + detekt 0 + GUI 冒烟 schema=12；M8.md §8 |
| 2026-09-09 | 人 | **通过 M8** | 原话「gui走查完成。M8通过」——M8 ✅ 已通过关闭；M9 备份恢复 / 并行面 M10 设置日志、M11 桌面集成解锁，待人工启动指令 |
| 2026-09-09 | Agent | **远程提交 + CI 留痕** | 人指令「git管理，远程提交」——推送 `56e1c5f..42e7cca`（M8 四提交）；CI run 34434101724 三平台全绿（ubuntu/windows/macos，M011/M012 迁移 + 资金/校准/资金页测试 win/mac 实证）；setup-java v4/Node 20 弃用注解登记 M13 前处理；M8 遗留 5 闭环 |
| 2026-09-10 | 人 + Agent | **M9 走查反馈修复轮 + D24 裁决落档** | 人工门验收反馈：①保存文件无 .cpro 后缀（可读可恢复）→ 保存路径自动补全后缀[BackupFileNames，CSV 同覆盖；恢复侧不校验扩展名]；②同账户增量回导结果页报「持仓异常 fdusd,one,pepe,usdc」→ 根因为账户既存负持仓（非导入造成）→ RestoreSummary 拆分「新产生/既存」双列示；③偏差裁决「备份密码不默认填账户密码、禁空、用户独立设置」→ **D24 决策档 + 台账（有效需求版本串含 D24）+ 五处下游回写（flows/ADR-005 修订/原型/task-breakdown/api-contracts）**；聚焦测试全绿 + detekt 0；M9.md §5-1/§7 |
| 2026-09-10 | 人 | **通过 M9** | 原话「m9通过，完成git管理，远程提交」——M9 ✅ 已通过关闭（走查反馈修复轮复验 + D24 裁决落档）；M10/M11/M12 解锁待人工启动指令 |
| 2026-09-10 | Agent | **M9 CI 勘误 + 远程提交** | 推送 `097215e..eb23aec`（M9 三提交）→ CI run 34448610233 windows-latest 失败：测试断言 `Path.toString()` 字符串比较在 Windows 为反斜杠形态（Linux 不可见）→ 断言改 Path 归一化比较（C0 勘误，M7 §7.9 同类先例）；复跑 run 34449195469 三平台全绿（windows 3m14s/ubuntu 1m48s/macos 2m30s）——**M9 遗留「CI 复跑待推送」闭环**（4 钥匙串真实后端测试 win/mac 实证通过） |
| 2026-09-10 | Agent | **执行 P4-M9 备份恢复** | 人指令「执行P4-M9」；T9.1–T9.4 产物见 docs/dev/modules/M9.md（CproCodec[头部单行+AAD 绑定+Argon2id/GCM+cg_id 跨设备键] + BackupMergePlanner 纯规则[uuid/订单号去重+备份优先覆盖+快照桶幂等] + DefaultBackupService[预览/准备/恢复单写事务/全量覆盖临时备份/CSV 三类无密钥] + ui/backup 数据管理区四步恢复向导[共性约束 7.3 全项]）；29 新增测试全绿，**427 执行 0 失败（431 报告，4 跳过待 CI）** + detekt 0 + 无缓存 clean build 全绿 + GUI schema=12 冒烟无异常；§5 规格落档 6 条（导出默认密码不可实现偏差/模糊级无输入面/快照本地优先/多步聚焦/临时备份同密码/app_version 常量）待人工走读；api-contracts §3/§5 M9 补录；**P4 维持进行中，M9 置待审核，停人工门** |
| 2026-09-10 | Agent | **执行 P4-M10 设置与日志诊断** | 人指令「执行P4-M10」；T10.1–T10.4 产物见 docs/dev/modules/M10.md（通用设置用例[法币/精度/枚举开关/稳定币白名单扩展生效/阈值/代理开关] + 费率完整 CRUD[编辑/改名键迁移] + 日志轮转[1 万条或 90 天先到为准，本地日志 + sync_logs] + 查看与导出日志[逐行脱敏] + 诊断报告[PRD §6 内容受限清单] + **完整设置页单页 8 分组**[主题双向同步持久化 + M5/M6/M7/M9 汇合遗留闭环 + SettingsSectionsHost 移除 + WzSelect/WzSwitch/WzSegmented 组件]）；新增 34 测试全绿，**466 报告（462 执行 0 失败 + 4 跳过待 CI）** + detekt 0 + 无缓存 clean build 全绿 + GUI 冒烟 schema=12、轮转日志行落盘、窗口驻留 rc=124；§5 规格落档 6 条（托盘与后台分组随 M11/代理开关先行/白名单默认固定/同步计数口径/实时轮转未做/主题单一通路）待人工走读；api-contracts §3 + data-model §2.3 M10 补录；**P4 维持进行中，M10 置待审核，停人工门** |
| 2026-09-10 | Agent | **M10 走查反馈修复轮** | 人工 3 项反馈：① 小额阈值重设计（预设档/默认 4000 废弃 → 自由数值输入，基础法币 ≥0，0 = 不启用——用户总额差异大不强加默认；SmallAmountThreshold 枚举删除，data-model/api-contracts 回写）；② 费率交易所 → 可搜索选择（候选 = 已知 BINANCE ∪ 现有规则，输入过滤点击填入，自由输入保留——FeeRateResolver ignoreCase 自由输入安全）；③ 系统代理含义答复（开 = 自动使用系统代理/关 = 直连；应用内 host:port 不在 PRD §7.2-6.2 范围，行为随 M11）；修复轮 +4 测试，470 报告（466 执行 0 失败）+ detekt 0 + GUI 冒烟 0 异常；M10.md §7；**M10 维持待审核** |
| 2026-09-10 | 人 | **通过 M10** | 原话「M10通过」——M10 ✅ 已通过关闭（走查反馈修复轮复验闭环）；M11/M12 解锁待人工启动指令 |
| 2026-09-10 | Agent | **执行 P4-M11 桌面集成** | 人指令「执行P4-M11」；T11.1–T11.3 + M5/M10 三项遗留产物见 docs/dev/modules/M11.md（**T11.3 系统代理**：JdkSystemProxy 置 java.net.useSystemProxies〔JDK 系统代理探测真正前提〕+ 检测[系统代理优先/环境变量兜底] + 两个客户端共用开关感知 selector + 状态栏文字指示与悬停提示 + 开关即时生效〔无需重启〕；**调度循环宿主**：行情按 RefreshCadence〔托盘驻留降频/额度 80% 降档〕+ 同步 15/30/60 + 运行期日志轮转 6h〔M10 §5-5 闭环〕+ 窗口恢复可见即刷〔429 退避内忽略〕+ 窄接口 SchedulerSources + 事件流 + 每 tick 异常隔离；**T11.1 托盘/通知**：AppHost 四者共享同一可见性真源 + 程序化占位图标 + 关窗决策与通知开关抽纯函数 + 托盘不可用降级〔关窗即退出，绝不藏窗口〕；**T11.2 开机自启**：三平台注册/注销/读回全可注入 + 仅打包版注册 + 启动自愈；**托盘与后台分组**：四项键 + 先注册后落键 + DesktopPreferences 镜像 + Mutex 串行 VM）；新增 10 测试文件 **85 用例**，**555 报告（551 执行 0 失败 0 错误 + 4 跳过待 CI）** + detekt 0 + 警告 0 + **无缓存 clean build 全绿（32 任务真正执行）** + GUI 冒烟 schema=12/窗口驻留 rc=124/**代理端到端实证**（https_proxy → indicator=代理：系统代理）；§5 规格落档 6 条 + §7 勘误 2 条待人工走读；data-model §2.3 M11 补录；**P4 维持进行中，M11 置待审核，停人工门** |
...
| 2026-09-12 | 人 | **通过 M13** | 原话「**M13 通过**」——M13 ✅ 关闭，**M1–M13 全部通过，P4 分模块开发收尾完成**（安全自查清单 §1.1 五条硬约束逐条核验 + 打包签名链路 + 四轮 CI 闭环）；**P5 集成与联调 解锁为「进行中」，待人工下达启动指令** |
| 2026-09-13 | 人 | **下达 P5 启动指令** | 原话「执行P5」——关闭 P4 收尾门，P5 集成与联调置为「进行中」，授权执行核心用户旅程联调 |
| 2026-09-13 | Agent | **执行 P5 集成与联调** | 建集成基座（真实组合根 `AppBootstrap.run` + 临时数据目录隔离）→ **核心旅程端到端打通**（登录/记住我 → 增资 → 手动交易 + CSV 导入 → 看板 ROI 101,000/+1.00% → 备份 → 增量合并回导幂等 → 备份元数据/CSV 导出）→ **交易所回环链路**（真实 HTTP + 真 HMAC 签名：同步入账 / 增量幂等 / 校准锚点 / **跨账户恢复后凭证按目标账户 DEK 重加密仍可调用交易所**）→ **异常态 4 项**（A1/V5/V7/V9/BACKUP_INVALID/校准三类阻止）→ **契约一致性核对**（22 接口逐签名，发现 F-1 文档回写缺口 → 回写 M2/M3/M11/M12/M13 五条补录 + MergePlan 口径澄清）→ **真实网络冒烟**（CG 报价+目录 21,162 条+快照落库；Binance 公开端点 3,698 对；全程无 Key，env 门控）→ **D25 到期项闭环**（原型补「界面语言」行 + 3 条 verify 断言）→ GUI 冒烟（schema=12 / 驻留 rc=124 / 权限 700·600 / 0 异常）；**647 用例（641 执行 0 失败 + 6 跳过）+ detekt 0 + 警告 0**；产物 `docs/test/integration-report.md` + 9 项集成测试；**P5 置待审核，停人工门** |
| 2026-09-13 | 人 | **P5 核心旅程人工验收（步骤 1/6/7/8 通过；2–5 报问题）** | ① 首启录入币种「未收录」，稍后才可录入、期间无候选提示；② 增资 100,000 显示可用现金 99,964.80（差额来源不明）；③ 买入 BTC 后看板/资产分布无 BTC，手动刷新才出现；卖出 0.1 BTC 已实现 999.65 而非 1,000；CSV 导入 ETH 时出现「币种不在库」类 toast 但记录存在；④ 看板净值/总收益/ROI 与报告写死的 101,000 对不上；⑤ **跨账户恢复只恢复 1 笔交易（应为 3 笔）**；步骤 1/6/7/8 通过 |
| 2026-09-13 | Agent | **P5 人工验收修复轮（1 P0 + 2 P1 + 1 口径待裁决）** | ① **P0 备份恢复丢行**：`BackupMergePlanner.txOrderKey` 对 null 订单号退化为 `"EXCHANGE|"` → 同交易所手动交易互相判重（增量合并与全量覆盖均丢行）→ 修复为「订单号非空才成键」+ 既有键装配收紧（备份格式/加密不变，旧 `.cpro` 重导即恢复）；域层 +4 回归 + 集成用例扩为 3 笔（修复前必红 3→1）；② **P1 首启目录为空**：行情循环启动即刷新一次（原为等满 5 分钟）→ 首启即可录入；调度 +1；③ **P1 持仓币不刷新**（M5 §6 登记遗留落地）：`defaultCoins` 注入 = 持仓 ∪ 自选（组合根晚绑定，全空仍回退 4 币）；数据层 +2 + 真实网络 `LiveMarketRefreshWiringTest`（`coins=4→5`、BTC 已定价）；④ **口径待裁决**：稳定币按市价折算（999.65/99,964.80 的根因）→ 报告 §8-1 交人工裁决；⑤ 报告 §7 写死数值属脚本口径错误 → 已改自洽校验；**655 用例（648 执行 0 失败 + 7 跳过）+ detekt 0 + 警告 0**；**P5 维持待审核，待人工复验** |
| 2026-09-13 | 人 | **拍板：回写共享规范/PRD 正文 + 自选种子收敛 USDT** | 原话「1. 回写《跨端共享规范》/PRD 正文 2. 行情页自选的「开箱种子」保留的 4 个主流稳定币收敛成只保留 usdt」 |
| 2026-09-13 | Agent | **PRD 基线回写 + 原型/自选种子收敛** | ①《跨端共享规范》**V1.0→V1.1**（名词表 3 处改写 + 新增「1:1 锚定折算」「持仓异常」2 条；折算价格解析/导入路径例外修订；新增文档版本历史与移动端对齐提示）；②`桌面端prd.md` **V1.9→V2.0**（副标题 + V2.0 修订历史行 + 名词 3 处 + 全局说明 2 处 + 故事 6.1/7.2 + 附录 A 用例 6）；③评审材料 `docs/prd/桌面端prd-V2修订说明.md`（差异清单 8 处 + 影响面核对表 + 4 条评审要点，**待人工评审确认**）；④台账 D27/D28/D29 行标注「已回写 + 升版」、有效需求串 = **PRD V2.0** + Δ{...}；⑤行情页自选种子 4 币→**仅 USDT**（`MarketConfig.DEFAULT_WATCH_SEED`）+ 原型 `WATCH_SEED` + `prototype-verify.js` 断言（复跑 errors=[] 83 项全绿）+ 数据层 3 处测试期望；**663 用例（656 执行 0 失败 + 7 跳过）+ detekt 0 + 警告 0** |
| 2026-09-13 | 人 | **P5 人工验收反馈（第五轮）+ 锚定口径裁定** | 问：「增资 100,000 → 买入 1 BTC@50,000 → 仪表盘 ROI −50% 是否正确？是否为 BTC 行情未刷新？」；裁定：「不再允许 USDT 之外再增锚定币；锚定币与稳定币是两个概念——稳定币可以增加，但不是 1:1 锚定的，也有实时价格」 |
...
| 2026-09-14 | 人 | **Windows 11 首轮人工走查** | 报出 6 条：托盘菜单乱码 / 自启未测 / Tab 焦点进不到页面 / 目标机不可得 / 断网指示滞后 / 登录回车不提交 + 进页面键盘不可操作 |
...
| 2026-09-15 | 人 | **第四轮两项变更分级拍板** | 原话「**拍板：按建议变更分级**」——**DEF-20 = C0**（实现偏差纠正，`AGENTS.md §7.3`）、**DEF-21 = C1**（新增焦点行为，未触 §8.1 红线） |
| 2026-09-15 | Agent | **C1 最小落盘清单落地（D31）+ C0 勘误确认** | ① 决策档 **`D31-键盘焦点流.md`**（背景/拍板结论/需求回溯/影响面扫描/关联文档/**验收标准 A1–A6**/P8 登记）；② `增量台账.md` D31 行 + 有效需求串 → **PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31, D32, D33}**（P6 三件套/`integration-report`/STATUS 同步引用）；③ `决策索引.md` D31 行；④ `task-breakdown **T12.6**`（含逐条验收证据映射）；⑤ 下游回写 `ia.md §1.1`（主壳键盘焦点流 5 条）+ `interaction.md §3-9`（键盘导航细则）；⑥ `M12.md §1.7` 升级为「已按 C1 建档 D31」；⑦ `defects.md` DEF-20/DEF-21 分级确认 + §0.1 裁决记录 ⑥；⑧ STATUS 已决策事项 **28** 转已决策。**原型显式勾选「不适用」**（纯键盘焦点行为无可视差异） |
| 2026-09-15 | 人 | **Windows 11 第五轮人工走查（真实币安只读 Key 冒烟）** | 报出 7 条：① API 添加弹窗像插进页面；② 行情数据源弹窗撑开页面；③ 恢复数据弹窗错位；④ 添加密钥保存后弹窗不关但内容已保存（编辑保存会关）；⑤ 设置分组标题比二级标签还小；⑥ 设置页层级字号/字重不统一（手续费加粗、数据管理偏大）；⑦ 问询「同步是否覆盖手写交易」 |
| 2026-09-15 | Agent | **人工门通过记录落盘（第九轮：TC-MAN-08 通过）** | 在 `test-cases.md **§7.0 人工门执行进展**` 与 `manual-test-guide.md **§1.1-1**` 新增统一记录表（10 条用例 × 平台 × 结果 × 判定轮次 × 证据指向）；`test-cases.md` TC-MAN-08 条目标注「2026-09-15 Windows 复验：通过 ✅」；STATUS 当前状态更新为「已通过 7 项 / 剩余 3 项（TC-MAN-01 待正式判定、TC-MAN-02 与 TC-MAN-09 未执行）」。**P6 仍待剩余人工用例 + 发布标准拍板** |
| 2026-09-15 | 人 | **第八轮三项变更分级拍板** | 原话「**按建议**」——**DEF-39 / DEF-40 / DEF-41 全部 C0** |
| 2026-09-15 | Agent | **C0 勘误登记 + 文档回写收口（第八轮三项）** | ① 模块勘误：`M12.md` §1.7（卡片指标两级 `metricPrimary`/`metricSecondary` 对齐原型 + 表格纵线 + 币种详情成交表接入统一组件）、`M7.md` §5.z（交易/资金表纵线）；② 设计/技术回写：`design-tokens.md §3`（卡片指标两级 27/21 与「禁止 32/14 类跨度」）、§4.3-1（横线 + 纵线网格口径）、`responsive-components.md §2/§3`；③ `defects.md §0.1` 新增裁决 ⑩ + DEF-39/40/41 分级确认；④ STATUS 已决策事项 32 转已决策。**均不建档、不进台账**（C0 口径） |
| 2026-09-15 | 人 | **Windows 11 第九轮：TC-MAN-08 真实桌面 GUI 全流程判定通过** | 原话「**TC-MAN-08 真实桌面 GUI 全流程：通过**」——该用例覆盖首启建账→增资→买入→CSV 导入→仪表盘/资产列表/币种详情→设置九组→备份恢复→切换账户登出→三层路径计数→两档窗口核对；经六～八轮修复（DEF-27…DEF-41）后复验通过 ✅。**人工门累计通过 7 项**（03/04/05/06/07/08/10） |
| 2026-09-15 | 人 | **Windows 11 第八轮人工走查（GUI 全流程复验）** | 通过 **TC-MAN-05 断网态 ✅**；报出 3 条：① 仪表盘第一行卡片数字比下面卡片明显偏大；② 表格只有横线、要加竖线；③ 币种资产详情页成交表没有单元格线 |
| 2026-09-15 | Agent | **第八轮修复（DEF-39/40/41）** | ① **DEF-39**：排版令牌新增 `metricPrimary`（27sp/600 衬线 + tnum）/`metricSecondary`（21sp/600），`StatCard` 改映射 → 两级比值 2.3×→1.29×（对齐原型 `.card .big`/`.big.sm`）；② **DEF-40**：`AdaptiveTable` 新增 `verticalDivider`（列边界 1px `line` 色纵线，贯穿表头与数据行，末列不画）+ 纯函数 `columnBoundariesOf` + 单测 `AdaptiveTableGridTest`（3 例）；③ **DEF-41**：币种详情成交表迁移到统一 `AdaptiveTable`（`COIN_TX_COLUMNS`）→ 获得横/纵线 + 单行 + 悬停全值 + 窄窗滚动；文档：`defects.md` DEF-39…41 + `design-tokens.md §3/§4.3-1` + `responsive-components.md §2/§3`；**718 用例（710 执行 0 失败 + 8 跳过）+ detekt 0 + 警告 0**；**P6 维持待审核，待人工复验 + 分级拍板（建议三项 C0）** |
| 2026-09-15 | 人 | **第七轮八项 + 总体方案定级拍板** | 原话「**按建议**」——**DEF-31/32/33/34/36/37 = C0**（实现偏差纠正）；**DEF-35/38 与「响应式与组件统一」= C1** |
| 2026-09-15 | Agent | **C1 落盘（D33）+ C0 勘误收口（第七轮）** | ① 决策档 **`D33-响应式与组件统一.md`**（背景/拍板结论/需求回溯/影响面扫描/关联文档/**验收 C1-1…C1-8**/P8 登记）；② 总体方案 **`docs/design/responsive-components.md`**（断点 → 组件 → 页面三层，含高 DPI 说明）；③ `增量台账.md` D33 行 + 有效需求串 → **PRD V2.0 + Δ{…, D32, D33}**（P6 三件套/`integration-report`/STATUS 同步）；④ `决策索引.md` D33 行；⑤ `task-breakdown **T12.7**`（响应式与组件统一，含逐条验收证据）；⑥ C0 侧：`M5.md`/`M7.md`/`M12.md` §勘误 + `design-tokens.md §4.3-1` + `interaction.md §3-13`；⑦ `defects.md` §0.1 裁决 ⑨ + DEF-31…38 分级确认；⑧ STATUS 已决策事项 31 转已决策 |
| 2026-09-15 | 人 | **Windows 11 第七轮人工走查（GUI 全流程 × 三档分辨率）** | 通过 2 项：**TC-MAN-03 读屏（NVDA）✅ / TC-MAN-04 目标机性能 ✅**；报出 8 条（同源）：① 2560×1600 币种列第三枚标签被裁；② 交易表仍横向滚动 + 手续费换行 + 删除按钮显示不全；③ 交易表单弹窗多一条「不到一行」的滚动条；④ 1024×768 卡片大数字换行变形 + 环形图图例换行；⑤ 1024×768 表格截断无悬停全值；⑥ 交易表删除按钮同 ②；⑦ 增资弹窗同 ③；⑧ 列表缺单元线。**并追问：能否有总体方案（合用表格/卡片组件 + 统一分辨率策略）而非小修小补** |
| 2026-09-15 | Agent | **响应式与组件体系统一（DEF-31…DEF-38）+ 总体方案文档** | ① 新增 **`docs/design/responsive-components.md`** 总体方案（断点→组件→页面三层）；② 新增断点设施 `ui/theme/Breakpoints.kt`（`WzWindowClass` COMPACT/MEDIUM/WIDE，按内容区有效宽度判定；`MainShell` 经 `ProvideWzWindowClass` 提供）；③ 表格组件升级：**列宽按最小宽比例分配**（保证每列 ≥ 最小宽）、**单元线**（`divider` 默认开）、`SingleLineText`（单行 + **截断悬停全值**）、滚动条槽 12dp；④ 新增卡片组件 `ui/components/WzCard.kt`（`WzCard`/`WzCardLabel`/`WzMetric` 单行自动缩字号/`SingleLineText`/`rowDivider`/`HoverTooltip`），`StatCard` 与环形图图例接入；⑤ 弹窗尺寸策略 `modalContentMaxHeight()`（窗口高 × 0.66）+ 交易/增资表单紧凑化；⑥ 资金表接入统一表格；行情自选列表加单元线；⑦ 回归 3 例（徽标完整 + 数值单行；手续费单行 + 删除按钮完整；交易弹窗无滚动）；**713 用例（705 执行 0 失败 + 8 跳过）+ detekt 0 + 警告 0**；**P6 维持待审核，待人工复验 + 分级拍板（建议 C0×5 + C1×2）** |
| 2026-09-15 | 人 | **第六轮四项变更分级拍板** | 原话「**按建议**」——**DEF-27 / DEF-28 / DEF-29 / DEF-30 全部 C0**（实现偏差纠正；人工确认「1024×768 下表格横向滚动」为修复手段而非交互口径变化） |
| 2026-09-15 | Agent | **C0 勘误登记 + 文档回写收口（第六轮四项）** | ① 模块勘误：`M5.md` §5.x（候选浮层必须能自行收起 + 入口焦点声明）、`M7.md` §5.x（窄窗表格/按钮 + 入口焦点）、`M12.md` §1.7（页面入口焦点契约 + 资产表窄窗）；② 设计/技术回写：`design-tokens.md **§4.3-1 窄窗口适配**`（列最小宽/单行/徽标内联/按钮单行 + 三档窗口目标）、`interaction.md §2.7`（候选浮层收起语义）、`§3-9`（入口焦点契约）、`§3-13`（窄窗表格口径）、`ia.md §1.1`（入口控件清单）；③ `defects.md §0.1` 增裁决 ⑧ + 四条分级确认；④ STATUS 已决策事项 30 转已决策。**均不建档、不进台账**（C0 口径） |
| 2026-09-15 | 人 | **Windows 11 第六轮人工走查（真实桌面 GUI 全流程 × 三档分辨率）** | 通过 3 项：**TC-MAN-06 纯键盘全流程 ✅ / TC-MAN-07 真实只读 Key 冒烟 ✅ / TC-MAN-10 外链与关于页 ✅**；新报 6 条：① 设置页打开时焦点定位到中部的「手续费→买入费率」；② 1280×800 资产列表标签竖排 + 数字互挤；③ 1280×800 交易表手续费换行、交易对+估算中两行；④ 1024×768 上述更严重；⑤ 1024×768 删除按钮显示不全且文字竖排；⑥ 1024×768 资金页「近90天」按钮竖排；⑦ 行情页搜索候选浮层只有点「添加」才消失（删光字母也不收起） |
| 2026-09-15 | Agent | **人工门第六轮修复（DEF-27/28/29/30）** | ① **DEF-27（P1）**：新增页面入口焦点契约 `ui/shell/PageEntryFocus.kt`（`PageEntryFocusState` + `Modifier.pageEntryFocus()`），页面在首个可聚焦控件声明入口焦点，主壳切页**优先请求声明目标**（修「子树遍历不按 Tab 序 → 焦点落到页面中部」）；6 个页面已声明；② **DEF-28（P2）**：新增 `ui/components/AdaptiveTable.kt`（列最小宽 + 窄窗整表横向滚动、不换行不截断），资产/交易两表接入；单元格与 `Badge` 统一单行；「估算中」内联；③ **DEF-29（P2）**：操作列最小 120dp、过滤行改 `FlowRow`、`WzButton` 文案单行不换行；④ **DEF-30（P2）**：行情搜索**取消在途搜索 + 结果落地复核关键词 + Esc 取消 + 失败提示**；文档：`defects.md` DEF-27…DEF-30 + `manual-test-guide.md §13` 三档分辨率复验指引；**710 用例（702 执行 0 失败 + 8 跳过）+ detekt 0 + 警告 0**；**P6 维持待审核，待人工复验 + 四项分级拍板** |
| 2026-09-15 | 人 | **第五轮四项变更分级拍板** | 原话「**拍板，按建议变更分级**」——**DEF-22/23（弹层挂页面根）= C0**、**DEF-25（保存失败语义）= C0**、**DEF-24（设置页层级标准）= C1** |
...
| 2026-09-16 | 人 | **第十轮续：实机复验反馈** | ① `msiexec /i .\wzf-windows\...` 在 PowerShell 报「无法打开此安装程序包」（管理员权限亦然），但**文件管理器双击可正常安装**；② **安装后应用可正常打开** ⇒ **DEF-43 修复实机生效** |
| 2026-09-16 | Agent | **DEF-43 闭环 + DEF-44 文档勘误修正** | ① DEF-43 标为**已闭环**（CI `*_AT=PASS` + 人工实机复验通过），§0 汇总同步；② 新增 **DEF-44（P3 · 测试文档勘误 · C0）**：`msiexec` 相对路径在提权后工作目录变为 `C:\Windows\System32` 故失败——`manual-test-guide` 全部安装命令改绝对路径写法（`Resolve-Path` + `-Verb RunAs`）+ 新增原因说明与「双击闪窗属正常」注记，`keyboard-walkthrough` 同步 |
| 2026-09-16 | 人 | **第十轮续两项拍板** | 原话「**按建议**」「**按建议两条**」——① **DEF-43 = C0**（实现/打包偏差：模块集漏装 `jdk.accessibility`）；② **DEF-42 的 jpackage 非 ASCII 路径限制 = 登记 P8 观察项 + 写入 P7 用户指南提示**（安装/解压路径请用英文） |
| 2026-09-16 | Agent | **两项裁决落地（C0 收口 + P8/P7 登记）** | ① DEF-43 C0：`M13.md §7` 勘误 ③ + `ADR-006 §1.1`（无障碍模块为必需）已回写，**不建档、不进台账**；`defects.md §0.1` 增裁决 ⑫、DEF-43 标题与分级行改「人工拍板 C0」、验收行含 CI 证据；② DEF-42：`defects.md` 增「本机症状最终归属 DEF-43」「P8 登记（立项输入 + 当前缓解 + 后续方案 + 影响面）」「P7 携带（用户指南两条提示）」三行，标题改「已处置」；③ `test-report.md` §5.4 残留风险表 +2 行、DoD 行同步；④ STATUS 增 P8 立项输入与 P7 携带项 |
| 2026-09-16 | 人 | **第十轮续：控制台包输出真实错误** | 控制台版调试包运行输出 `java.awt.AWTError: Assistive Technology not found: com.sun.java.accessibility.AccessBridge`（栈指向 Skiko/UIManager），末行 `Failed to launch JVM`；同轮提供 `JAVA_TOOL_OPTIONS/_JAVA_OPTIONS/JDK_JAVA_OPTIONS` 均为空、`JAVA_HOME=C:\Users\lifxu\.version-fox\sdks\java` |
| 2026-09-16 | Agent | **DEF-43 定位 → 修复 → 本地复验（建议 C0）** | ① 本机复现：AT 属性 + 打包 app-image → 同型 `AWTError`，`runtime/release` MODULES 无 `jdk.accessibility`；② 修复：模块集补 `jdk.accessibility` + 新增 `AssistiveTech.kt`（AWT 前校验，不可用则清空属性并告警）+ `Main.kt` 接入 + `AssistiveTechTest`（4 例）；③ CI：探针加 `-AssistiveTech`，app-image 与**安装版**冒烟各加 AT 变体；④ 文档：`defects.md` DEF-43（含影响面扫描与需求回溯）+ `M13.md §7` 勘误 + `ADR-006 §1.1`（无障碍模块为必需）+ `manual-test-guide §16.2-1/TC-MAN-03`；⑤ 本地 `clean build detekt --no-build-cache` 全绿 |
| 2026-09-15 | 人 | **第十轮 DEF-42 两项拍板** | 原话选择「**A 维持 C0（追认）+ B 采纳 C1（D34），下一步实施 B**」——A = CI 打包版启动冒烟（观察期非阻断）追认为 C0；B = 安装形态调整 + 便携版产物按 C1 建档实施 |
| 2026-09-15 | Agent | **D34 落地（C1 最小落盘清单）** | ① 打包配置：`windows { perUserInstall = false; dirChooser = false }`（默认 `C:\Program Files\WuZhuFolio`，纯 ASCII）；② 三平台**便携版**产物（CI `portable/*`，含 LICENSE + README）；③ CI 新增「安装版实跑冒烟」（MSI 静默安装 → 断言落点 → 实跑 → 卸载）与按需 JDK21 实验 job；④ 决策档 **D34** + 台账 D34 行（有效需求串 → `PRD V2.0 + Δ{…, D33, D34}`）+ 决策索引 + `task-breakdown **T12.8**`（验收 A1–A7）+ `ADR-006 §1/§1.1/§6` 回写 + `M13.md §9` + `defects.md` DEF-42/裁决 ⑪ + `test-cases` 新增 **TC-MAN-11**（总数 299→300）+ `manual-test-guide §5.3/§16.5/§17`；⑤ 本地 `clean build detekt --no-build-cache`：**718 用例（710 执行 0 失败 + 8 跳过）+ detekt 0 + 警告 0**。**P6 维持待审核**，待人工复验 TC-MAN-02（先卸载旧 per-user 版）与 TC-MAN-11 |
...
| 2026-09-14 | Agent | **执行 P6 系统测试与质量** | 产出 `test-plan.md`（分层策略 + 9 项开放项处置 + 人工门 11 条）· `test-cases.md`（**299 条**：✅186/🟡87/⬜10/🔵16，逐条可追溯）· `security-checklist.md`（**P6 复跑版**：五条硬约束逐条打勾 + 4 项到期项闭环）· `defects.md`（**P0/P1=0**，P2=6）· `test-report.md`（主产物，含待裁决 5 项）；**修复 3 项 P2 实现偏差**（DEF-01 备份导出失败模式 / DEF-02 recvWindow 契约 / DEF-06 恢复向导映射）+ 闭环 4 项 M13 到期登记项（大载荷内存曲线 / settings 键命名空间 / 登出 tick / P5-4）+ 运行期出站抓包与 GUI·权限实证；回写 `interaction.md §2.9`、`api-contracts.md §3/§4`；**677 用例（669 执行 0 失败 + 8 跳过）+ detekt 0 + 警告 0**；**P6 置待审核，停人工门** |
| 2026-09-21 | 人 | **Windows 11 第十一轮：TC-MAN-02 开机自启 + TC-MAN-11 便携版两项通过** | 原话「**P6的开机自启、便捷版运行，两项都通过**」——被测产物 = **DEF-43 修复版**（run [35115439554](https://github.com/mapleafly/wuzhufolio/actions/runs/35115439554)，commit `6f2c77b`，msi `5d2ec322…`，`build=0.1.0+6f2c77b`）：**① 打包安装版（per-machine，`C:\Program Files\WuZhuFolio`）双击正常启动**（第十轮的 `Failed to launch JVM` 不再出现）→ **D34 A2 ✅**；**② TC-MAN-02 开机自启全链路 ✅**（开关 → `reg query "HKCU\...\Run"` 有项 → 注销重登驻留托盘 → 关开关后注册项消失）；**③ TC-MAN-11 便携版解压即用 ✅**（D34 便携包解压到纯 ASCII 路径即可运行）→ **D34 A5/A6 ✅**。据此 **DEF-42 / DEF-43 闭环、P1 归零**、**P6 DoD「P0/P1 清零」达成**；**人工门累计通过 9/11**（剩余 TC-MAN-01 托盘判定、TC-MAN-09 抓包+权限）。 |
| 2026-09-21 | Agent | **第十一轮复验结果落盘（P1 归零 + 四项验收闭环）** | ① 判通过记录三处同步：`test-cases.md **§7.0**`（TC-MAN-02 改 ✅、**新增 TC-MAN-11 行**、剩余 4→2 项、两卡片加判定注）、`manual-test-guide.md **§1.1-1**`（同步 + **新增 §19 第十一轮复验结果**：被测形态/两项结果/对 P6 门的意义/遗留）与 **§6 TC-MAN-02** 卡片；② 缺陷闭环：`defects.md` **DEF-42 标题改 ✅ 已闭环** + 新增「人工复验（2026-09-21 · A2 闭环）」行、§0 汇总与合计行、**§0.1 裁决 ⑬**、**新增 DEF-46（P3 · 测试文档勘误 · C0）**；③ 验收留痕补齐：**D34 §6** A2/A5/A6 转 ✅ + 人工复验留痕块、`task-breakdown **T12.8**` 验收句、**M11 §5-2 延期项闭环**（打包版自启端到端，提前于 P7）+ §5-3 托盘进展注、**M13 §9** 验收行、`增量台账` D34 行与说明、`test-plan` §2.3/§7 行、`security-checklist §7` 第 7/8/10/12 行与 §8 复跑计数；④ **DEF-46 修复**：`manual-test-guide.md` 自 §16.4 起**整段复粘**（重复 115 行、且两版 §16.6 探针命令互相矛盾）→ 删除复粘副本、保留首份与 §18.1，**1015 → 900 行**；⑤ `test-report.md`：§0 DoD 行改 ✅、一句话结论、状态行、§6 仍需人工项、**§6.1 进展表**、§7 残留风险 3 行 + TOTAL 计数 708→718 口径统一；⑥ STATUS：当前阶段/阶段总览 P6 行/当前阻塞点/三项延期验证在册 ②/交接记录。**未改任何产品代码**（复验为纯人工判定 + 文档留痕），用例数维持 **718（710 执行 0 失败 + 8 跳过）**。 |
| 2026-09-21 | 人 | **拍板关闭 P6 门 + 剩余两项转 P7 携带** | 选项原话「**先关闭 P6 门并把两项登记为 P7 携带**」——P6 DoD 三项全部达成（P0/P1 = 0 / P2 明确结论 / `security-checklist.md` 全部通过）+ 人工门用例通过 9/11；**TC-MAN-01（托盘走查正式判定）与 TC-MAN-09（出站抓包 + 权限实证）显式延期至 P7**，到期检查点 = **P7 发布前**，未到点前不得视为已完成。 |
...
| 2026-09-22 | 人 | **P7 发布人工门：批准发布（范围收敛 Windows + Linux）** | 原话要点「**批准发布：只发布 windows 版和 ubuntu 类适用的版本**（这两个我可以本机测试，其他暂时还不能测试发布版本），其他按照建议进行」「§9 剩余 3 项按建议进行；**Linux 托盘与开机自启真机补测放在发布后，从 github 下载安装包进行真机测试**」「**证书采购：需要更多信息才能判断**」。 |
