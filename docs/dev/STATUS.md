...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M11 桌面集成 ✅ 已通过（2026-09-10 人工「m11 通过」关闭）**；M1–M11 均已通过。P0–P3 均已关闭。
...
- **下一人工门**：无待审核项——**M12 UI 整合收尾**（主壳整合 + 聚合页〔仪表盘/资产列表/币种详情〕+
  行情页 UI 收尾 + 异常态全量走查 + a11y + i18n + 精度/阈值消费 + 状态栏数据源 + M11 §6 交接的四项打磨）
  待人工下达启动指令。
  **M11 的两项延期验证仍在册**（M11.md §5-2/§5-3，不阻塞 M12 启动但须在到期检查点前完成）：
  ① 托盘 GUI 走查须在真实 Windows/macOS/Linux 桌面执行（检查点 = M12 走查或 P6 系统测试）；
  ② 开机自启端到端须在 P7 打包版执行（检查点 = P7 发布前）。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ✅、M4 ✅、M5 ✅、M6 ✅（2026-09-07）、M7 ✅（2026-09-08）、M8 ✅（2026-09-09）、M9 ✅（2026-09-10）、M10 ✅（2026-09-10）、**M11 ✅（2026-09-10）**；下一模块 = M12（待启动指令） |
...
- **P4 下一启动点（2026-09-10）**：**M11 桌面集成 ✅ 已通过关闭**（人工「m11 通过」；两项环境相关验证
  按拍板延期留册，到期检查点见 M11.md §5-2/§5-3）。下一模块 = **M12 UI 整合收尾**（T12.1 主壳整合 +
  聚合页〔仪表盘/资产列表/币种详情〕+ 行情页 UI 收尾 / T12.2 异常态全量走查 / T12.3 a11y / T12.4 i18n；
  另接收 M11 §6 交接的四项打磨：托盘正式图标资产、状态栏数据源与 429 频发文案、
  toast 遮挡打磨、通用组/网络组 VM 防重入写法统一），待人工下达启动指令。
