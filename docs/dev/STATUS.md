...
- **P4-M6 待人工三轮复验（根因已闭环）**：M6 交易所同步 2026-09-05 产出；人工门反馈 3 项已修复（M6.md §8.1–8.3）后二轮复验仍见「新增 0 · 未解析跳过 120」→ 定性 = myTrades 拉取已通、卡在本地币种解析（详见 M6.md §8.5）；三轮（2026-09-07）按人工带回样本定位根因 = BNB 同名 2 资产 + 排名缓存未接/未热 → 已修（排名缓存接入共享目录 + warmUpRankCache() 每轮自动预热，回归测试覆盖，详见 M6.md §8.6）；二轮已做：① 解析失败原因上浮（sync_logs message 附首笔样本「交易对：基础币 X 未收录/歧义」+ 应用日志 warn 3 条样本）；② 交易所同步改用共享市值排名缓存目录（消歧规则③生效，M3 遗留 4 落地）。请三轮复验：立即同步后把 message 首笔样本（或日志 warn「sync unresolved samples」行）带回，以定位资产面（未收录 vs 歧义）；若大量歧义且市值为 0，先点一次「立即刷新行情」再同步。其余复验项同 §8.4（编辑/移除已按修复轮验证）。时间范围口径：首次同步 = 每候选交易对最近成交 ≤500 笔（非固定时间窗），增量按成交 id；更早历史 CSV。验收通过后解锁 M7/M10（由人工拍板）。转后续模块注意清单：M6 遗留（同步定时宿主归 M11、SettingsSectionsHost 汇合归 M10、transactions 表 M009 已建 M7 复用、真实 Binance 冒烟留 P5、api_keys 备份归 M9、CI 三平台复跑待推送）+ M5 遗留（折算解析接线归 M7、调度循环宿主归 M11、状态栏提示归 M12、真实 API 冒烟留 P5）+ M4 遗留（事件构造层接线归 M7/M8、费率 CRUD 接线归 M7/M10、校准执行流归 M8、校验宽松口径复核归 M8）+ M3 遗留（搜索扫描优化、CI 三平台复跑待推送）+ M2 遗留（手动/CSV/恢复向导占位归 M7/M9、枚举开关 UI M10、顶栏/下拉/眼睛 M12）+ 4GB 目标机 KDF 复核改为 M13 前开放待办。
