...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M2 账户与会话 ✅ 已通过（2026-09-03 人工「M2 通过」）**；下一模块 M3 币种主数据已解锁为「进行中」，待人工下达启动指令。P0–P3 均已关闭。
...
- **下一人工门**：**M3 币种主数据（T3.1 coins 目录/T3.2 CoinResolver 消歧/T3.3 FiatNormalizer）**——验收标准见 docs/tech/task-breakdown.md §3 M3；依赖 T1（已过）；待人工「执行P4-M3」启动指令。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅（2026-09-03 人工「M2 通过」）；下一模块 = M3（已解锁，待启动指令） |
...
- **P4-M3 待启动指令**：M2 已于 2026-09-03 人工「M2 通过」关闭 ✅；M3 币种主数据（T3.1–T3.3）解锁为进行中，待人工「执行P4-M3」启动指令。转 M3 注意清单：M2 遗留（向导占位 M6/M7/M9、枚举开关 UI M10、顶栏/下拉/眼睛 M12）、4GB 目标机 KDF 复核改为 M13 前开放待办、钥匙串真实后端 CI 实证（KeyringRememberMeStoreTest 已随 M2 关闭提交入 CI）。
