...
11. **【第四轮：PRD 基线回写 + 原型/自选种子收敛（2026-09-13 人工拍板）】**
    ...
    📌 **待人工评审确认**）。有效需求串 = **PRD V2.0** + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31}；
...
> **输入**：P5 集成版 + PRD V2.0（有效需求 = PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31}）+
...
| 18 | 上述四项的**变更分级** | 待人工拍板：**DEF-22/23/25 建议 C0**（实现偏差纠正，先例 DEF-01/§7.3）；**DEF-24 建议 C1**（新增设计规范条目「设置页层级标准」+ 跨模块样式统一）→ 拍板 C1 后补决策档 + 台账 + 下游回写 |
...
28. **键盘焦点流（D31，2026-09-15 人工门第四轮裁决「按建议变更分级」· C1）**：人工第四轮走查提出两项
    ...
    **C1 最小落盘清单已齐全**：决策档 `docs/dev/decisions/D31-键盘焦点流.md`（含影响面扫描 + 验收标准 A1–A6）+
    `增量台账.md` D31 行（有效需求串 = **PRD V2.0 + Δ{D21, D24, D25, D26, D27, D28, D29, D30, D31}**）+
    `决策索引.md` + `task-breakdown **T12.6**` + `ia.md §1.1` + `interaction.md §3-9` + `M12.md §1.7`。
    **不触 §8.1 五条红线**（无数据/schema/加密/备份格式/接口变更；原型不适用已在台账勾选）。

29. **⏳ 待人工拍板：第五轮四项变更分级（DEF-22/23/24/25，2026-09-15）**：**DEF-22/23**（设置页弹层挂页面根，
    新增 `PageOverlayHost/PageOverlay`）**建议 C0**（实现偏差纠正：§7.3 要求弹层覆盖页面；新增组件为修复手段，
    不新增产品语义）；**DEF-25**（API 密钥保存语义：落库后失败不再上抛、弹窗照常关闭）**建议 C0**（失败模式补全，
    先例 DEF-01）；**DEF-24**（设置页层级标准 `sectionTitle` 15/600 + 跨模块样式统一、回写 design-tokens §3）
    **建议 C1**（新增设计规范条目）；**DEF-26** 为核实非缺陷（不涉及分级）。**拍板 C1 后**由 Agent 补决策档
