...
- **勘误（C0）**：`CoinResolutionException` 增类型化 `kind`、`FeeRulePolicy.validate` 改类型化 `Error`
  ...
  detekt 配置补回测试目录排除
- 测试：**612 用例（608 执行 0 失败 + 4 跳过待 CI win/mac）** = domain 209（+5） / data 255（+18） /
  ui 132（+34） / app 16；detekt 0；**编译警告 0**；无缓存 clean build 全执行
...
② 复跑 `./gradlew clean build detekt --no-build-cache`（应 612 用例 = 608 执行 0 失败 + 4 跳过 + detekt 0 + 警告 0）；
