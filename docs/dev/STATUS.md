...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M11 桌面集成 ⏳ 待审核（2026-09-10 Agent 完成，停人工门）**；M1–M10 均已通过。P0–P3 均已关闭。
...
- **下一人工门**：**M11 桌面集成待人工走查**（T11.1 托盘/通知 · T11.2 开机自启 · T11.3 系统代理 +
  调度循环宿主 + 托盘与后台设置分组）。**注意两项环境限制**（模块记录 M11 §5-2/§5-3）：
  ① 本机 WSLg `SystemTray.isSupported()=false`，托盘交互须在真实桌面验证；
  ② 开机自启仅在打包版可用（开发态如实置灰），端到端验证延期至 P7。
  **上述两项已于 2026-09-10 人工拍板认可（「托盘留待真实桌面再验证」「开机自启动延期」），
  第 3 项「按建议办理」（M11 新分组 Mutex 串行 + M10 旧写法登记 M12 + toast 遮挡登记 M12）同批拍板；
  仍不等于模块走查通过——M11 维持「待审核」，待人工完成 §4 步骤 4 走查后关闭。**
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ✅、M4 ✅、M5 ✅、M6 ✅（2026-09-07）、M7 ✅（2026-09-08）、M8 ✅（2026-09-09）、M9 ✅（2026-09-10）、M10 ✅（2026-09-10）、**M11 ⏳ 待审核（2026-09-10）**；下一模块 = M12（待启动指令 / M11 通过后解锁） |
...
## P4 · M11 桌面集成（⏳ 待审核--2026-09-10 Agent 完成，停人工门）
...
- **P4 下一启动点（2026-09-10）**：**M11 桌面集成 ⏳ 待人工走查**（T11.1/T11.2/T11.3 + 调度循环宿主 +
  托盘与后台设置分组）。**两项环境限制须先读**（模块记录 `docs/dev/modules/M11.md` §5）：
  ① **本机 WSLg 无系统托盘**（`SystemTray.isSupported()=false`）→ 托盘菜单/驻留/通知的 GUI 走查
  须在真实 Windows/macOS/Linux 桌面执行；降级路径（关窗即退出 + 设置行置灰）本机已实证。
  ② **开机自启仅打包版可用**（开发态无 `jpackage.app-path`，`supported=false` 如实置灰）→
  「开启后自启驻留托盘」端到端延期至 P7 打包版验证。
  本机可完整验证的部分 = 代理检测与状态栏指示（已端到端实证）、调度宿主启动与降频口径、
  设置分组四行开关与写入、关窗决策与通知开关（单测钉死）。
