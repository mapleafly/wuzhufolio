...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M8 资金管理 ✅ 已通过（2026-09-09 人工「gui走查完成。M8通过」关闭，含两轮走查反馈闭环：候选点选直达保存 + 默认币种置顶）**；M1–M8 均已通过。P0–P3 均已关闭。
...
- **下一人工门**：无待审核项——**M9 备份恢复（T9.1–T9.4，.cpro 导出/导入；transactions/capital_flows/reconciliation_records 全部就绪）或并行面 M10 设置与日志诊断 / M11 桌面集成**待人工下达启动指令。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ✅、M4 ✅、M5 ✅、M6 ✅（2026-09-07）、M7 ✅（2026-09-08）、**M8 ✅（2026-09-09，含两轮走查反馈闭环）**；下一模块 = M9/M10/M11（待启动指令） |
...
- **P4 下一启动点（2026-09-09）**：M8 已通过关闭 ✅；下一模块 = **M9 备份恢复**（.cpro 导出/导入/合并幂等——transactions/capital_flows/reconciliation_records 全部就绪，黄金用例 10 归 M9）或并行面 **M10 设置与日志诊断**（费率 CRUD 扩展 + 设置页汇合 + 日志诊断）/ **M11 桌面集成**（调度循环宿主 + 托盘），待人工下达启动指令。
