...
> **另提报引擎级缺陷待人工定级（§7.4，建议 C2）**：负持仓区间仍累积成本 → 恢复正持仓后平均成本与
> 已实现盈亏失真（实测 BTC 均价被抬到 997,018、单笔卖出 −57,843.47）；本模块**未动引擎**，仅界面标注兜底。
>
...
- **勘误（C0）**：`CoinResolutionException` 增类型化 `kind`、`FeeRulePolicy.validate` 改类型化 `Error`
  ...
  detekt 配置补回测试目录排除
- 测试：**606 用例（602 执行 0 失败 + 4 跳过待 CI win/mac）** = domain 204 / data 254（+17） /
...
② 复跑 `./gradlew clean build detekt --no-build-cache`（应 606 用例 = 602 执行 0 失败 + 4 跳过 + detekt 0 + 警告 0）；
...
**遗留问题（转后续模块，详见模块记录 §6）**：**引擎负区间成本累积缺陷定级（§7.4，建议 C2，人工门待拍板）**；
P6 —— 读屏（NVDA/JAWS）实测、`TxFilter.coinSymbol` 子串匹配口径、
