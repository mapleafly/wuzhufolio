...
- **当前状态**：**P4 分模块开发 ⏩ 进行中——M8 资金管理 ⏳ 待审核（2026-09-09 产出，停人工门）**；M1–M7 均已通过。P0–P3 均已关闭。
...
- **下一人工门**：**M8 资金管理（T8.1–T8.3）人工验收**——按 docs/dev/modules/M8.md §4 走查（重点：增资/撤资/校准三路径 + V7/V9 文案 + 校准留痕）；通过后解锁 M9 备份恢复或并行面 M10/M11。
...
| P4 | 分模块开发 | 进行中 | 代码 + `docs/dev/modules/` | M1 ✅、M2 ✅、M3 ✅、M4 ✅、M5 ✅、M6 ✅（2026-09-07）、M7 ✅（2026-09-08）；**M8 ⏳ 待审核（2026-09-09）**；下一模块 = M9/M10/M11（待 M8 过门后拍板） |
...
## P4 · M8 资金管理（⏳ 待审核--2026-09-09 产出，停人工门）
...
- 测试：**398 全绿 0 失败**（domain 160 / data 166 / ui 72；新增 29 项：data 资金 11 + 校准 9 +
  ui 走查 9）+ detekt 0 + 编译警告 0；无缓存 clean build 全绿
...
2. 复跑 clean build detekt（命令见 M8.md §4，应 398 测试 0 失败 + detekt 0 + 警告 0）；
3. 聚焦：`:data:test --tests "…DefaultFundServiceTest"`（11）/ `"…CalibrationServiceTest"`（9）/
   `:ui:test --tests "…FundsPageUiTest"`（9）；
...
- **P4 下一启动点（2026-09-09）**：M8 资金管理已产出（398 测试全绿 + GUI schema=12 冒烟无异常），**停在人工门待验收**——按 docs/dev/modules/M8.md §4 走查；通过后解锁 M9 备份恢复或并行面 M10/M11。
...
| 2026-09-09 | Agent | **执行 P4-M8 资金管理** | 人指令「执行p4-M8」；T8.1–T8.3 产物见 docs/dev/modules/M8.md（FundService/CalibrationUseCase 契约 + M011/M012 schema 10→12 + 事件构造层资金半边 LedgerEventAssembler + 资金/校准服务 + 资金页六文件 + M7 交易半边接线[增资解锁手动买入 V5]）；29 新增测试全绿，398 执行 0 失败 + detekt 0 + 编译警告 0 + 无缓存 clean build 全绿 + GUI schema=12 冒烟无异常；§5 规格落档 6 条（V7 同点绝对收紧/锚点入库相对校验/校准入口暂挂资金页/名义零折算/决胜序/TEXT 勘误）待人工走读；**P4 维持进行中，M8 置待审核，停人工门** |
