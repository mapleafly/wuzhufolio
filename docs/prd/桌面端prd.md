...
| **V2.0** | 2026-09-13 | 产品部 | 定稿 | 按 P5 集成联调人工拍板（决策档 D27/D28/D29，《跨端共享规范》同步升至 V1.1）修订折算与净值口径：①**稳定币白名单默认仅 USDT（固定不可移除）**，其他稳定币可在设置中扩展/移除；②**1:1 锚定折算集合固定 = USDT**——基础法币为 USD 时 USDT 折算价恒为 1（优先于市价快照），白名单扩展项与其他币种一律按市价折算（修正此前「稳定币按市价折算导致票面金额被打 0.9996 折」的口径）；③**负持仓币市值不计入净值与可用现金**（导入路径产生的账本数据缺口信号；界面标记「持仓异常」+ 仪表盘显式提示被排除金额，补录/校准后自动恢复）；④同步修订名词解释（可用现金余额/稳定币白名单/资产净值，新增「1:1 锚定折算」「持仓异常」）、全局说明「成本计算规范」折算价格解析与导入路径例外、故事 6.1/7.2 设置项表述 |
...
* **名词解释**
  ...
  * **可用现金余额 (Available Cash Balance):** 账户内现金类币种（稳定币白名单）持仓折算基础法币后的合计，为派生展示值（不单独记账），便于快速查看可随时用于交易或提现的部分。其中 **USDT 恒按 1:1 折算**，白名单内的其他稳定币（用户扩展项）按当前市价折算；持仓为负的币种不计入。
  ...
  * **稳定币白名单 (Stablecoin Whitelist):** 用于判定“现金类币种”，**默认仅 USDT（固定项，不可移除）**；其他稳定币可由用户在设置中扩展与移除。白名单决定「可用现金余额」的统计范围，不决定折算方式。
  * **1:1 锚定折算 (USD-Pegged Conversion):** 基础法币为 USD 时 **USDT 折算价恒为 1**（优先于市价快照）；**锚定集合固定 = {USDT}，不提供任何扩展方式**，白名单扩展项与其他币种一律按市价折算。**「锚定币」与「稳定币」是两个概念**——稳定币可任意增删（白名单），但只有 USDT 享受 1:1，其余稳定币按实时市价折算。
  * **持仓异常 (Negative Position Anomaly):** 某币种持仓为负的状态，只可能来自导入路径（CSV 导入 / API 同步不做余额校验）；其市值**不计入净值与可用现金**，界面标记「持仓异常」并显式提示被排除金额，补录增资/补导入历史/校准后自动恢复。
...
* **成本计算规范（平均成本法）**
  ...
  * **导入路径例外：** CSV 导入与 API 同步不做余额校验；导入后若某币种持仓为负，在资产列表该行标记“持仓异常”，并引导用户补录增资、补导入历史数据或执行持仓校准。**负持仓币种市值不计入净值与可用现金**，仪表盘显式提示被排除的金额（不做隐式扣减、也不静默忽略）。
  * **持仓校准规则：**
  ...
  * **折算价格解析：** ① 基础法币为 USD 且币种为 **USDT（1:1 锚定）** → 折算价恒为 1；② 其余币种“记录时行情价”取数顺序为本地价格快照 → 行情 API 回填（近 90 天小时级、更早当日日线价）；离线无法获取时记录标记“待定价”，联网后自动回填并重算（见全局说明“行情数据与时间分辨率规则”）。当前估值（市值/可用现金/未实现盈亏/24 小时盈亏）与事件折算共用同一取数链，保证成本与市值口径一致；USDT 的 24 小时盈亏恒为 0。
...
* **用户故事 3.1 (仪表盘):**
  ...
  * **验收标准:**
    ...
    2. 显著位置显示以基础法币（默认为USD）计价的总资产净值（全部币种持仓市值之和，**持仓为负的币种不计入**；其中现金类币种持仓合计即“可用现金余额”）。
...
* **6. 设置 (Settings)**
  * **6.1 通用设置:** 
    ...
    * **稳定币白名单:** 允许用户查看并扩展稳定币白名单（**默认仅 USDT，固定不可移除**；其他稳定币可扩展与移除），白名单内币种视为“现金类币种”，合计计入“可用现金余额”；其中 **USDT 按 1:1 折算**，扩展项按市价折算。
    * **主题切换:** 提供明亮/黑暗主题切换选项。
...
6. **CSV 导入产生负持仓后补增资**：CSV 导入卖出 1 BTC 但本地此前无 BTC → 持仓 −1 BTC 标记“持仓异常”，**其市值不计入净值与可用现金**（仪表盘显式提示被排除金额；V2.0 口径）；补录 1 BTC 增资后重放，持仓归 0 以上、异常标记消除、市值自动重新计入。
