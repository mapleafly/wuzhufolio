...
docs/
  ...
  release/    # P7 发布
  dev/        # 过程状态：STATUS.md（状态看板）、decisions/（D 系决策档 + 决策索引.md）、增量台账.md（变更控制台账）、modules/（模块完成记录）、retrospective.md
...
5. **弹层必须挂在页面根、覆盖整页**（2026-09-15 人工门第五轮 DEF-22/23 确立）：就地叠加层依赖 `fillMaxSize()`
   铺满页面，而 `verticalScroll`/`LazyColumn` 等滚动容器给子项的**高度约束是无限的**——弹层若落在滚动容器内部，
   只能退化为内容高度，表现为「撑开页面 / 挤占后面内容 / 浮在某个区域上看着错位」（实测：设置页添加 API、
   行情数据源、恢复数据三处弹窗）。统一做法：页面根 `PageOverlayHost(Modifier.fillMaxSize())` 包住滚动内容，
   深层组件 `PageOverlay { WzModal(...) }` 提交弹层到页面根；新增「滚动容器 + 深层弹层」结构前先读本条。
...
## 8. 变更控制（需求基线锁定后，2026-09-04 人工拍板确立）

> P0 定稿后 `docs/prd/` 为只读基准。进入代码阶段（P3/P4 起）后新增/修改/删除功能，必须先**分级**再走对应流程：
> 不得静默实现、不得先码后补文档。**有效需求 = PRD 定稿版 + `docs/dev/增量台账.md` 的全部 Δ**——P6 系统测试、
> P8 复盘、移动端对齐、以及 Agent 接任务时，均以此为准，而非只看 PRD 正文。

### 8.1 变更分级（C0 / C1 / C2）

| 级别 | 定义 | 流程 | PRD 正文 |
|------|------|------|----------|
| **C0 勘误** | 不改产品语义，纠正口径/笔误/实现偏差 | 模块记录 §勘误 + 回写对应技术文档；**不建决策档、不进台账** | 不改 |
| **C1 增量/小改** | 纯新增、**轻量修改**或**无副作用删除**，不破坏既有需求、无新敏感面、不返工已通过模块 | **决策档 + 台账登记 + 下游文档回写 + 代码**（见 §8.2） | 不改（只读） |
| **C2 大修改** | 命中下述任一红线 | **mini-P0/P1/P2 闭环**（见 §8.3） | 由人定；改则升版本号并走评审 |

**升 C2 的红线**（命中任一即 C2）：
1. 触及 §1.1 硬约束（本地化 / 零遥测 / 密钥与加密 / 备份格式）；
2. 改数据模型表结构或加密边界；
3. 改**已通过模块**的行为（**返工级**：改数据/接口/反转行为；参数/文案小调不算）；
4. 跨端影响（移动端 PRD/SRD 亦需同步）；
5. 删除或反转**已定稿/已交付**需求（占位/草稿的清理不算）。

**判定流程**（级别由**影响面**决定，不由动词「增/改/删」决定）：
1. 仅纠正口径/笔误/实现偏差、不改产品语义 → **C0**；
2. 触任一红线（上表 1–5）→ **C2**；
3. 其余（纯新增 / 轻量修改 / 无副作用删除）→ **C1**。

> 例：「行情页默认种子 4→6 币」= 无副作用小改 → C1；「删一个从未建成的占位入口」→ C1；「删核心功能/改表结构/改加密」→ C2。

### 8.2 C1 增量/小改的最小落盘清单（DoD）

Agent 完成 C1 增量/小改的判定标准（缺任一不算完成）：
1. **决策档** `docs/dev/decisions/Dn-*.md`（背景 / 拍板结论 / 需求回溯 / **影响面扫描** / 关联文档更新）；
2. **台账登记**：`docs/dev/增量台账.md` 加一行，并勾选下游文档回写状态；
3. **下游文档回写**（按影响面勾选）：
   - 设计侧：`ia.md` 页面清单 / `flows.md` / `interaction.md` 异常态 / `prototype/`（可显式延期，见下）；
   - 技术侧：`data-model.md` / `api-contracts.md` / `task-breakdown.md`（**新增任务编号**）。
4. **验收标准进入 task-breakdown**：不得只写在模块记录里；
5. 模块记录 `docs/dev/modules/<模块名>.md` 摘要 + STATUS「已决策事项」决策号 + 「交接记录」行。

**延期设计的规则**：C1 增量若某设计产物（如 prototype 视觉基准）延后，必须**显式登记**延期 + 到期检查点
（如「随 M12 收尾轮补」）+ DoD，不得默认滑过。

### 8.3 C2 大修改的 mini 闭环

按瀑布走「需求 → 设计 → 技术 → 返工 → 人工门」，但**按增量做、不重做全局**：
1. **需求**：是否改 PRD 正文由人拍板（改则升版本号并走评审，否则写增量需求文档入决策档）；
2. **设计**：只补受影响的页面/流程/异常态（`ia/flows/interaction/prototype` 增量），不重跑全局验证；
3. **技术**：ADR（必要时）/ data-model / api-contracts / task-breakdown 增补；
4. **返工**：改代码 + 单测，走对应模块人工门；
5. **禁止**：先改代码后补文档。

### 8.4 Agent / DSH 约束

1. **双向入口**：人发起变更（拍板「加/改/删一个功能」）同样走本流程——人给方向 → Agent 出「分级建议 + 影响面扫描」→
   人定级 → 执行；Agent 主动发现「缺/需改/需删」时，**先停人工门提报**（输出分级建议 + 影响面扫描），由人定级拍板。
   **两种入口都不得自行定级自行实现。**
2. C0/C1 拍板后 Agent 方可动；C2 一律停、走 §8.3，禁止先码后补。
3. `docs/prd/*` 为只读：Agent 不得写入（除非 C2 且有人明确指令）；DSH 侧对 `docs/prd/` 加 git 保护 / CODEOWNERS 兜底（机制层面）。
4. 接模块任务时，先读 `docs/dev/增量台账.md` 核对「有效需求」再动手。

### 8.5 修改 / 删除的专属要求

- **影响面扫描（改/删强制）**：C1/C2 的改、删类决策档必须附「影响面扫描」——列出所有引用该行为/字段/页面的文档与代码
  （grep 摘要即可），DoD = 逐处同步，缺一不算完成。
- **删除额外要求**：数据迁移方案（表/字段删除 → schema 版本 + 迁移脚本 + 旧数据处置）、备份兼容（旧 `.cpro` 能否恢复）、
  受影响 P6 用例与原型条目下线；台账登记为「🗑 已删除」而非「✅ 已实现」。
- **修改既有 Δ（amend）**：改一个已登记的增量时，台账**追加修订行**（如 D23 修订 D21），不覆盖原行；决策档标注 `amends D21`。

### 8.6 决策编号与有效需求版本

1. **编号**：D 系编号统一；`docs/dev/decisions/` 为 C1/C2 决策档统一落点，从 **D23 起连续编号**；
   历史决策全量索引见 `docs/dev/decisions/决策索引.md`（D1 开源为历史既定编号；D16–D22 按「D-N = STATUS 已决策事项第 N 条」）。
2. **有效需求版本**：`docs/dev/增量台账.md` 表头维护「有效需求 = PRD 定稿版 + Δ{Dn,...}」字符串，
   P6 系统测试、P8 复盘、移动端对齐统一引用该字符串，不各自拼写。
