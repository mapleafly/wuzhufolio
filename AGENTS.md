...
### 7.3 GUI 共性约束（2026-09-04 人工验收确立，P4 起各模块强制）

> 由 M2（密码弹窗键盘不可用）与 M5（Key 弹窗输入框不可键入、仅右键可用）两次同根缺陷确立——
> 新增任何含输入/交互的弹层前先读本节，遇平台输入异常先查是否触碰 ①，不另起炉灶。

1. **弹窗一律同窗口就地叠加**：禁止用 `Popup`/`Dialog` 承载交互（桌面端 Popup 含 focusable=false 均无法可靠
   接收键盘输入，且 focusable=true 会创建独立 AWT 窗口脱离 Compose 语义树）。统一使用 `WzModal`
   （2026-09-04 已改为就地叠加层实现：同窗口 Box 覆盖、焦点体系与主场景一致、Esc/遮罩/关闭按钮可关）。
2. **含输入框的弹窗打开即聚焦首输入框**：向 `WzModal` 传 `initialFocusRequester`（`WzTextField` 提供
   `fieldFocusRequester` 参数接入），键盘/IME 立即可用、无需先点按；不得只做「鼠标可点、右键可粘贴」的验收。
3. **验收强制项**：任何新增页面/弹窗含文本输入的模块，验收清单必须包含 ① Compose UI 测试
   `performTextInput` + `assertIsFocused` 路径 ② 人工 GUI 键盘逐字录入复验；二者缺一不得通过模块人工门。
4. 缺陷修复模式登记：修复后把根因/修法记入模块记录与本节，作为后续模块的预防性约束。
